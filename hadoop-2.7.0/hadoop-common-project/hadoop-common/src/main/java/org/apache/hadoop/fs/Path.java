...
public class Path implements Comparable {
  ...
  private long replication = -1;     // a replication for this path
  ...
  public static Path getPathWithoutSchemeAndAuthority(Path path) {
    ...
    Path newPath = path.isUriPathAbsolute() ?
      new Path(null, null, path.toUri().getPath(), path.replication) :
      path;
    return newPath;
  }
  ...
  public Path(Path parent, Path child) {
    ...
    initialize(resolved.getScheme(), resolved.getAuthority(),
               ...
               resolveReplication(parent, child));
  }

  private static long resolveReplication(Path parent, Path child) {
     // Child's replication has precedence over the parent's
     long replication = -1;
     if (child.replication > 0)
        replication = child.replication;
     else if (parent.replication > 0)
  ...
  /** Construct a path from a String.  Path strings are URIs, but with
   * unescaped elements and some additional normalization. */
  public Path(String pathString, long replication) throws IllegalArgumentException {
    ...
    initialize(scheme, authority, path, null, replication);
  }
  ...
  /** Construct a path from a URI  */
  public Path(URI aUri, long replication) {
    uri = aUri.normalize();
  ...
  public Path(String scheme, String authority, String path) {
    this(scheme, authority, path, -1);
  }
  
  /** Construct a Path from components. */
  public Path(String scheme, String authority, String path, long replication) {
    checkPathArg( path );
  ...
  private void initialize(String scheme, String authority, String path,
      String fragment, long replication) {
    try {
  ...
  public static Path mergePaths(Path path1, Path path2) {
    ...
    return new Path(path1.toUri().getScheme(), 
        ...
        resolveReplication(path1, path2));
  }
  ...
  public Path makeQualified(URI defaultUri, Path workingDir ) {
    ...
    return new Path(newUri, this.replication);
  }
}
