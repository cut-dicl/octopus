...
abstract class CommandWithDestination extends FsCommand {  
  ...
  private long replicationVector = -1l;
  ...
  protected void setReplicationVector(long replicationVector) {
  ...
  throws IOException {
    ...
    try {
      ...
      targetFs.writeStreamToFile(in, tempTarget, lazyPersist, replicationVector);
  ...
  private static class TargetFileSystem extends FilterFileSystem {
    ...
    void writeStreamToFile(InputStream in, PathData target,
                           boolean lazyPersist, long replVector) throws IOException {
      FSDataOutputStream out = null;
      try {
        out = create(target, lazyPersist, replVector);
    ...
    FSDataOutputStream create(PathData item, boolean lazyPersist, long replVector)
        throws IOException {
      try {
        if (lazyPersist) {
          ...
          return create(item.path, true,
              getConf().getInt("io.file.buffer.size", 4096),
              replVector < 1 ? getDefaultReplication(item.path) : replVector,
              getDefaultBlockSize(item.path));
        }
