...
class SetReplication extends FsCommand {
  ...
  public static final String DESCRIPTION =
    "Set the replication vector of a file. If <path> is a directory " +
    ...
    "Format: [DISK=<repl>,SSD=<repl>,MEMORY=<repl>,REMOTE=<repl>,UNSPEC=<repl>] " +
    "or a number. All parts are optional and may appear in any order.";
  
  protected ReplicationVector newRep = new ReplicationVector(0l);
  ...
  protected void processOptions(LinkedList<String> args) throws IOException {
    ...
    try {
      newRep.setVectorEncoding(ReplicationVector.ParseReplicationVector(
          args.removeFirst()));
    } catch (IllegalArgumentException e) {
      displayWarning("Illegal replication vector");
      throw e;
    }
    if (newRep.getTotalReplication() < 1) {
      throw new IllegalArgumentException("total replication must be >= 1");
    }
  }
  ...
  protected void processPath(PathData item) throws IOException {
    ...
    if (item.stat.isFile()) {
      if (!item.fs.setReplication(item.path, newRep.getVectorEncoding())) {
        throw new IOException("Could not set replication for: " + item);
      }
      out.println("Replication " + newRep.toString() + " set: " + item);
  ...
  private void waitForReplication() throws IOException {
    for (PathData item : waitList) {
      ...
      boolean done = false;
      int totalRep = newRep.getTotalReplication();
      while (!done) {
        item.refreshStatus();    
        ...
        for(; i < locations.length; i++) {
          ...
          if (currentRep != totalRep) {
            if (!printedWarning && currentRep > totalRep) {
