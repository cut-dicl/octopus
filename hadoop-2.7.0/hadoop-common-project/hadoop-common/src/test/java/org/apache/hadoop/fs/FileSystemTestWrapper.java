...
public final class FileSystemTestWrapper extends FSTestWrapper {
  ...
  public FSDataOutputStream create(Path f, EnumSet<CreateFlag> createFlag,
    ...
    // replication
    long replicationVector = fs.getDefaultReplication(f);
    CreateOpts.ReplicationVector repOpt =
        CreateOpts.getOpt(CreateOpts.ReplicationVector.class, opts);
    replicationVector = (repOpt != null) ? repOpt.getValue() : replicationVector;
    // blockSize
    ...
    return fs.create(f, permission, overwrite, bufferSize, replicationVector,
        blockSize, progress);
  }
