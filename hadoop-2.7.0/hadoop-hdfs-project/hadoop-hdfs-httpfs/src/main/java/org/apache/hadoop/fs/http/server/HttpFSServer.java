    ...
    switch (op.value()) {
      case CREATE: {
        ...
        if (!hasData) {
          ...
          Long replicationVector = params.get(ReplicationParam.NAME,
          ...
          FSOperations.FSCreate command =
            new FSOperations.FSCreate(is, path, permission, override,
                replicationVector, blockSize);
          fsExecute(user, command);
          AUDIT_LOG.info(
            "[{}] permission [{}] override [{}] replication [{}] blockSize [{}]",
            new Object[]{path, permission, override, replicationVector, blockSize});
      ...
      case SETREPLICATION: {
        ...
        FSOperations.FSSetReplication command =
          new FSOperations.FSSetReplication(path, replicationVector);
        JSONObject json = fsExecute(user, command);
        AUDIT_LOG.info("[{}] to [{}]", path, replicationVector);
