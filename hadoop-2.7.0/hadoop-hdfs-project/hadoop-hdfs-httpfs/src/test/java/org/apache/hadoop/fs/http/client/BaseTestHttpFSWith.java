...
public abstract class BaseTestHttpFSWith extends HFSTestCase {
  ...
  private void testCreate(Path path, boolean override) throws Exception {
    ...
    if (!isLocalFS()) {
      Assert.assertEquals(status.getTotalReplication(), 2);
  ...
  private void testListStatus() throws Exception {
    ...
    Assert.assertEquals(status2.getTotalReplication(), status1.getTotalReplication());
  ...
  private void testSetReplication() throws Exception {
    ...
    FileStatus status1 = fs.getFileStatus(path);
    fs.close();
    Assert.assertEquals(status1.getTotalReplication(), (short) 1);
  }
