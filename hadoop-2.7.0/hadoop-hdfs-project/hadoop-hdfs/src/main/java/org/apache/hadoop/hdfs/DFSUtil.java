...
public class DFSUtil {
  ...
  public static BlockLocation[] locatedBlocks2Locations(List<LocatedBlock> blocks) {
    ...
    for (LocatedBlock blk : blocks) {
      ...
      //--------------------------------------- 
      DatanodeInfo[] pendingCachedLocations = blk.getPendingCachedLocations();
      String[] pendingCachedHosts = new String[pendingCachedLocations.length];
      for (int i=0; i<pendingCachedLocations.length; i++) {
    	  pendingCachedHosts[i] = pendingCachedLocations[i].getHostName();
      }
      
      blkLocations[idx] = new BlockLocation(xferAddrs, hosts, blk.getStorageTypes(), cachedHosts,
                                            racks,
                                            blk.getStartOffset(),
                                            blk.getBlockSize(),
                                            blk.isCorrupt(),
