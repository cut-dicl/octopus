...
public class BlockStoragePolicy {
  ...
  public List<StorageType> chooseStorageTypes(final long replicationVector) {
    ...
    for (StorageType type : StorageType.values()) {
      short count = rv.getReplication(type);
      for (short k = 0; k < count; ++k) {
        types.add(type);
      }
    }
    
    // Distribute the remaining replicas based on the policy
    short replication = rv.getReplication(null);
    int i = 0, j = 0;
    ...
    for (;i < replication && j < storageTypes.length; ++j) {
      if (!storageTypes[j].isTransient()
          && rv.getReplication(storageTypes[j]) == 0) {
  ...
  public List<StorageType> chooseStorageTypes(final long replicationVector,
    ...
    return chooseStorageTypes(replicationVector, chosen, null);
  }

  private List<StorageType> chooseStorageTypes(final long replicationVector,
      final Iterable<StorageType> chosen, final List<StorageType> excess) {
    final List<StorageType> types = chooseStorageTypes(replicationVector);
   ...
   *
   * @param replicationVector the replication vector.
   * @param chosen the storage types of the chosen replicas.
  ...
  public List<StorageType> chooseStorageTypes(final long replicationVector,
    ...
    final List<StorageType> storageTypes = chooseStorageTypes(
        replicationVector, chosen, excess);
    ...
    if (storageTypes.size() < expectedSize) {
      LOG.warn("Failed to place enough replicas: expected size is " + expectedSize
          ...
          + "(replication=" + ReplicationVector.GetTotalReplication(replicationVector)
   ...
   * replication vector and the storage types of the chosen replicas.
   *
   * @param replicationVector the replication vector.
   * @param chosen the storage types of the chosen replicas.
   * @return a list of {@link StorageType}s for deletion.
   */
  public List<StorageType> chooseExcess(final long replicationVector,
      final Iterable<StorageType> chosen) {
    final List<StorageType> types = chooseStorageTypes(replicationVector);
    final List<StorageType> excess = new LinkedList<StorageType>();
    diff(types, chosen, excess);
  ...
  public ReplicationVector resolveReplicationVector(ReplicationVector currRV) {
    ...
    // Distribute the remaining replicas based on the policy
    short replication = currRV.getReplication(null);
    int i = 0, j = 0;
    ...
    for (; i < replication && j < storageTypes.length; ++j) {
      if (!storageTypes[j].isTransient()
          && currRV.getReplication(storageTypes[j]) == 0) {
        newRV.incrReplication(storageTypes[j], (short) 1);
        ++i;
      }
    }
    ...
    if (!last.isTransient()) {
      for (; i < replication; i++) {
        newRV.incrReplication(last, (short) 1);
      }
    }
