...
public class CacheDirectiveIterator
  ...
  private EnumSet<CacheFlag> flags;  //Added for Octopus Prefetching by Elena
  
  public CacheDirectiveIterator(ClientProtocol namenode,
      CacheDirectiveInfo filter, Sampler<?> traceSampler, EnumSet<CacheFlag> flags) {
    super(0L);
  ...
  public BatchedEntries<CacheDirectiveEntry> makeRequest(Long prevKey)
    ...
    try {
      entries = namenode.listCacheDirectives(prevKey, filter, flags);
    } catch (IOException e) {
      if (e.getMessage().contains("Filtering by ID is unsupported")) {
        ...
        entries = namenode.listCacheDirectives(id - 1, filter, flags);
