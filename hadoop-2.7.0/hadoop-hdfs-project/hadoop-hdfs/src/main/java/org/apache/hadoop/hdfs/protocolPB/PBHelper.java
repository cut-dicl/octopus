...
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.BlockCommandProto.Action;
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.BlockIdCommandProto;
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.BlockRecoveryCommandProto;
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.BlockReportContextProto;
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.CacheBlockReportProto;
...
import org.apache.hadoop.hdfs.protocol.proto.HdfsProtos.CipherSuiteProto;
...
import org.apache.hadoop.hdfs.protocol.proto.HdfsProtos.DatanodeInfoWithStorageProto;
...
import org.apache.hadoop.hdfs.server.protocol.CacheBlockReport;
...
public class PBHelper {
  ...
  static public DatanodeInfo convert(DatanodeInfoProto di) {
    ...
    return new DatanodeInfo(
        ...
        di.getCacheAvailable(), di.getCacheBpUsed(),
  ...
  public static DatanodeInfoProto convert(DatanodeInfo info) {
    ...
    builder
        ...
        .setCacheAvailable(info.getCacheRemaining())
        .setCacheBpUsed(info.getCacheBpUsed())
  ...
  //Added from Elena
  public static DatanodeInfoWithStorageProto convert(DatanodeInfoWithStorage b) {
      return DatanodeInfoWithStorageProto.newBuilder()
              .setStorageID(b.getStorageID())
              .setStorageType(convertStorageType(b.getStorageType()))
              .setDn(convert((DatanodeInfo)b))
              .build();

  }

  public static DatanodeInfoWithStorage convert(DatanodeInfoWithStorageProto proto) {
      return new DatanodeInfoWithStorage(PBHelper.convert(proto.getDn()), 
              proto.getStorageID(), 
  ...
  public static LocatedBlockProto convert(LocatedBlock b) {
    ...
    DatanodeInfoWithStorage[] cachedLocs = b.getCachedLocations();
    for (int i = 0; i < cachedLocs.length; ++i) {
        builder.addCachedLocs(PBHelper.convert(cachedLocs[i]));  
    }

    final String[] allStorageIDs = b.getAllStorageIDs();
    if (allStorageIDs != null) {
      builder.addAllAllSortedStorageIDs(Arrays.asList(allStorageIDs));
    }
    
    //---------------------------------------
    //Added for Octopus Prefetching by Elena
    //--------------------------------------- 
    DatanodeInfoWithStorage[] pendingCachedLocs = b.getPendingCachedLocations();
    for (int i = 0; i < pendingCachedLocs.length; ++i) {
        builder.addPendingCachedLocs(PBHelper.convert(pendingCachedLocs[i]));  
    }
    //--------------------------------------- 
  ...
  public static LocatedBlock convert(LocatedBlockProto proto) {
    ...
    List<DatanodeInfoWithStorageProto> cachedLocations = proto.getCachedLocsList();
    DatanodeInfoWithStorage[] cachedLocs = new DatanodeInfoWithStorage[cachedLocations.size()];
    for (int i = 0; i < cachedLocations.size(); i++) {
       cachedLocs[i] = PBHelper.convert(cachedLocations.get(i));
    }
    
    //---------------------------------------------------
    //Changes/additions for Octopus Prefetching by Elena
    //---------------------------------------------------
    List<DatanodeInfoWithStorageProto> pendingCachedLocations = proto.getPendingCachedLocsList();
    DatanodeInfoWithStorage[] pendingCachedLocs = new DatanodeInfoWithStorage[pendingCachedLocations.size()];
    for (int i = 0; i < pendingCachedLocations.size(); i++) {
    	pendingCachedLocs[i] = PBHelper.convert(pendingCachedLocations.get(i));
    }
    
    LocatedBlock lb = new LocatedBlock(PBHelper.convert(proto.getB()), targets,
        storageIDs, storageTypes, proto.getOffset(), proto.getCorrupt(),cachedLocs, pendingCachedLocs); 
    lb.setBlockToken(PBHelper.convert(proto.getBlockToken()));

    final int allSortedStorageIDsCount = proto.getAllSortedStorageIDsCount();
    final String[] allSortedStorageIDs;
    if (allSortedStorageIDsCount == 0) {
       allSortedStorageIDs = null;
    } else {
      Preconditions.checkState(allSortedStorageIDsCount == locs.size() + cachedLocations.size());
      allSortedStorageIDs = proto.getAllSortedStorageIDsList().toArray(
            new String[allSortedStorageIDsCount]);
    }

    lb.updateAllSortedLocations(allSortedStorageIDs);
  ...
  public static ReplicaStateProto convert(ReplicaState state) {
    switch (state) {
    case CACHING:
      return ReplicaStateProto.CACHING;
    case CACHED:
      return ReplicaStateProto.CACHED;  
    case RBW:
  ...
  public static BlockIdCommandProto convert(BlockIdCommand cmd) {
    ...
    builder.setType(convertStorageType(cmd.getStorageType()));
    
    return builder.build();
  }

  //Added from Elena
  public static List<CacheBlockReport> convertList(List<CacheBlockReportProto> reportsList) {
        if (reportsList == null)
            return new ArrayList<CacheBlockReport>(0);
        final int len = reportsList.size();
        List<CacheBlockReport> rlist = new ArrayList<CacheBlockReport>(len);
        for (int i = 0; i < len; ++i) {
            rlist.add(PBHelper.convert(reportsList.get(i)));
        }

        return rlist;
    }

  public static List<CacheBlockReportProto> convertListProto(List<CacheBlockReport> reportsList) {
        if (reportsList == null)
            return new ArrayList<CacheBlockReportProto>(0);
        final int len = reportsList.size();
        List<CacheBlockReportProto> rlist = new ArrayList<CacheBlockReportProto>(len);
        for (int i = 0; i < len; ++i) {
            rlist.add(PBHelper.convert(reportsList.get(i)));
        }
  ...
  public static CacheBlockReportProto convert(CacheBlockReport cbr) {
      CacheBlockReportProto.Builder builder = CacheBlockReportProto.newBuilder()
              .setStorage(convert(cbr.getStorage()))
              .addAllBlocks(cbr.getBlockIds());

      return builder.build();
  }

  public static CacheBlockReport convert(CacheBlockReportProto blkIdCmd) {
      return new CacheBlockReport(convert(blkIdCmd.getStorage()), blkIdCmd.getBlocksList());
  }
  ...
  public static BlockCommand convert(BlockCommandProto blkCmd) {
    ...
    if (blkCmd.getAction() == Action.INVALIDATE) {
        // Only the storage types list is sent
        targetStorageTypes = new StorageType[targetStorageTypesList.size()][];
        for(int i = 0; i < targetStorageTypes.length; i++) {
            List<StorageTypeProto> p = targetStorageTypesList.get(i).getStorageTypesList();
  ...
  public static BlockIdCommand convert(BlockIdCommandProto blkIdCmd) {
    ...
    return new BlockIdCommand(action, blkIdCmd.getBlockPoolId(), blockIds, convertStorageType(blkIdCmd.getType()));
  }
  ...
  public static ReceivedDeletedBlockInfoProto convert(
    ...
    case DELETED_BLOCK:
      ...
      break;
    case CACHED_BLOCK:
        status = ReceivedDeletedBlockInfoProto.BlockStatus.CACHED;
        break;
    case UNCACHED_BLOCK:
        status = ReceivedDeletedBlockInfoProto.BlockStatus.UNCACHED;
        break;
    default:
  ...
  public static ReceivedDeletedBlockInfo convert(
    ...
    case DELETED:
      ...
      break;
    case CACHED:
        status = BlockStatus.CACHED_BLOCK;
        break;
    case UNCACHED:
        status = BlockStatus.UNCACHED_BLOCK;
        break;
    }
  ...
  public static FsServerDefaults convert(FsServerDefaultsProto fs) {
    ...
    return new FsServerDefaults(
        ...
        fs.getWritePacketSize(), fs.getReplicationVector(),
  ...
  public static FsServerDefaultsProto convert(FsServerDefaults fs) {
    ...
    return FsServerDefaultsProto.newBuilder().
      ...
      .setReplicationVector(fs.getReplicationVector())
  ...
  public static int convertCacheFlags(EnumSet<CacheFlag> flags) {
    ...
    if(flags.contains(CacheFlag.SKIP_RESCAN)) {
        value |= CacheFlagProto.SKIP_RESCAN.getNumber();
    }
    return value;
  }
  ...
  public static EnumSet<CacheFlag> convertCacheFlags(int flags) {
    ...
    if((flags & CacheFlagProto.SKIP_RESCAN_VALUE) == CacheFlagProto.SKIP_RESCAN_VALUE) {
        result.add(CacheFlag.SKIP_RESCAN);
    }
    return result;
  }
  ...
  public static HdfsFileStatus convert(HdfsFileStatusProto fs) {
    ...
    return new HdfsLocatedFileStatus(
        ...
        fs.getReplicationVector(), fs.getBlocksize(),
  ...
  public static StorageTypeProto convertStorageType(StorageType type) {
    ...
    case MEMORY:
       return StorageTypeProto.MEMORY;
    case DISK_H:
       return StorageTypeProto.DISK_H;
    case SSD_H:
       return StorageTypeProto.SSD_H;
    case REMOTE:
       return StorageTypeProto.REMOTE;
    default:
  ...
  public static StorageType convertStorageType(StorageTypeProto type) {
    switch(type) {
      ...
      case DISK_H:
         return StorageType.DISK_H;
      case SSD_H:
         return StorageType.SSD_H;
      case REMOTE:
  ...
  public static StorageReportProto convert(StorageReport r) {
    StorageReportProto.Builder builder = StorageReportProto.newBuilder()
        ...
        .setStorage(convert(r.getStorage()))
    	  .setWriteThroughput(r.getWriteThroughput())
    	  .setReadThroughput(r.getReadThroughput())
        .setCacheCapacity(r.getCacheCapacity())
        .setCacheUsed(r.getCacheUsed())
        .setCacheRemaining(r.getCacheRemaining())
        .setCacheBpUsed(r.getCacheBpUsed())
        .setReaderCount(r.getReaderCount())
        .setWriterCount(r.getWriterCount());
    return builder.build();
  }
  ...
  public static StorageReport convert(StorageReportProto p) {
    return new StorageReport(
        ...
        p.getBlockPoolUsed(), p.getWriteThroughput(), p.getReadThroughput(), 
        p.getCacheCapacity(), p.getCacheUsed(), 
        p.getCacheRemaining(), p.getCacheBpUsed(),
        p.getReaderCount(), p.getWriterCount());
  }
  ...
  public static CacheDirectiveInfo convert
    ...
    if (proto.hasReplVector()) {
      //builder.setReplVector(Shorts.checkedCast(
       //   proto.hasReplVector()));
        builder.setReplVector(proto.getReplVector());
    }
    if (proto.hasPool()) {
  ...
  public static CacheDirectiveStatsProto convert(CacheDirectiveStats stats) {
    ...
    builder.setType(PBHelper.convertStorageType(stats.getStorageType()));
  ...
  public static CacheDirectiveStats convert(CacheDirectiveStatsProto proto) {
    ...
    builder.setStorageType(PBHelper.convertStorageType(proto.getType()));
    return builder.build();
  }
  ...
  public static CacheDirectiveEntryProto convert(CacheDirectiveEntry entry) {
    ...
    EnumMap<StorageType, CacheDirectiveStats> stats = entry.getStats();
    for(CacheDirectiveStats stat : stats.values()){
        builder.addStats(PBHelper.convert(stat));       
    }
  ...
  public static CacheDirectiveEntry convert(CacheDirectiveEntryProto proto) {
    ...
    EnumMap<StorageType, CacheDirectiveStats> stats = new EnumMap<StorageType, CacheDirectiveStats>(StorageType.class);

    for(int i=0; i<proto.getStatsCount(); i++){
        CacheDirectiveStats s = PBHelper.convert(proto.getStats(i));
        stats.put(s.getStorageType(), s);
    }
  ...
  public static CachePoolInfoProto convert(CachePoolInfo info) {
    ...
    EnumMap<StorageType, Long> typesLimit = info.getLimits();
    if ( typesLimit != null) {
        for(Map.Entry<StorageType, Long> e : typesLimit.entrySet()){
            builder.addTypes(convertStorageType(e.getKey()));
            builder.addLimitPerType(e.getValue());
        }
    }
    return builder.build();
  }
  ...
  public static CachePoolInfo convert (CachePoolInfoProto proto) {
    ...
    if (proto.hasMode()) {
      ...
      //Changed from Elena
      if(proto.getTypesCount() != proto.getLimitPerTypeCount())  
          throw new RuntimeException("The sizes of limit list and storage type list are not equal!!!");

      if (proto.getTypesCount()>0)  {
          List<StorageTypeProto> types = proto.getTypesList();
          List<Long> limits = proto.getLimitPerTypeList();

          EnumMap<StorageType, Long> typesLimit = new EnumMap<StorageType, Long>(StorageType.class);

          for(int i=0; i<types.size(); i++){
              typesLimit.put(convertStorageType(types.get(i)), limits.get(i));
          }

          info.setLimits(typesLimit);
      }
  ...
  public static CachePoolStatsProto convert(CachePoolStats stats) {
    ...
    builder.setFilesNeeded(stats.getFilesNeeded());
    builder.setFilesCached(stats.getFilesCached());
    builder.setType(convertStorageType(stats.getStorageType()));
    return builder.build();
  }
  ...
  public static CachePoolStats convert (CachePoolStatsProto proto) {
    ...
    builder.setFilesNeeded(proto.getFilesNeeded());
    builder.setFilesCached(proto.getFilesCached());
    builder.setStorageType(convertStorageType(proto.getType()));
    return builder.build();
  }
  ...
  public static CachePoolEntryProto convert(CachePoolEntry entry) {
    ...
    EnumMap<StorageType, CachePoolStats> stats = entry.getStats();
    for(CachePoolStats stat : stats.values()){
        builder.addStats(PBHelper.convert(stat));       
    }
  ...
  public static CachePoolEntry convert (CachePoolEntryProto proto) {
    ...
    //Changed from Elena
    EnumMap<StorageType, CachePoolStats> stats = new EnumMap<StorageType, CachePoolStats>(StorageType.class);

    for(int i=0; i<proto.getStatsCount(); i++){
        CachePoolStats s = PBHelper.convert(proto.getStats(i));
        stats.put(s.getStorageType(), s);
    }
        ...
        switch (p.getType()) {
          ...
          case EVENT_CREATE:
            ...
            events.add(new Event.CreateEvent.Builder()
                ...
                .replicationVector(create.getReplicationVector())
          ...
          case EVENT_METADATA:
            ...
            events.add(new Event.MetadataUpdateEvent.Builder()
                ...
                .mtime(meta.getMtime())
                .atime(meta.getAtime())
                .replicationVector(meta.getReplicationVector())
                .ownerName(
            ...
            events.add(InotifyProtos.EventProto.newBuilder()
                ...
                .setContents(
                    InotifyProtos.CreateEventProto.newBuilder()
                        ...
                        .setReplicationVector(ce2.getReplicationVector())
          ...
          case METADATA:
            ...
            InotifyProtos.MetadataUpdateEventProto.Builder metaB =
                InotifyProtos.MetadataUpdateEventProto.newBuilder()
                    ...
                    .setReplicationVector(me.getReplicationVector())
