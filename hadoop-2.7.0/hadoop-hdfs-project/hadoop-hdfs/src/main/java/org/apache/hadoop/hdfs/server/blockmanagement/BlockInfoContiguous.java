...
public class BlockInfoContiguous extends Block
  ...
  /**
   * Count the number of datanode storages the block belongs to.
   */
  public int numNodes() {
    assert this.triplets != null : "BlockInfo is not initialized";
    assert triplets.length % 3 == 0 : "Malformed BlockInfo";
  ...
  /**
   * Count the number of non-volatile datanode storages the block belongs to.
   */
  public int numNonVolatileNodes() {
     assert this.triplets != null : "BlockInfo is not initialized";
     assert triplets.length % 3 == 0 : "Malformed BlockInfo";
     ...
     for (int idx = 0; idx < len; idx++) {
       ...
       if (cur == null)
         break;
       if (!cur.getStorageType().isVolatile())
         count++;
     }
     return count;
  }
  ...
  /**
   * Return the number of expected non-volatile datanode storages
   * based on the block collection
   */
  public int numExpectedNonVolatileNodes() {
     if (bc == null)
        return 0;
     
      return ReplicationVector.GetNonVolatileReplicationCount(bc
            .getBlockReplicationVector());
  }
   ...
   */
  DatanodeStorageInfo findStorageInfo(DatanodeDescriptor dn, StorageType type) {
    int len = getCapacity();
    for (int idx = 0; idx < len; idx++) {
      DatanodeStorageInfo cur = getStorageInfo(idx);
      if (cur == null)
        break;
      if (cur.getDatanodeDescriptor() == dn && cur.getStorageType() == type)
        return cur;
    }
    return null;
  }
