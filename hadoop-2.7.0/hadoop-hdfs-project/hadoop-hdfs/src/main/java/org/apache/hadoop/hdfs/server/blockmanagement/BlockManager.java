...
import org.apache.hadoop.hdfs.server.namenode.replicationmanagement.ReplicationManager;
...
public class BlockManager {
  ...
  /** Store blocks -> DatanodeStorageInfo(s) map of corrupt replicas */
   ...
   * DataNodeStorageInfo. We'll eventually remove these extras.
   */
  ...
  /** Default total number of replicas */
  public final short defaultTotalReplication;
  ...
  private ReplicationManager replManager = null;
  ...
  public BlockManager(final Namesystem namesystem, final Configuration conf)
    ...
    BlockStoragePolicy policy = storagePolicySuite.getPolicy(HdfsConstants.DYNAMIC_STORAGE_POLICY_ID);
    if (policy != null)
       ((DynamicBlockStoragePolicy) policy).setStats(heartbeatManager);
    ...
    this.defaultTotalReplication = ReplicationVector.GetTotalReplication(conf
        .getLong(DFSConfigKeys.DFS_REPLICATION_KEY,
    ...
    LOG.info("defaultReplication         = " + defaultTotalReplication);
    ...
    LOG.info("blockPlacementPolicy       = " + blockplacement.getClass().getSimpleName());
    LOG.info("defaultStoragePolicy       = " + storagePolicySuite.getDefaultPolicy().getName());
  }
  ...
  public void setDefaultStoragePolicy(byte defaultPolicy) {
      storagePolicySuite.setDefaultPolicy(defaultPolicy);
  }
  ...
  private void dumpBlockMeta(Block block, PrintWriter out) {
    List<DatanodeStorageInfo> containingStorages = 
        new ArrayList<DatanodeStorageInfo>();
    List<DatanodeStorageInfo> containingLiveReplicasNodes = 
        new ArrayList<DatanodeStorageInfo>();
    
    NumberReplicationVectors numReplicas = new NumberReplicationVectors();
    // source node returned is not used
    chooseSourceDatanode(block, containingStorages,
        containingLiveReplicasNodes, numReplicas,
        UnderReplicatedBlocks.LEVEL, null);
    
    // containingLiveReplicasNodes can include READ_ONLY_SHARED replicas which are 
    // not included in the numReplicas.liveReplicas() count
    assert containingLiveReplicasNodes.size() >= numReplicas.liveReplicas().getTotalReplication();
    ReplicationVector usableRV = new ReplicationVector(numReplicas.liveReplicas());
    usableRV.addReplicationVector(numReplicas.decommissionedReplicas());
    ...
    out.print(block + ((usableRV.getTotalReplication() > 0)? "" : " MISSING") + 
              " (replicas:" +
    ...
    DatanodeDescrTypePairs corruptNodes = corruptReplicas.getNodes(block);
  ...
  private BlockInfoContiguous completeBlock(final BlockCollection bc,
    ...
    if (completeBlock.numExpectedNonVolatileNodes() >= 1) {
      namesystem.adjustSafeModeBlockTotals(0, 1);
      namesystem.incrementSafeBlockCount(
         Math.min(ucBlock.numNonVolatileNodes(), minReplication));
    }
  ...
  public LocatedBlock convertLastBlockToUnderConstruction(
    ...
    for (DatanodeStorageInfo storage : targets) {
      invalidateBlocks.remove(storage.getDatanodeDescriptor(),
          storage.getStorageType(), oldBlock);
    }
    
    // Adjust safe-mode totals, since under-construction blocks don't
    // count in safe-mode.
    if (ucBlock.numExpectedNonVolatileNodes() >= 1)
       namesystem.adjustSafeModeBlockTotals(
           // decrement safe if we had enough
           ucBlock.numNonVolatileNodes() >= minReplication ? -1 : 0,
  ...
  private List<DatanodeStorageInfo> getValidLocations(Block block) {
    ...
    for(DatanodeStorageInfo storage : blocksMap.getStorages(block)) {
      ...
      if (!invalidateBlocks.contains(storage.getDatanodeDescriptor(),
          storage.getStorageType(), block)) {
  ...
  /**
   * Clamp the specified total replication between the minimum and the maximum
   * replication levels.
   */
  public long adjustReplication(long replicationVector) {
    ReplicationVector rv = new ReplicationVector(replicationVector);
    short oldTotalRepl = rv.getTotalReplication();
    short newTotalRepl = oldTotalRepl < minReplication ? minReplication
        : oldTotalRepl > maxReplication ? maxReplication : oldTotalRepl;

    if (newTotalRepl > oldTotalRepl) {
      // Add the difference to the default type
      rv.incrReplication(StorageType.DEFAULT,
          (short) (newTotalRepl - oldTotalRepl));
    } else if (newTotalRepl < oldTotalRepl) {
      // Subtract the difference from the default type
      rv.decrReplication(StorageType.DEFAULT,
          (short) (oldTotalRepl - newTotalRepl));

      oldTotalRepl = rv.getTotalReplication();
      if (oldTotalRepl > newTotalRepl) {
          rv.removeReplicas((short) (oldTotalRepl - newTotalRepl));
      }
    }
  ...
  /**
   * Resolve the replication vector of the inode, i.e., determine the storage
   * types for any unspecified replicas in the vector
   * 
   * @return a replication vector without unspecified replicas
   */
   public long resolveReplication(long replicationVector,
                                  byte storagePolicyID) {

    ReplicationVector rv = new ReplicationVector(replicationVector);
    if (rv.getReplication(null) > 0) {
      BlockStoragePolicy storagePolicy = getStoragePolicy(storagePolicyID);
      if (storagePolicy != null) {
        rv = storagePolicy.resolveReplicationVector(rv);
      }
    }
   ...
   */
   public void verifyReplication(String src,
    ...
    short replication = ReplicationVector.GetTotalReplication(replicationVector);
  ...
  void removeBlocksAssociatedTo(final DatanodeDescriptor node) {
    ...
    while(it.hasNext()) {
      removeStoredBlock(it.next(), node, null);
    }
  ...
  void removeBlocksAssociatedTo(final DatanodeStorageInfo storageInfo) {
    ...
    StorageType type = storageInfo.getStorageType();
    
    while(it.hasNext()) {
      Block block = it.next();
      removeStoredBlock(block, node, type);
      invalidateBlocks.remove(node, type, block);
    }
   ...
   * datanode and storage type, and log the operation
   */
  void addToInvalidates(final Block block, final DatanodeInfo datanode,
    ...
    invalidateBlocks.add(block, datanode, type, true);
  }
  ...
  private void addToInvalidates(Block b) {
    ...
    for(DatanodeStorageInfo storage : blocksMap.getStorages(b, State.NORMAL)) {
      final DatanodeDescriptor node = storage.getDatanodeDescriptor();
      invalidateBlocks.add(b, node, storage.getStorageType(), false);
      datanodes.append(node).append("[").append(storage.getStorageType()).append("] ");
    }
  ...
  public void findAndMarkBlockAsCorrupt(final ExtendedBlock blk,
      final DatanodeInfo dn, String storageID, StorageType type, String reason)
      throws IOException {
    ...
    markBlockAsCorrupt(new BlockToMarkCorrupt(storedBlock,
        ...
        type == null ? StorageType.DEFAULT : type,
        node);
  }
   ...
   * @param type storage type. should be the same as the storage info if not null.
   * @throws IOException
   */
  private void markBlockAsCorrupt(BlockToMarkCorrupt b,
    ...
    if (bc == null) {
      ...
      addToInvalidates(b.corrupted, node, type);
      return;
    } 
    ...
    corruptReplicas.addToCorruptReplicasMap(b.corrupted, node, type, b.reason,
        b.reasonCode);
  ...
  /**
   * Invalidates the given block on the given datanode on the given storage
   * type.
  ...
  private boolean invalidateBlock(BlockToMarkCorrupt b, DatanodeInfo dn,
    ...
    blockLog.info("BLOCK* invalidateBlock: {} on {} [{}]", b, dn, type);
    DatanodeDescriptor node = getDatanodeManager().getDatanode(dn);
    if (node == null) {
    ...
    if (nr.replicasOnStaleNodes() > 0) {
      ...
      addToInvalidates(b.corrupted, dn, type);
      removeStoredBlock(b.stored, node, type);
  ...
  int computeReplicationWorkForBlocks(List<List<Block>> blocksToReplicate) {
    ...
    ReplicationVector requiredRV = new ReplicationVector(0l);
    ReplicationVector effectiveRV = new ReplicationVector(0l);
    ReplicationVector additionalRequiredRV = new ReplicationVector(0l);
      ...
      synchronized (neededReplications) {
        for (int priority = 0; priority < blocksToReplicate.size(); priority++) {
          for (Block block : blocksToReplicate.get(priority)) {
            ...
            containingStorages = new ArrayList<DatanodeStorageInfo>();
            List<DatanodeStorageInfo> liveReplicaNodes = new ArrayList<DatanodeStorageInfo>();
            NumberReplicationVectors numReplicas = new NumberReplicationVectors();
            srcNode = chooseSourceDatanode(
                block, containingStorages, liveReplicaNodes, numReplicas,
                priority, null);
            ...
            requiredRV.setVectorEncoding(bc.getBlockReplicationVector());

            // liveReplicaNodes can include READ_ONLY_SHARED replicas which are 
            // not included in the numReplicas.liveReplicas() count
            assert liveReplicaNodes.size() >= numReplicas.liveReplicas().getTotalReplication();

            // do not schedule more if enough replicas is already pending
            effectiveRV.setReplicationVector(numReplicas.liveReplicas());
            effectiveRV.addReplicationVector(pendingReplications
                .getReplicationVector(block));
      
            if (effectiveRV.isAllGreaterOrEqual(requiredRV)) {
            ...
            boolean useSrcAsTarget = false;
            if (requiredRV.isAnyGreater(numReplicas.liveReplicas())) {
              // Need additional replicas
              additionalRequiredRV.setReplicationVector(requiredRV);
              additionalRequiredRV.subtractReplicationVector(effectiveRV);
              
              if (numReplicas.liveReplicas().isAnyGreater(requiredRV)) {
                 // Some replica is no longer needed. Choose that location as src
                 ReplicationVector hintRV = new ReplicationVector(numReplicas.liveReplicas());
                 hintRV.subtractReplicationVector(requiredRV);
                 DatanodeDescriptor srcNode2 = chooseSourceDatanode(
                       block, containingStorages, liveReplicaNodes, null,
                       priority, hintRV);
                 if (srcNode2 != null) {
                    srcNode = srcNode2;
                    useSrcAsTarget = true;
              ...
              additionalRequiredRV.setVectorEncoding(0l);
              additionalRequiredRV.setReplication(null, (short) 1);
            }
            
            work.add(new ReplicationWork(block, bc, srcNode, containingStorages,
                liveReplicaNodes, additionalRequiredRV,
                priority, useSrcAsTarget));
          }
        }
      }
    } finally {
    ...
    for(ReplicationWork rw : work){
      // Exclude all decommissioning or corrupt containing nodes 
      // from being targets. Exclude live-replica nodes if the storage
      // type of the replica matches a target type
      excludedNodes.clear();
      for (DatanodeStorageInfo dsi : rw.containingStorages) {
        if (!rw.liveReplicaStorages.contains(dsi) ||
            rw.additionalReplVector.getReplication(dsi.getStorageType()) > 0) {
          excludedNodes.add(dsi.getDatanodeDescriptor());
        }
      }
    ...
    try {
      for(ReplicationWork rw : work){
        ...
        synchronized (neededReplications) {
          ...
          // do not schedule more if enough replicas is already pending
          NumberReplicationVectors numReplicas = countNodesPerType(block);
          effectiveRV.setReplicationVector(numReplicas.liveReplicas());
          effectiveRV.addReplicationVector(pendingReplications
              .getReplicationVector(block));
          ...
          if ( (numReplicas.liveReplicas().isAllGreaterOrEqual(requiredRV)) &&
               (!blockHasEnoughRacks(block)) ) {
          ...
          effectiveRV.addReplicationVector(DatanodeStorageInfo
              .createReplicationVector(targets));
          if(effectiveRV.isAllGreaterOrEqual(requiredRV)) {
            neededReplications.remove(block, priority); // remove from neededReplications
            neededReplications.decrementReplicationIndex(priority);
          }
      ...
      for(ReplicationWork rw : work){
        ...
        if (targets != null && targets.length != 0) {
          ...
          for (int k = 0; k < targets.length; k++) {
            ...
            targetList.append('[');
            targetList.append(targets[k].getStorageType());
            targetList.append(']');
          }
  ...
  public DatanodeStorageInfo[] chooseTarget4WebHDFS(String src,
    ...
    return blockplacement.chooseTarget(src, 1l, clientnode,
  ...
  public DatanodeStorageInfo[] chooseTarget4AdditionalDatanode(String src,
    ...
    return blockplacement.chooseTarget(src, additionalReplVector, clientnode,
   ...
   * @see BlockPlacementPolicy#chooseTarget(String, long, Node,
   *      Set, long, List, BlockStoragePolicy)
   */
  public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src,
      final long replicationVector, final Node client,
    ...
    final DatanodeStorageInfo[] targets = blockplacement.chooseTarget(src,
        replicationVector, client, 
        new ArrayList<DatanodeStorageInfo>(totalReplicas), 
        false, excludedNodes, blocksize, 
   ...
   *
   * Hero: If a storage contains a block replica that is not needed and
   * is not in the excess list, we want to use it as a source. 
   * This scenario appears when the replication vector of a file changes
   * and causes a replica to move from one tier to another.
   * The relevant storage types are provided as hint.
   *
   * In addition form a list of all nodes containing the block
   * and calculate its replication numbers.
   *
   * @param block Block for which a replication source is needed
   * @param containingStorages List to be populated with storages found to contain 
   *                           the given block
   ...
   *                                   block.
   * @param hintRV Replication vector with preferred storage types. Could be null.
   ...
   */
   ...
   DatanodeDescriptor chooseSourceDatanode(Block block,
       ...
       NumberReplicationVectors numReplicas,
       int priority,
       ReplicationVector hintRV) {
    containingStorages.clear();
    nodesContainingLiveReplicas.clear();
    DatanodeDescriptor srcNode = null;
    ReplicationVector live = new ReplicationVector(0l);
    ReplicationVector decommissioned = new ReplicationVector(0l);
    ReplicationVector corrupt = new ReplicationVector(0l);
    ReplicationVector excess = new ReplicationVector(0l);
    ReplicationVector stale = new ReplicationVector(0l);
    ReplicationVector nonVolatileLive = new ReplicationVector(0l);
    boolean switchNodes = true;
    
    DatanodeDescrTypePairs nodesCorrupt = corruptReplicas.getNodes(block);
    for(DatanodeStorageInfo storage : blocksMap.getStorages(block)) {
      final DatanodeDescriptor node = storage.getDatanodeDescriptor();
      final StorageType type = storage.getStorageType();
      LightWeightLinkedSet<Block> excessBlocks =
        excessReplicateMap.get(storage.getStorageID());
      short countableReplica = (short) (storage.getState() == State.NORMAL ? 1 : 0); 
      if ((nodesCorrupt != null) && (nodesCorrupt.contains(node, type)))
        corrupt.incrReplication(type, countableReplica);
      else if (node.isDecommissionInProgress() || node.isDecommissioned())
        decommissioned.incrReplication(type, countableReplica);
      else if (excessBlocks != null && excessBlocks.contains(block)) {
        excess.incrReplication(type, countableReplica);
      } else {
        nodesContainingLiveReplicas.add(storage);
        live.incrReplication(type, countableReplica);
        if (!type.isVolatile())
           nonVolatileLive.incrReplication(type, countableReplica);
      }
      containingStorages.add(storage);
      ...
      if (srcNode == null) {
        srcNode = node;
        if (hintRV != null && hintRV.getReplication(type) > 0)
           switchNodes = false;
        continue;
      }

      if (hintRV != null && hintRV.getReplication(type) > 0) {
         // See Hero's note in method's comment
         srcNode = node;
         switchNodes = false;
      } else if (switchNodes && DFSUtil.getRandom().nextBoolean()) {
    ...
    if(numReplicas != null)
        numReplicas.initialize(live, decommissioned, corrupt, excess, stale,
               nonVolatileLive);
    return srcNode;
  }
    ...
    if (timedOutItems != null) {
      ...
      try {
        for (int i = 0; i < timedOutItems.length; i++) {
          NumberReplicationVectors num = countNodesPerType(timedOutItems[i]);
          ReplicationVector expectedRV = getReplicationVector(timedOutItems[i]);
          if (isNeededReplication(timedOutItems[i], expectedRV, num.liveReplicas())) {
  ...
  private void removeZombieReplicas(BlockReportContext context,
    ...
    DatanodeDescriptor dn = zombie.getDatanodeDescriptor();
    StorageType type = zombie.getStorageType();
    ...
    while (iter.hasNext()) {
      ...
      removeStoredBlock(block, dn, type);
      invalidateBlocks.remove(dn, type, block);
    }
  ...
  private Collection<Block> processReport(
    ...
    for (Block b : toInvalidate) {
      addToInvalidates(b, node, type);
    }
    for (BlockToMarkCorrupt b : toCorrupt) {
      markBlockAsCorrupt(b, storageInfo, type, node);
    }
  ...
  public void markBlockReplicasAsCorrupt(BlockInfoContiguous block, 
    ...
    for (DatanodeStorageInfo storage : getStorages(block)) {
      ...
      if (isCorrupt) {
        ...
        markBlockAsCorrupt(b, storage, storage.getStorageType(),
    ...
    for (BlockReportReplica iblk : report) {
      ...
      if (c != null) {
        if (shouldPostponeBlocksFromFuture) {
          ...
          markBlockAsCorrupt(c, storageInfo, storageInfo.getStorageType(),
              storageInfo.getDatanodeDescriptor());
        }
        continue;
      }
      ...
      if (isBlockUnderConstruction(storedBlock, ucState, reportedState)) {
        ...
        if (namesystem.isInSnapshot(blockUC)) {
          int numOfReplicas = blockUC.numExpectedNonVolatileNodes();
  ...
  private BlockInfoContiguous processReportedBlock(
    ...
    if(invalidateBlocks.contains(dn, type, block)) {
      /*
    ...
    if (reportedState == ReplicaState.FINALIZED
        && (storedBlock.findStorageInfo(storageInfo) == -1 ||
            corruptReplicas.isReplicaCorrupt(storedBlock, dn, type))) {
  ...
  throws IOException {
    ...
    NumberReplicas numReplicas = countNodes(storedBlock);
    if (storedBlock.getBlockUCState() == BlockUCState.COMMITTED
        && numReplicas.liveReplicas() >= minReplication) {
      ...
      namesystem.incrementSafeBlockCount(numReplicas.nonVolatileLiveReplicas());
    }
  }
  ...
  throws IOException {
    ...
    if (result == AddBlockResult.ADDED) {
      ...
      blockLog.warn("BLOCK* addStoredBlock: block {} moved to storageType " +
          "{} on node {}", storedBlock, type, node);
    } else {
      // if the same block is added again and the replica was corrupt
      // previously because of a wrong gen stamp, remove it from the
      // corrupt block list.
      corruptReplicas.removeFromCorruptReplicasMap(block, node, type,
    ...
    if (bc != null && replManager != null 
          && bc instanceof INodeFile && storedBlock.isComplete()) {
       replManager.invokePostReplicationProcess((INodeFile) bc, storedBlock);
    }

    // Now check for completion of blocks and safe block count
    NumberReplicationVectors num = countNodesPerType(storedBlock);
    ReplicationVector liveReplicasRV = num.liveReplicas();
    ReplicationVector currentReplicasRV = new ReplicationVector(liveReplicasRV);
    currentReplicasRV.addReplicationVector(pendingReplications
        .getReplicationVector(storedBlock));

    if(storedBlock.getBlockUCState() == BlockUCState.COMMITTED &&
        liveReplicasRV.getTotalReplication() >= minReplication) {
      ...
      // handles the safe block count maintenance.
      namesystem.incrementSafeBlockCount(
            currentReplicasRV.getNonVolatileReplicationCount());
    }
    ...
    ReplicationVector expectedReplicasRV = new ReplicationVector(
        bc.getBlockReplicationVector());
    if (isNeededReplication(storedBlock, expectedReplicasRV, currentReplicasRV)) {
      updateNeededReplications(storedBlock, curReplicaDelta, 0);
    } else {
      neededReplications.remove(storedBlock, currentReplicasRV,
          num.decommissionedReplicas(), expectedReplicasRV);
    }

    // handle overReplication
    if (currentReplicasRV.isAnyGreater(expectedReplicasRV)) {
      processOverReplicatedBlock(storedBlock,
          expectedReplicasRV.getVectorEncoding(), storageInfo, delNodeHint);
    }
    
    // If the file replication has reached desired value
    // we can remove any corrupt replicas the block may have
    int corruptReplicasCount = corruptReplicas.numCorruptReplicas(storedBlock);
    int numCorruptNodes = num.corruptReplicas().getTotalReplication();
    if (numCorruptNodes != corruptReplicasCount) {
      LOG.warn("Inconsistent number of corrupt replicas for " +
          storedBlock + "blockMap has " + numCorruptNodes + 
          " but corrupt replicas map has " + corruptReplicasCount);
    }
    if ((corruptReplicasCount > 0)
        && (liveReplicasRV.isAllGreaterOrEqual(expectedReplicasRV)))
  ...
  private void logAddStoredBlock(BlockInfoContiguous storedBlock,
    ...
    sb.append("BLOCK* addStoredBlock: blockMap updated: ")
      .append(storageInfo.getDatanodeDescriptor())
      .append('[').append(storageInfo.getStorageType()).append(']')
      .append(" is added to ");
  ...
  private void invalidateCorruptReplicas(BlockInfoContiguous blk) {
    DatanodeDescrTypePairs nodes = corruptReplicas.getNodes(blk);
    boolean removedFromBlocksMap = true;
    if (nodes == null)
      return;
    // make a copy of the array of nodes in order to avoid
    // ConcurrentModificationException, when the block is removed from the node
    DatanodeDescrTypePair[] nodesCopy = nodes.toArray();
    for (DatanodeDescrTypePair pair : nodesCopy) {
      try {
        if (!invalidateBlock(new BlockToMarkCorrupt(blk, null,
              Reason.ANY), pair.getDatanodeDescriptor(), pair.getStorageType())) {
  ...
  private MisReplicationResult processMisReplicatedBlock(BlockInfoContiguous block) {
    ...
    ReplicationVector expectedRV = new ReplicationVector(bc.getBlockReplicationVector());
    NumberReplicationVectors num = countNodesPerType(block);
    
    // add to under-replicated queue if need to be
    if (isNeededReplication(block, expectedRV, num.liveReplicas())) {
      if (neededReplications.add(block, num.liveReplicas(), num
          .decommissionedReplicas(), expectedRV)) {
        
        if (num.liveReplicas().isAnyGreater(expectedRV)
            && num.replicasOnStaleNodes().getTotalReplication() == 0) {
          // under- AND over-replicated block
          processOverReplicatedBlock(block, expectedRV.getVectorEncoding(), null, null);
          return MisReplicationResult.UNDER_OVER_REPLICATED;
        } else {
    ...
    if (num.liveReplicas().isAnyGreater(expectedRV)) {
      if (num.replicasOnStaleNodes().getTotalReplication() > 0) {
  ...
  /** Set replication vectors for the blocks. */
  public void setReplication(final long oldReplVector, final long newReplVector,
      final String src, final Block... blocks) {
    if (newReplVector == oldReplVector) {
      return;
    }

    // HERO TODO fix per type - might be ok??
    short oldRepl = ReplicationVector.GetTotalReplication(oldReplVector);
    short newRepl = ReplicationVector.GetTotalReplication(newReplVector);
    ...
    if (oldRepl > newRepl) {
      ...
      LOG.info("Decreasing replication from " + oldRepl + " to " + newRepl
          + " for " + src);
      for(Block b : blocks) {
        processOverReplicatedBlock(b, newReplVector, null, null);
      }
    } else if (oldRepl < newRepl) { // replication factor is increased
      LOG.info("Increasing replication from " + oldRepl + " to " + newRepl
          + " for " + src);
    } else {
      LOG.info("Changing replication from "
          + ReplicationVector.StringifyReplVector(oldReplVector) + " to "
  ...
  private void processOverReplicatedBlock(final Block block,
      final long replicationVector, 
      final DatanodeStorageInfo addedNode,
      DatanodeDescriptor delNodeHint) {
    assert namesystem.hasWriteLock();
    if (addedNode != null && addedNode.getDatanodeDescriptor() == delNodeHint) {
      delNodeHint = null;
    }
    ...
    for(DatanodeStorageInfo storage : blocksMap.getStorages(block, State.NORMAL)) {
      ...
      LightWeightLinkedSet<Block> excessBlocks = excessReplicateMap.get(storage
          .getStorageID());
      if (excessBlocks == null || !excessBlocks.contains(block)) {
    ...
    chooseExcessReplicates(nonExcess, block, replicationVector, 
  ...
  private void chooseExcessReplicates(final Collection<DatanodeStorageInfo> nonExcess, 
                              Block b, long replicationVector,
                              DatanodeStorageInfo addedNode,
                              DatanodeDescriptor delNodeHint,
                              BlockPlacementPolicy replicator) {
    assert namesystem.hasWriteLock();
    
    short totalRepl = ReplicationVector.GetTotalReplication(replicationVector);
    ...
    DatanodeStorageInfo delNodeHintStorage
        = DatanodeStorageInfo.getDatanodeStorageInfo(nonExcess, delNodeHint);
    DatanodeStorageInfo addedNodeStorage = null;
    if (addedNode != null) {
       for (DatanodeStorageInfo storage : nonExcess) {
          if (addedNode == storage) {
             addedNodeStorage = addedNode;
          }
       }
    }

    // special case: check if the newlly added storage was on a datanode
    // that already had a replica on a different excess tier
    if (addedNodeStorage != null && delNodeHintStorage == null) {
       for (DatanodeStorageInfo storage : moreThanOne) {
          if (addedNodeStorage.getDatanodeDescriptor() == storage.getDatanodeDescriptor()
                && addedNodeStorage.getStorageType() != storage.getStorageType()
                && excessTypes.contains(storage.getStorageType())) {
             delNodeHintStorage = storage;
             break;
          }
       }
    }
    
    // special case: check if there are two replicas on the same node
    // and one of the tiers is an excess
    if (addedNodeStorage == null && delNodeHintStorage == null) {
       for (DatanodeStorageInfo s1 : moreThanOne) {
            if (excessTypes.contains(s1.getStorageType())) {
             for (DatanodeStorageInfo s2 : moreThanOne) {
                if (s1.getDatanodeDescriptor() == s2.getDatanodeDescriptor()
                      && s1.getStorageType() != s2.getStorageType()) {
                   delNodeHintStorage = s1;
                   break;
                }
             }
             
             if (delNodeHintStorage != null)
                break;
          }
       }
    }
    ...
    while (nonExcess.size() - totalRepl > 0) {
      ...
      } else { // regular excess replica removal
        cur = replicator.chooseReplicaToDelete(bc, b, totalRepl, // totalRepl not used
      ...
      //
      addToInvalidates(b, cur.getDatanodeDescriptor(), cur.getStorageType());
  ...
  private void addToExcessReplicate(DatanodeStorageInfo dn, Block block) {
    assert namesystem.hasWriteLock();
  ...
  public void removeStoredBlock(Block block, DatanodeDescriptor node,
      StorageType optionalType) {
    blockLog.info("BLOCK* removeStoredBlock: {} from {}[{}]", block, node, 
          ((optionalType != null) ? optionalType : "ALL"));
    assert (namesystem.hasWriteLock());
    {
      if (!blocksMap.removeNode(block, node, optionalType)) {
      ...
      for (DatanodeStorageInfo storage : node.getStorageInfos()) {
        if (optionalType == null || storage.getStorageType() == optionalType) {
           removeExcessBlock(storage, block);
        }
      }
      ...
      if (optionalType == null)
        ...
        corruptReplicas.removeFromCorruptReplicasMap(block, node, optionalType);
    }
  }

  public void removeExcessBlock(DatanodeStorageInfo storage, Block block) {
     LightWeightLinkedSet<Block> excessBlocks = excessReplicateMap
           .get(storage.getStorageID());
       if (excessBlocks != null) {
         if (excessBlocks.remove(block)) {
           ...
           if (excessBlocks.size() == 0) {
             excessReplicateMap.remove(storage.getStorageID());
         }
       }
     }
  }
  ...
  void addBlock(DatanodeStorageInfo storageInfo, Block block, String delHint)
    ...
    //
    Block blk = blocksMap.getStoredBlock(block);
    pendingReplications.decrement(blk == null ? block : blk, storageInfo);
  ...
  public void processIncrementalBlockReport(final DatanodeID nodeID,
    ...
    for (ReceivedDeletedBlockInfo rdbi : rdbs) {
      ...
      case DELETED_BLOCK:
        removeStoredBlock(rdbi.getBlock(), node, storageInfo.getStorageType());
        deleted++;
        break;
      case RECEIVED_BLOCK:
  ...
  public NumberReplicas countNodes(Block b) {
    ...
    int excess = 0;
    int stale = 0;
    int nonVolatileLive = 0;

    DatanodeDescrTypePairs nodesCorrupt = corruptReplicas.getNodes(b);
    for(DatanodeStorageInfo storage : blocksMap.getStorages(b, State.NORMAL)) {
      final DatanodeDescriptor node = storage.getDatanodeDescriptor();
      if ((nodesCorrupt != null) && (nodesCorrupt.contains(storage))) {
        corrupt++;
      } else if (node.isDecommissionInProgress() || node.isDecommissioned()) {
        decommissioned++;
      } else {
        ...
        if (blocksExcess != null && blocksExcess.contains(b)) {
          ...
          live++;
          if (!storage.getStorageType().isVolatile())
             nonVolatileLive++;
        }
      }
      if (storage.areBlockContentsStale()) {
    ...
    return new NumberReplicas(live, decommissioned, corrupt, excess, stale, nonVolatileLive);
  }

  /**
   * Return the number of node types hosting a given block, grouped by the state
   * of those replicas.
   */
  public NumberReplicationVectors countNodesPerType(Block b) {
    ReplicationVector live = new ReplicationVector(0l);
    ReplicationVector decommissioned = new ReplicationVector(0l);
    ReplicationVector corrupt = new ReplicationVector(0l);
    ReplicationVector excess = new ReplicationVector(0l);
    ...
    for (DatanodeStorageInfo storage : blocksMap.getStorages(b, State.NORMAL)) {
      ...
      final StorageType type = storage.getStorageType();
      if ((nodesCorrupt != null) && (nodesCorrupt.contains(storage))) {
        corrupt.incrReplication(type, (short) 1);
      } else if (node.isDecommissionInProgress() || node.isDecommissioned()) {
        decommissioned.incrReplication(type, (short) 1);
      } else {
        LightWeightLinkedSet<Block> blocksExcess = excessReplicateMap
            .get(storage.getStorageID());
        if (blocksExcess != null && blocksExcess.contains(b)) {
          excess.incrReplication(type, (short) 1);
        } else {
          live.incrReplication(type, (short) 1);
          if (!type.isVolatile())
             nonVolatileLive.incrReplication(type, (short) 1);
        }
      }
      if (storage.areBlockContentsStale()) {
        stale.incrReplication(type, (short) 1);
      }
    }
    return new NumberReplicationVectors(live, decommissioned, corrupt, excess,
        stale, nonVolatileLive);
  }
  ...
  void processOverReplicatedBlocksOnReCommission(
    ...
    while(it.hasNext()) {
      ...
      BlockCollection bc = blocksMap.getBlockCollection(block);
      long replicationVector = bc.getBlockReplicationVector();
      ReplicationVector expectedRV = new ReplicationVector(replicationVector);
      NumberReplicationVectors num = countNodesPerType(block);

      if (num.liveReplicas().isAnyGreater(expectedRV)) {
        // over-replicated block 
        processOverReplicatedBlock(block, replicationVector, null, null);
        numOverReplicated++;
      }
    }
  ...
  /**
   * Count the number of complete blocks that are suppose to have
   * at least 1 replica on non-volatile datanode storage
   * 
   * Note: This method is expensive as it iterates over all blocks
   * @return
   */
  public int getCompleteNonVolatileBlocks() {
     int numBlocks = 0;
     
     for (BlockInfoContiguous block : blocksMap.getBlocks()) {
        if (block.isComplete()
            && block.numExpectedNonVolatileNodes() >= 1)
           numBlocks++;
     }
  ...
  @VisibleForTesting
  public int removePendingBlock(BlockInfoContiguous block) {
     // Only used for testing
     pendingReplications.remove(block);
     BlockCollection bc = block.getBlockCollection();
     if (bc != null && replManager != null && bc instanceof INodeFile) {
        return replManager.invokePostReplicationProcess((INodeFile) bc, block);
     }
  ...
  public boolean containsNeededReplications(final Block block) {
     return neededReplications.contains(block);
  }
  ...
  private void updateNeededReplications(final Block block,
    ...
    try {
      ...
      NumberReplicationVectors repl = countNodesPerType(block);
      ReplicationVector curExpectedReplicas = getReplicationVector(block);
      if (isNeededReplication(block, curExpectedReplicas, repl.liveReplicas())) {
        neededReplications.update(block, repl.liveReplicas()
            .getTotalReplication(), repl.decommissionedReplicas()
            .getTotalReplication(), curExpectedReplicas.getTotalReplication(),
            curReplicasDelta, expectedReplicasDelta);
      } else {
        int oldReplicas = repl.liveReplicas().getTotalReplication()
            - curReplicasDelta;
        int oldExpectedReplicas = curExpectedReplicas.getTotalReplication()
            - expectedReplicasDelta;
        neededReplications.remove(block, oldReplicas, repl
  ...
  public void checkReplication(BlockCollection bc) {
    final ReplicationVector expectedRV = new ReplicationVector(
        bc.getBlockReplicationVector());
    
    for (Block block : bc.getBlocks()) {
      final NumberReplicationVectors n = countNodesPerType(block);
      if (isNeededReplication(block, expectedRV, n.liveReplicas())) { 
        neededReplications.add(block, n.liveReplicas(),
            n.decommissionedReplicas(), expectedRV);
      }
      if (n.liveReplicas().isAnyGreater(expectedRV)) {
        processOverReplicatedBlock(block, expectedRV.getVectorEncoding(), null, null);
      }
    }
  }
   ...
   *         otherwise, return the replication factor of the block.
   */
  private ReplicationVector getReplicationVector(Block block) {
    final BlockCollection bc = blocksMap.getBlockCollection(block);
    return new ReplicationVector(bc == null ? 0l
        : bc.getBlockReplicationVector());
  }
  ...
  private int invalidateWorkForOneNode(DatanodeInfo dn) {
    final List<BlockTypePair> toInvalidate;
  ...
  boolean blockHasEnoughRacks(Block b) {
    ...
    DatanodeDescrTypePairs corruptNodes = corruptReplicas.getNodes(b);
    ...
    for(DatanodeStorageInfo storage : blocksMap.getStorages(b)) {
      ...
      if (!cur.isDecommissionInProgress() && !cur.isDecommissioned()) {
        if ((corruptNodes == null)
            || !corruptNodes.contains(cur, storage.getStorageType())) {
  ...
  /**
   * Check if any expected replication factor is greater than the corresponding
   * current replication factor or that the block does not have enough racks.
   */
  boolean isNeededReplication(Block b, ReplicationVector expectedRV,
      ReplicationVector currentRV) {
    return expectedRV.isAnyGreater(currentRV) || !blockHasEnoughRacks(b);
  }

  /**
   * Check if the TOTAL number of current replicas is less than expected
   * or if the block does not have enough racks.
   */
  boolean isNeededReplicationTotal(Block b, int expected, int current) {
    return expected > current || !blockHasEnoughRacks(b);
  }
   ...
   */
  public DatanodeDescrTypePairs getCorruptReplicas(Block block) {
  ...
  public String getCorruptReason(Block block, DatanodeDescriptor node,
      StorageType type) {
   return corruptReplicas.getCorruptReason(block, node, type);
 }
  ...
  private static class ReplicationWork {
    ...
    private final List<DatanodeStorageInfo> containingStorages;
    private final List<DatanodeStorageInfo> liveReplicaStorages;
    private final ReplicationVector additionalReplVector;

    private DatanodeStorageInfo targets[];
    private final int priority;
    private final boolean useSrcAsTarget;
    ...
    public ReplicationWork(Block block,
        ...
        ReplicationVector additionalReplVector,
        int priority,
        boolean useSrcAsTarget) {
      this.block = block;
      this.bc = bc;
      this.srcNode = srcNode;
      this.srcNode.incrementPendingReplicationWithoutTargets();
      this.containingStorages = containingStorages;
      this.liveReplicaStorages = liveReplicaStorages;
      this.additionalReplVector = new ReplicationVector(additionalReplVector);
      this.priority = priority;
      this.useSrcAsTarget = useSrcAsTarget;
      this.targets = null;
    }
    ...
    private void chooseTargets(BlockPlacementPolicy blockplacement,
      ...
      try {
        List<DatanodeDescriptor> favoredNodes = null;
        if (useSrcAsTarget) {
           favoredNodes = new ArrayList<DatanodeDescriptor>(1);
           favoredNodes.add(srcNode);
        }
        
        targets = blockplacement.chooseTarget(bc.getName(),
            additionalReplVector.getVectorEncoding(), srcNode,
  ...
  enum MisReplicationResult {
    ...
    OVER_REPLICATED,
    /** The block is under-replicated in some storage and over-replicated in another */
    UNDER_OVER_REPLICATED,
