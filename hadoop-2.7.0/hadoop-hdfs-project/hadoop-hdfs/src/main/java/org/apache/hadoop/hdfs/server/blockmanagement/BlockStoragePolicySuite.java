...
public class BlockStoragePolicySuite {
  ...
  public static BlockStoragePolicySuite createDefaultSuite(Configuration conf) {
    ...
    //Added by Elena
    final byte dynamic = HdfsConstants.DYNAMIC_STORAGE_POLICY_ID;
    policies[dynamic] = new DynamicBlockStoragePolicy(dynamic,
        HdfsConstants.DYNAMIC_STORAGE_POLICY_NAME,
        new StorageType[]{StorageType.MEMORY, StorageType.SSD, StorageType.DISK},
        new StorageType[]{StorageType.DISK, StorageType.SSD},
        StorageType.EMPTY_ARRAY,
        conf);
    final byte allmem = HdfsConstants.ALLMEM_STORAGE_POLICY_ID;
    policies[allmem] = new BlockStoragePolicy(allmem,
        HdfsConstants.ALLMEM_STORAGE_POLICY_NAME,
        new StorageType[]{StorageType.MEMORY},
        new StorageType[]{StorageType.DISK, StorageType.SSD},
        new StorageType[]{StorageType.DISK, StorageType.SSD});
    final byte alltiers = HdfsConstants.ALLTIERS_STORAGE_POLICY_ID;
    policies[alltiers] = new BlockStoragePolicy(alltiers,
        HdfsConstants.ALLTIERS_STORAGE_POLICY_NAME,
        new StorageType[]{StorageType.MEMORY, StorageType.SSD, StorageType.DISK},
        new StorageType[]{StorageType.DISK, StorageType.SSD},
        new StorageType[]{StorageType.DISK, StorageType.SSD});
    
    //Added from elena
    byte defaultStoragePolicy = getHdfsConstantsPolicy(
            conf.get(DFSConfigKeys.DFS_DEFAULT_STORAGE_TIER_POLICY_KEY,
            DFSConfigKeys.DFS_DEFAULT_STORAGE_TIER_POLICY_DEFAULT), policies);
    
    return new BlockStoragePolicySuite(defaultStoragePolicy, policies);
  }

  private byte defaultPolicyID;
  ...
  public void setDefaultPolicy(byte defaultPolicy) {
     if (defaultPolicy < policies.length && policies[defaultPolicy] != null)
        this.defaultPolicyID = defaultPolicy;
  }
  ...
  //Added from Elena
  public static byte getHdfsConstantsPolicy(String policyName, BlockStoragePolicy[] polcs) {
      Preconditions.checkNotNull(policyName);
    
      if (polcs != null) {
        for (BlockStoragePolicy policy : polcs) {
          if (policy != null && policy.getName().equalsIgnoreCase(policyName)) {
            return policy.getId();
          }
        }
      ...
      return HdfsConstants.HOT_STORAGE_POLICY_ID;
  }
