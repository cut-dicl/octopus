...
class BlocksMap {
  private static class StorageIterator implements Iterator<DatanodeStorageInfo> {
    ...
    public boolean hasNext() {
      return blockInfo != null && nextIdx < blockInfo.getCapacity()
              && blockInfo.getStorageInfo(nextIdx) != null;
    }
  ...
  void removeBlock(Block block) {
    ...
    for(int idx = blockInfo.numNodes()-1; idx >= 0; idx--) {
      DatanodeStorageInfo dsi = blockInfo.getStorageInfo(idx);
      dsi.removeBlock(blockInfo); // remove from the list and wipe the location
    }
  }
   ...
   * If the optionalType is defined, then remove only from the corresponding 
   * storage; otherwise from all storages of the datanode.
   * Remove the block from the block map
   * only if it does not belong to any file and data-nodes.
   */
  boolean removeNode(Block b, DatanodeDescriptor node, StorageType optionalType) {
    BlockInfoContiguous info = blocks.get(b);
    if (info == null)
      return false;

    // remove block from the data-node list and the node from the block info
    boolean removed = node.removeBlock(info, optionalType);

    if (info.getStorageInfo(0) == null     // no datanodes left
  ...
  BlockInfoContiguous replaceBlock(BlockInfoContiguous newBlock) {
    ...
    for (int i = currentBlock.numNodes() - 1; i >= 0; i--) {
      final DatanodeStorageInfo storage = currentBlock.getStorageInfo(i);
