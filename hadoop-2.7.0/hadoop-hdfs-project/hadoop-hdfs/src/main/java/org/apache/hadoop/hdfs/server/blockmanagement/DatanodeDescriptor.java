...
public class DatanodeDescriptor extends DatanodeInfo {
  ...
  /** Block and storage type pair */
  @InterfaceAudience.Private
  @InterfaceStability.Evolving
  public static class BlockTypePair {
    public final Block block;
    public final StorageType type;    

    BlockTypePair(Block block, StorageType type) {
      this.block = block;
      this.type = type;
    }

    public static Block[] toBlocks(BlockTypePair[] pairs) {
      Block[] blocks = new Block[pairs.length];
      for(int i = 0; i < blocks.length; i++) {
        blocks[i] = pairs[i].block;
      }
      return blocks;
    }

    public static StorageType[] toStorageTypes(BlockTypePair[] pairs) {
      StorageType[] storageTypes = new StorageType[pairs.length];
      for(int i = 0; i < pairs.length; i++) {
        storageTypes[i] = pairs[i].type;
      }
      return storageTypes;
    }
    ...
    public int hashCode() {
      int result = 1;
      result = 31 * result + ((block == null) ? 0 : block.hashCode());
    ...
    public boolean equals(Object obj) {
      ...
      if (getClass() != obj.getClass())
        return false;
      BlockTypePair other = (BlockTypePair) obj;
      if (block == null) {
        if (other.block != null)
          return false;
  ...
  /** DatanodeDescriptor and storage type pair */
  @InterfaceAudience.Private
  @InterfaceStability.Evolving
  public static class DatanodeDescrTypePair {
    private DatanodeDescriptor dn;
    private StorageType type;    

    DatanodeDescrTypePair(DatanodeDescriptor dn, StorageType type) {
      this.dn = dn;
      this.type = type;
    }
    ...
    public DatanodeDescrTypePair update(DatanodeDescriptor dn, StorageType type) {
      this.dn = dn;
      this.type = type;
      return this;
    }
    ...
    public int hashCode() {
      int result = 1;
      result = 31 * result + ((dn == null) ? 0 : dn.hashCode());
      result = 31 * result + ((type == null) ? 0 : type.hashCode());
      return result;
    }
    ...
    public boolean equals(Object obj) {
      ...
      if (getClass() != obj.getClass())
        return false;
      DatanodeDescrTypePair other = (DatanodeDescrTypePair) obj;
      if (dn == null) {
        if (other.dn != null)
          return false;
    ...
    public static DatanodeDescriptor[] toDatanodeDescriptors(
        DatanodeDescrTypePair[] pairs) {
      DatanodeDescriptor[] dns = new DatanodeDescriptor[pairs.length];
      for(int i = 0; i < pairs.length; i++) {
        dns[i] = pairs[i].dn;
      }
      return dns;
    ...
    public static StorageType[] toStorageTypes(DatanodeDescrTypePair[] pairs) {
      StorageType[] storageTypes = new StorageType[pairs.length];
  ...
  /** A read-only collection of DatanodeDescriptor and storage type pairs */
  @InterfaceAudience.Private
  @InterfaceStability.Evolving
  public static class DatanodeDescrTypePairs {
    private Collection<DatanodeDescrTypePair> pairs;
    private DatanodeDescrTypePair key;
    
    public DatanodeDescrTypePairs(Collection<DatanodeDescrTypePair> pairs) {
      this.pairs = pairs;
      this.key = new DatanodeDescrTypePair(null, StorageType.DEFAULT);
    }
    
    public boolean contains(DatanodeStorageInfo info) {
      return pairs.contains(key.update(info.getDatanodeDescriptor(),
          info.getStorageType()));
    }
    
    public boolean contains(DatanodeDescriptor node, StorageType type) {
      return pairs.contains(key.update(node, type));
    }
    
    public DatanodeDescrTypePair[] toArray() {
      return pairs.toArray(new DatanodeDescrTypePair[0]);
    }
  ...
  private final EnumCounters<StorageType> storageCounts = 
        new EnumCounters<StorageType>(StorageType.class);

  //Added from Elena
  /** A queue of blocks to be cached by other datanode - this datanode be the source */
  private final BlockQueue<BlockTargetPair> toCache = new BlockQueue<BlockTargetPair>();

  void addBlockToCache(Block block, DatanodeStorageInfo[] targets) {
      assert(block != null && targets != null && targets.length > 0);
      toCache.offer(new BlockTargetPair(block, targets));
  }

  public List<BlockTargetPair> getCacheCommand(int maxTransfers) {
      return toCache.poll(maxTransfers);
  }
  ...
  private final LightWeightHashSet<BlockTypePair> invalidateBlocks = new LightWeightHashSet<BlockTypePair>();

  /** Approximate numbers of reads scheduled on this datanode */
  private AtomicInteger approxReadsScheduled = new AtomicInteger(0);
  ...
  public DatanodeDescriptor(DatanodeID nodeID) {
    super(nodeID);
    updateHeartbeatState(StorageReport.EMPTY_ARRAY, 0, 0, null);
  }
  ...
  //Added from Elena
  public  DatanodeStorageInfo[] getStorageInfosType(StorageType storageType){
      synchronized (storageMap) {
          final List<DatanodeStorageInfo> storages = new ArrayList<DatanodeStorageInfo>(storageMap.size());
          for(DatanodeStorageInfo storage : storageMap.values()){
              if(storage.getStorageType().equals(storageType)){
    ...
    synchronized (storageMap) {
      ...
      while (iter.hasNext()) {
        ...
        if (storageInfo.getLastBlockReportId() != curBlockReportId) {
          ...
          iter.remove();
          storageCounts.subtract(storageInfo.getStorageType(), 1);
          if (zombies == null) {
  ...
  /**
   * Remove block from the list of blocks belonging to the datanode. Remove
   * datanode from the block.
   * If the optionalType is specified, then the block is removed only from
   * the storage with the corresponding type. Otherwise, it is removed from
   * all storages
   */
  boolean removeBlock(BlockInfoContiguous b, StorageType optionalType) {
    boolean removed = false;

    if (optionalType != null) {
      final DatanodeStorageInfo s = b.findStorageInfo(this, optionalType);
      if (s != null) { // if block exists on this datanode storage
        removed = s.removeBlock(b);
      }
    } else {
      // Remove from all storages
      for (DatanodeStorageInfo storage : getStorageInfos()) {
        if (b.findStorageInfo(storage) >= 0 && storage.removeBlock(b)) {
          removed = true;
        }
      }
    }
  ...
  public void resetBlocks() {
    ...
    synchronized (storageMap) {
        for (DatanodeStorageInfo storage : storageMap.values()) {
            storage.clearCachedBlocksLists();
        }
    }
  ...
  public void clearBlockQueues() {
    synchronized (invalidateBlocks) {
      ...
      this.toCache.clear(); //added from elena
    }
   ...
   */
  public void updateHeartbeat(StorageReport[] reports, int xceiverCount, int volFailures,
      VolumeFailureSummary volumeFailureSummary) {
    updateHeartbeatState(reports, xceiverCount,
  ...
  public void updateHeartbeatState(StorageReport[] reports, int xceiverCount, int volFailures,
    ...
    long totalCacheRemaining = 0;
    long totalCacheBpUsed = 0;
    ...
    for (StorageReport report : reports) {
      ...
      totalCacheCapacity += report.getCacheCapacity();
      totalCacheUsed += report.getCacheUsed();
      totalCacheRemaining += report.getCacheRemaining();
      totalCacheBpUsed += report.getCacheBpUsed();
    }
    ...
    setCacheCapacity(totalCacheCapacity);
    setCacheUsed(totalCacheUsed);
    setCacheRemaining(totalCacheRemaining);
    setCacheBpUsed(totalCacheBpUsed);
   ...
   */
  void addBlocksToBeInvalidated(List<BlockTypePair> blocklist) {
    assert(blocklist != null && blocklist.size() > 0);
    synchronized (invalidateBlocks) {
      for(BlockTypePair blk : blocklist) {
   ...
   */
  public BlockTypePair[] getInvalidateBlocks(int maxblocks) {
    synchronized (invalidateBlocks) {
      BlockTypePair[] deleteList = invalidateBlocks.pollToArray(
          new BlockTypePair[Math.min(invalidateBlocks.size(), maxblocks)]);
  ...
  /**
   * @return Approximate average number of blocks currently scheduled 
   * to be written to a single storage of the given storage type.
   */
  public int getBlocksScheduledPerStorage(StorageType t) {
    int numStorage = (int) storageCounts.get(t);
    if (numStorage > 0)
    return (int) Math.ceil((currApproxBlocksScheduled.get(t)
        + prevApproxBlocksScheduled.get(t)) / (double) numStorage);
    else 
       return 0;
  }
  ...
  int getApproxReadsScheduled() {
     return approxReadsScheduled.get();
  }

  void updateApproxReadsScheduled(int delta) {
     approxReadsScheduled.addAndGet(delta);
  }
  
   /**
    * @return the approximate number of readers and writers connected to this
    *         datanode
    */
  int getApproXceiverCount() {
     return approxReadsScheduled.get()
         + (int) (currApproxBlocksScheduled.sum() + prevApproxBlocksScheduled.sum());
  }
  ...
  public DatanodeStorageInfo updateStorage(DatanodeStorage s) {
    synchronized (storageMap) {
      ...
      if (storage == null) {
        ...
        storageCounts.add(storage.getStorageType(), 1);
        ...
        storageCounts.subtract(storage.getStorageType(), 1);
