...
public class DatanodeManager {
  ...
  private final BlockRetrievalPolicy retrievalPolicy;
  ...
  DatanodeManager(final BlockManager blockManager, final Namesystem namesystem,
    ...
    retrievalPolicy = BlockRetrievalPolicy.getInstance(conf, networktopology,
            host2DatanodeMap);
    LOG.info("BlockRetrievalPolicy = " + retrievalPolicy.getClass().getSimpleName());
  ...
  /** @return the retrieval policy. */
  BlockRetrievalPolicy getBlockRetrievalPolicy() {
  ...
  public void sortLocatedBlocks(final String targethost,
    ...
    Node client = host2DatanodeMap.getDatanodeByHost(targethost, false);
    if (client == null) {
    ...
    for (LocatedBlock b : locatedblocks) {
      DatanodeInfo[] di = b.getAllLocations();
      ...
      retrievalPolicy.sortBlockLocations(client, b.getAllLocations(),
               b.getAllStorageIDs(), activeLen);
  ...
  /**
   * Get datanode storage info based on datanode ID and storage ID.
  ...
  public DatanodeStorageInfo getDatanodeStorageInfo(
        DatanodeInfoWithStorage dnInfo)
            throws UnregisteredNodeException {
     DatanodeDescriptor dd = getDatanode(dnInfo.getDatanodeUuid());
     if (dd != null)
        return dd.getStorageInfo(dnInfo.getStorageID());
     return null; 
  }
  ...
  public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
    ...
    synchronized (heartbeatManager) {
      synchronized (datanodeMap) {
        ...
        BlockTypePair[] blks = nodeinfo.getInvalidateBlocks(blockInvalidateLimit);
        if (blks != null) {
        ...
        if (shouldSendCachingCommands && 
            ...
            // Check for caching cmds
            List<BlockTargetPair> onCacheList = nodeinfo.getCacheCommand(maxTransfers);
            if (onCacheList != null) {
                cmds.add(new BlockCommand(DatanodeProtocol.DNA_CACHE, blockPoolId, onCacheList));
                sendingCachingCommands = true;
            }

            // Check for uncaching cmds
            for (DatanodeStorageInfo storage : nodeinfo.getStorageInfos()) {
                DatanodeCommand pendingUncacheCommand = getCacheCommand(
                        storage.getPendingUncached(), storage,
  ...
  private DatanodeCommand getCacheCommand(CachedBlocksList list,
      DatanodeStorageInfo datanodeStorageInfo, int action, String poolId) {
    ...
    return new BlockIdCommand(action, poolId, blockIds, datanodeStorageInfo.getStorageType());
  }
  ...
  public void clearPendingCachingCommands() {
      ...
      for (DatanodeDescriptor dn : datanodeMap.values()) {
          for (DatanodeStorageInfo storage : dn.getStorageInfos()){
              storage.getPendingCached().clear();
              storage.getPendingUncached().clear();
          }
      }
  }
