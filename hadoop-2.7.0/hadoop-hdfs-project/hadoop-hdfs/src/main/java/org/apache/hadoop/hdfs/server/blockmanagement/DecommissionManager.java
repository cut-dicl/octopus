...
public class DecommissionManager {
  ...
  private boolean isSufficientlyReplicated(BlockInfoContiguous block, 
    ...
    if (!blockManager.isNeededReplicationTotal(block, numExpected, numLive)) {
    ...
    if (numExpected > numLive) {
      if (bc.isUnderConstruction() && block.equals(bc.getLastBlock())) {
        ...
        if (numLive >= blockManager.defaultTotalReplication) {
          return true;
        }
      }
    }
  ...
  private class Monitor implements Runnable {
    ...
    private void processBlocksForDecomInternal(
      ...
      while (it.hasNext()) {
        ...
        final NumberReplicationVectors num = blockManager.countNodesPerType(block);
        final ReplicationVector liveReplicasRV = num.liveReplicas();
        final ReplicationVector expectedRV = new ReplicationVector(bc.getBlockReplicationVector());

        // Schedule under-replicated blocks for replication if not already
        // pending
        if (blockManager.isNeededReplication(block, expectedRV, liveReplicasRV)) {
        ...
        if (isSufficientlyReplicated(block, bc, liveReplicasRV.getTotalReplication())) {
        ...
        if (firstReplicationLog) {
          logBlockReplicationInfo(block, bc, datanode, num.toNumberReplicas(),
        ...
        if ((liveReplicasRV.getTotalReplication() == 0) && 
            (num.decommissionedReplicas().getTotalReplication() > 0)) {
