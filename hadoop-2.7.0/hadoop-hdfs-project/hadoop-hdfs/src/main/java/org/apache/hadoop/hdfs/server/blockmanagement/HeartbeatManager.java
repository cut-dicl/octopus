...
public class HeartbeatManager implements DatanodeStatistics {
  ...
  @Override
  public synchronized long getCacheRemaining() {
     return stats.cacheRemaining;
  }

  @Override
  public synchronized boolean containsStorageTier(StorageType type) {
     return stats.tierStats.containsKey(type);
  }

  @Override
  public synchronized long getCapacityTotal(StorageType type) {
     Stats s = stats.tierStats.get(type);
     return (s != null) ? s.capacityTotal : 0L;
  }

  @Override
  public synchronized long getCapacityUsed(StorageType type) {
     Stats s = stats.tierStats.get(type);
     return (s != null) ? s.capacityUsed : 0L;
  }

  @Override
  public synchronized long getCapacityRemaining(StorageType type) {
     Stats s = stats.tierStats.get(type);
     return (s != null) ? s.capacityRemaining : 0L;
  }

  @Override
  public synchronized float getCapacityUsedPercent(StorageType type) {
     Stats s = stats.tierStats.get(type);
     if (s != null)
  ...
  @Override
  public synchronized float getCapacityRemainingPercent(StorageType type) {
     Stats s = stats.tierStats.get(type);
     if (s != null)
        return DFSUtil.getPercentRemaining(
           s.capacityRemaining, s.capacityTotal);
     else
        return 0f;
  }

  @Override
  public synchronized double getAvgWriteThroughput(StorageType type) {
     Stats s = stats.tierStats.get(type);
  ...
  @Override
  public synchronized double getAvgReadThroughput(StorageType type) {
     Stats s = stats.tierStats.get(type);
     if (s != null && s.nodesInService > 0)
  ...
  synchronized void register(final DatanodeDescriptor d) {
    if (!d.isAlive) {
      ...
      d.updateHeartbeatState(StorageReport.EMPTY_ARRAY, 0, 0, null);
    }
  }
  ...
  private static class Stats {
    ...
    private EnumMap<StorageType, HeartbeatManager.Stats> tierStats = 
          new EnumMap<StorageType, HeartbeatManager.Stats>(StorageType.class);
    ...
    private void add(final DatanodeDescriptor node) {
      ...
      if (!(node.isDecommissionInProgress() || node.isDecommissioned())) {
        ...
        cacheRemaining += node.getCacheRemaining();
      } else {
        capacityTotal += node.getDfsUsed();
        cacheCapacity += node.getCacheUsed();
      }
      
      // Add stats per storage type
      for (DatanodeStorageInfo dsi : node.getStorageInfos()) {
         Stats s = tierStats.get(dsi.getStorageType());
         if (s == null) {
            s = new Stats();
            tierStats.put(dsi.getStorageType(), s);
         }
         s.add(dsi);
      }
    }
    ...
    private void subtract(final DatanodeDescriptor node) {
      ...
      // Subtract stats per storage type
      for (DatanodeStorageInfo dsi : node.getStorageInfos()) {
         Stats s = tierStats.get(dsi.getStorageType());
         if (s != null) {
            s.subtract(dsi);
    ...
    private void add(final DatanodeStorageInfo dsi) {
       capacityUsed += dsi.getDfsUsed();
       cacheUsed += dsi.getCacheUsed();
       blockPoolUsed += dsi.getBlockPoolUsed();
       xceiverCount += dsi.getXceiverCount();
       if (!(dsi.getDatanodeDescriptor().isDecommissionInProgress() 
             || dsi.getDatanodeDescriptor().isDecommissioned())) {
         nodesInService++;
         nodesInServiceXceiverCount += dsi.getXceiverCount();
         capacityTotal += dsi.getCapacity();
         cacheCapacity += dsi.getCacheCapacity();
         capacityRemaining += dsi.getRemaining();
         cacheRemaining += dsi.getCacheRemaining();
         writeThroughputSum += dsi.getWriteThroughput();
         readThroughputSum += dsi.getReadThroughput();
       } else {
         capacityTotal += dsi.getDfsUsed();
         cacheCapacity += dsi.getCacheUsed();
       }
     }

    private void subtract(final DatanodeStorageInfo dsi) {
       capacityUsed -= dsi.getDfsUsed();
       cacheUsed -= dsi.getCacheUsed();
       blockPoolUsed -= dsi.getBlockPoolUsed();
       xceiverCount -= dsi.getXceiverCount();
