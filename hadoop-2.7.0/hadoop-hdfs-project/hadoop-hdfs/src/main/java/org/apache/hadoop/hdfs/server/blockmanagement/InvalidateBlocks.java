...
class InvalidateBlocks {
  ...
  private final Map<DatanodeInfo, LightWeightHashSet<BlockTypePair>> node2blocks =
      new TreeMap<DatanodeInfo, LightWeightHashSet<BlockTypePair>>();
  ...
  /**
   * @return true if the given storage for the given storage type has the given
   *         block listed for invalidation. Blocks are compared including their
   *         generation stamps: if a block is pending invalidation but with a
   *         different generation stamp, returns false.
   */
  synchronized boolean contains(final DatanodeInfo dn, 
      final StorageType type,
      final Block block) {
    final LightWeightHashSet<BlockTypePair> s = node2blocks.get(dn);
    if (s == null) {
      return false; // no invalidate blocks for this storage ID
    }
    BlockTypePair blockInSet = s.getElement(new BlockTypePair(block, type));
   ...
   * which will be invalidated on the specified datanode and storage type.
   */
  synchronized void add(final Block block, final DatanodeInfo datanode,
      final StorageType type, final boolean log) {
    LightWeightHashSet<BlockTypePair> set = node2blocks.get(datanode);
    if (set == null) {
      set = new LightWeightHashSet<BlockTypePair>();
      node2blocks.put(datanode, set);
    }
    
    BlockTypePair pair = new BlockTypePair(block, type);
    if (set.add(pair)) {
      numBlocks++;
      if (log) {
        NameNode.blockStateChangeLog.info("BLOCK* {}: add {} to {}",
            getClass().getSimpleName(), pair, datanode);
      }
    }
  }
  ...
  synchronized void remove(final DatanodeInfo dn) {
    final LightWeightHashSet<BlockTypePair> blocks = node2blocks.remove(dn);
    if (blocks != null) {
  ...
  synchronized void remove(final DatanodeInfo dn, final StorageType type,
      final Block block) {
    final LightWeightHashSet<BlockTypePair> v = node2blocks.get(dn);
    if (v != null && v.remove(new BlockTypePair(block, type))) {
      numBlocks--;
      if (v.isEmpty()) {
  ...
  synchronized void dump(final PrintWriter out) {
    ...
    for(Map.Entry<DatanodeInfo, LightWeightHashSet<BlockTypePair>> entry : node2blocks.entrySet()) {
      final LightWeightHashSet<BlockTypePair> blocks = entry.getValue();
      if (blocks.size() > 0) {
  ...
  synchronized List<BlockTypePair> invalidateWork(final DatanodeDescriptor dn) {
    ...
    final LightWeightHashSet<BlockTypePair> set = node2blocks.get(dn);
    if (set == null) {
      return null;
    }
    ...
    final List<BlockTypePair> toInvalidate = set.pollN(limit);
