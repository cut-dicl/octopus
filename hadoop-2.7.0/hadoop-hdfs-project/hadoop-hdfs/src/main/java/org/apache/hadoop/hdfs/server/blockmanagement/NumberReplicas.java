...
public class NumberReplicas {
  ...
  private int nonVolatileLiveReplicas;

  NumberReplicas() {
    initialize(0, 0, 0, 0, 0, 0);
  }

  NumberReplicas(int live, int decommissioned, int corrupt, int excess, int stale, int nonVolatileLive) {
    initialize(live, decommissioned, corrupt, excess, stale, nonVolatileLive);
  }

  void initialize(int live, int decommissioned, int corrupt, int excess, int stale, int nonVolatileLive) {
    liveReplicas = live;
    decommissionedReplicas = decommissioned;
    corruptReplicas = corrupt;
    excessReplicas = excess;
    replicasOnStaleNodes = stale;
    nonVolatileLiveReplicas = nonVolatileLive;
  }
   ...
   */
  public int nonVolatileLiveReplicas() {
