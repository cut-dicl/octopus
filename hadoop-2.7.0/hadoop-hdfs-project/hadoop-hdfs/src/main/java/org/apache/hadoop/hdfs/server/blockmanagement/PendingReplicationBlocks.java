...
class PendingReplicationBlocks {
  ...
  private final Map<BlockCollection, Integer> pendingFiles;
  ...
  PendingReplicationBlocks(long timeoutPeriod) {
    ...
    pendingFiles = new HashMap<BlockCollection, Integer>();
  }
   ...
   * @return if this was the last block of the file, return the file
   */
  BlockCollection decrement(Block block, DatanodeStorageInfo dn) {
    synchronized (pendingReplications) {
      PendingBlockInfo found = pendingReplications.get(block);
      if (found != null) {
   ...
   * @return if this was the last block of the file, return the file
   */
  BlockCollection remove(Block block) {
    synchronized (pendingReplications) {
      pendingReplications.remove(block);
      return decrementFile(block);
    }
  ...
  private void incrementFile(Block block) {
     if (block instanceof BlockInfoContiguous) {
        BlockCollection bc = ((BlockInfoContiguous) block).getBlockCollection();
        if (pendingFiles.containsKey(bc))
           pendingFiles.put(bc, new Integer(pendingFiles.get(bc) + 1));
        else
           pendingFiles.put(bc, new Integer(1));
     }
  }

  private BlockCollection decrementFile(Block block) {
     // Decrement file count. Return file if its last block was removed.
     if (block instanceof BlockInfoContiguous) {
        BlockCollection bc = ((BlockInfoContiguous) block).getBlockCollection();
        if (pendingFiles.containsKey(bc)) {
           int count = pendingFiles.get(bc) - 1;
           if (count > 0)
              pendingFiles.put(bc, new Integer(count));
           else {
              pendingFiles.remove(bc);
              return bc;
           }
        }
     }
  ...
  /**
   * Get the replication vector of the pending replicas
   * @param block
   * @return
   */
  ReplicationVector getReplicationVector(Block block) {
    synchronized (pendingReplications) {
      PendingBlockInfo found = pendingReplications.get(block);
      if (found != null) {
        return found.getReplicationVector();
      }
    }
    return new ReplicationVector(0l);
  }
  ...
  static class PendingBlockInfo {
    ...
    ReplicationVector getReplicationVector() {
      return DatanodeStorageInfo.createReplicationVector(targets);
    }
  }
