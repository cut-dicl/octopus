...
class UnderReplicatedBlocks implements Iterable<Block> {
  ...
  /**
   * see {@link #add(Block, int, int, int)}
   * @param block
   * @param curReplicas
   ...
   */
  synchronized boolean add(Block block,
      ReplicationVector curReplicas, 
      ReplicationVector decomissionedReplicas,
      ReplicationVector expectedReplicas) {
    return add(block, curReplicas.getTotalReplication(),
        decomissionedReplicas.getTotalReplication(),
        expectedReplicas.getTotalReplication());
  }
  ...
  /**
   * see {@link #remove(Block, int, int, int)}
   * @param block
   * @param oldReplicas
   * @param decommissionedReplicas
   * @param oldExpectedReplicas
   * @return
   */
  synchronized boolean remove(Block block, 
      ReplicationVector oldReplicas, 
      ReplicationVector decommissionedReplicas,
      ReplicationVector oldExpectedReplicas) {
    return remove(block, oldReplicas.getTotalReplication(),
        decommissionedReplicas.getTotalReplication(),
        oldExpectedReplicas.getTotalReplication());
  }
