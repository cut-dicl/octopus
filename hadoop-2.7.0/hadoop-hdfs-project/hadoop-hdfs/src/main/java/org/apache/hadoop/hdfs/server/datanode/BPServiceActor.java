...
class BPServiceActor implements Runnable {
  ...
  //Added from Elena 
  void notifyNamenodeCachedBlock(ReceivedDeletedBlockInfo bInfo,
          String storageUuid, boolean now) {
      synchronized (pendingIncrementalBRperStorage) {
        addPendingReplicationBlockInfo(
            bInfo, dn.getFSDataset().getStorage(storageUuid));
  ...
  void notifyNamenodeUncachedBlock(
          ReceivedDeletedBlockInfo bInfo, String storageUuid) {
      synchronized (pendingIncrementalBRperStorage) {
        addPendingReplicationBlockInfo(
            bInfo, dn.getFSDataset().getStorage(storageUuid));
  ...
  DatanodeCommand cacheReport() throws IOException {
    ...
    if (startTime - lastCacheReport > dnConf.cacheReportInterval) {
      ...
      Map<DatanodeStorage, List<Long>> blockIds = dn.getFSDataset().getCacheReport(bpid);
      List<CacheBlockReport> reports = new ArrayList<CacheBlockReport>(blockIds.size());
      for (Entry<DatanodeStorage, List<Long>> entry : blockIds.entrySet()) {
          reports.add(new CacheBlockReport(entry.getKey(),entry.getValue()));
      }
      
      long createTime = monotonicNow();   
      cmd = bpNamenode.cacheReport(bpRegistration, bpid, reports);
