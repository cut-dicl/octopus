...
class BlockReceiver implements Closeable {
  ...
  BlockReceiver(final ExtendedBlock block, final StorageType storageType,
    ...
    try{
      ...
      // Open local disk out
      //
      if (isDatanode) { //replication or move or caching
          if(stage == BlockConstructionStage.TRANSFER_CACHING){
              // this is a transfer destination for caching
              replicaHandler = datanode.data.createCachingBlock(null, storageType, block);
          } else{
              replicaHandler = datanode.data.createTemporary(null, storageType, block);
          }
        ...
        case PIPELINE_SETUP_APPEND:
          replicaHandler = datanode.data.append(storageType, block, newGs, minBytesRcvd);
          block.setGenerationStamp(newGs);
          datanode.notifyNamenodeReceivingBlock(
              block, replicaHandler.getReplica().getStorageUuid());
          break;
        ...
        case TRANSFER_FINALIZED:
          ...
          replicaHandler =
          datanode.data.createTemporary(null, storageType, block);
          break;
      ...
      } else if (!(out instanceof ByteBufferQueueOutputStream)) {
    ...
    try {
      ...
      if (isDatanode || isTransfer) {
        ...
        try (ReplicaHandler handler = claimReplicaHandler()) {
          ...
          } else if(stage == BlockConstructionStage.TRANSFER_CACHING){
              datanode.data.finalizeCachedBlock(block);
          } else {
  ...
  class PacketResponder implements Runnable, Closeable {   
    ...
    private void finalizeBlock(long startTime) throws IOException {
      ...
      datanode.closeBlock(
          block, DataNode.EMPTY_DEL_HINT, replicaInfo.getStorageUuid(), false);
