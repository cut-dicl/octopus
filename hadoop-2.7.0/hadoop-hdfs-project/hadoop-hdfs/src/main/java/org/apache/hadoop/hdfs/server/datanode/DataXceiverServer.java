...
class DataXceiverServer implements Runnable {
  ...
  final BlockBalanceThrottler balanceThrottler; // for block balancer
  Map<String, DataTransferThrottler> volWriteThrottlers; // for client writes
  Map<String, DataTransferThrottler> volReadThrottlers; // for client reads
  Map<String, DataTransferThrottler> volTransferThrottlers; // for transfers
  ...
  DataXceiverServer(PeerServer peerServer, Configuration conf,
    ...
    this.volReadThrottlers = new HashMap<String, DataTransferThrottler>();
    this.volWriteThrottlers = new HashMap<String, DataTransferThrottler>();
    this.volTransferThrottlers = new HashMap<String, DataTransferThrottler>();
  }
  ...
  // Add read/write throttlers based on storage type (if configured)
  synchronized void addVolumeThrottlers(FsVolumeSpi volume, Configuration conf) {
     ...
     if (bandwidth > 0) {
        volWriteThrottlers.put(volume.getStorageID(),
              new DataTransferThrottler(bandwidth));
     }
     ...
     bandwidth = conf.getLong(
           ...
           DFSConfigKeys.DFS_DATANODE_VOLUME_BANDWIDTHPERSEC_DEFAULT);
     if (bandwidth > 0) {
        volReadThrottlers.put(volume.getStorageID(),
              new DataTransferThrottler(bandwidth));
     }
     
     bandwidth = conf.getLong(
           DFSConfigKeys.DFS_DATANODE_VOLUME_TRANSF_BANDWIDTHPERSEC_PREFIX_KEY
              + volume.getStorageType().toString(),
           DFSConfigKeys.DFS_DATANODE_VOLUME_BANDWIDTHPERSEC_DEFAULT);
     if (bandwidth > 0) {
        volTransferThrottlers.put(volume.getStorageID(),
              new DataTransferThrottler(bandwidth));
     }
  }
  
  // Remove read/write throttlers if present
  synchronized void removeVolumeThrottlers(FsVolumeSpi volume) {
     volWriteThrottlers.remove(volume.getStorageID());
     volReadThrottlers.remove(volume.getStorageID());
     volTransferThrottlers.remove(volume.getStorageID());
  }
}
