...
public class FsVolumeImpl implements FsVolumeSpi {
  ...
  FsVolumeImpl(FsDatasetImpl dataset, String storageID, File currentDir,
    ...
    this.configuredCapacity = conf.getLongBytes(
          DFSConfigKeys.DFS_DATANODE_DU_CAPACITY_PREFIX_KEY + storageType.toString(), 
          DFSConfigKeys.DFS_DATANODE_DU_CAPACITY_DEFAULT);
    ...
    float cachePerc = conf.getFloat(
            ...
            conf.getFloat(
                  DFSConfigKeys.DFS_DATANODE_CACHE_CAPACITY_PERCENT_ALL_KEY,
                  DFSConfigKeys.DFS_DATANODE_CACHE_CAPACITY_PERCENT_DEFAULT));
    cachePerc = (cachePerc < 0) ? 0 : cachePerc;
    cachePerc = (cachePerc > 1) ? 1 : cachePerc;
    cacheCapacity = (long)(cachePerc * getCapacity());
  ...
  public long getCacheUsed() {
	  ...
	  synchronized(dataset) {
		  for(BlockPoolSlice s : bpSlices.values()) {
			  cacheUsed += s.getCacheUsed();
		  }
	  }
	  return cacheUsed;
  }
    
  public long getCacheBpUsed(String bpid) throws IOException{
	  return getBlockPoolSlice(bpid).getCacheUsed();
  }
  
  void decCachedUsed(String bpid, long value) {
      synchronized(dataset) {
          BlockPoolSlice bp = bpSlices.get(bpid);
          if (bp != null) {
              bp.decrCacheUsed(value);
          }
      }
  }
...
//    }
  ...
  public long getCacheAvailable() throws IOException {
	  long remaining = Math.min(cacheCapacity - getCacheUsed(), getAvailable());   
	  return (remaining > 0) ? remaining : 0;
  }

  public File addFinalizedCachedBlock(String bpid, Block b, File f)
          throws IOException{
      return getBlockPoolSlice(bpid).addCachedBlock(b, f);
  }
