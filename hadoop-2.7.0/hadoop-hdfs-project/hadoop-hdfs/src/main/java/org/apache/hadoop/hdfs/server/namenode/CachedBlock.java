...
public final class CachedBlock implements Element, 
  ...
  public CachedBlock(long blockId, long replVector, byte mark) {
    this.blockId = blockId;
    this.triplets = EMPTY_ARRAY;
    this.replVector = new ReplicationVector(replVector); 
    this.mark = mark;
  }
  ...
  public void setReplVectorAndMark(long replVector, byte mark) {
    assert replVector >= 0;
    this.replVector.setVectorEncoding(replVector);
    this.mark = mark;
  }

  //Added from Elena
  public void setReplVectorAndMark(StorageType type, short replication, byte mark) {
      assert replication >= 0;
      this.replVector.setReplication(type, replication);
      this.mark = mark;
  }
  ...
  public ReplicationVector getReplicationVector(){
     return replVector; 
  }
   ...
   */
  public List<DatanodeStorageInfo> getDatanodes(Type type) {
    List<DatanodeStorageInfo> nodes = new LinkedList<DatanodeStorageInfo>();
    for (int i = 0; i < triplets.length; i += 3) {
      CachedBlocksList list = (CachedBlocksList)triplets[i];
      if ((type == null) || (list.getType() == type)) {
        nodes.add(list.getDatanodeStorageInfo());
      }
    }
    return nodes;
  }
  ...
  public String toString() {
    return new StringBuilder().append("{").
        ...
        append("replVector=").append(getReplicationVector()).append(", ").
