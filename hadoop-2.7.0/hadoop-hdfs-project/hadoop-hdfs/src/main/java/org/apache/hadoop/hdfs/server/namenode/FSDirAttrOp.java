...
public class FSDirAttrOp {
  ...
  static boolean setReplication(
      FSDirectory fsd, BlockManager bm, String src, final long replicationVector)
      throws IOException {
    bm.verifyReplication(src, replicationVector, null);
    ...
    try {
      ...
      // Resolve all unspecified replicas in the replication vector
      long newReplVector = replicationVector;
      INode lastINode = iip.getLastINode();
      if (lastINode != null && lastINode.isFile()) {
        newReplVector = bm.resolveReplication(replicationVector,
            lastINode.getStoragePolicyID());
      }

      final long[] fileReplVectors = new long[2]; // 0: old, 1: new
      final Block[] blocks = unprotectedSetReplication(fsd, src, newReplVector,
          fileReplVectors);
      isFile = blocks != null;
      if (isFile) {
        fsd.getEditLog().logSetReplication(src, newReplVector);
        bm.setReplication(fileReplVectors[0], fileReplVectors[1], src, blocks);
      }
    } finally {
      fsd.writeUnlock();
    }
  ...
  //////////////////////////////////////////////////////////////////////////////
  //Added from elena
  public static boolean setReplication(
          FSDirectory fsd, BlockManager bm, INodeFile file, final long newReplVector)
          throws IOException {
      final boolean isFile;
      fsd.writeLock();
      try {
         final long[] fileReplVectors = new long[2]; // 0: old, 1: new
         Block[] blocks;
         INodesInPath iip = fsd.getINodesInPath4Write(file.getFullPathName(), true);
   		
         blocks = unprotectedSetReplicationInternal(fsd, iip, file, newReplVector, fileReplVectors);
         isFile = blocks != null;
   	
         if (isFile) {
           if (fsd.getEditLog() != null)
              fsd.getEditLog().logSetReplication(file.getFullPathName(), newReplVector);
           bm.setReplication(fileReplVectors[0], fileReplVectors[1], file.getFullPathName(), blocks);
         }
      }
      finally {
         fsd.writeUnlock();
  ...
  //Added from Elena
  static void setDefaultStoragePolicy(BlockManager bm, final String policyName)
          throws IOException {
      ...
      //fsd.getEditLog().logSetStoragePolicy(src, policy.getId());
      bm.setDefaultStoragePolicy(policy.getId());
  } 

  static BlockStoragePolicy getDefaultStoragePolicy(BlockManager bm)
          throws IOException {
      return bm.getDefaultStoragePolicy();
  }
  ...
  static Block[] unprotectedSetReplication(
      FSDirectory fsd, String src, long replicationVector, long[] fileReplVectors)
    ...
    return unprotectedSetReplicationInternal(fsd, iip, file, replicationVector, fileReplVectors);
  }

//////////////////////////////////////////////////////////////////////////////////////////////
//Added from elena
   static Block[] unprotectedSetReplicationInternal(FSDirectory fsd,
         INodesInPath iip, INodeFile file, long replicationVector,
         long[] fileReplVectors) throws SnapshotAccessControlException,
         UnresolvedLinkException, QuotaExceededException {
      assert fsd.hasWriteLock();
        
		final short oldBR = file.getBlockReplication();
		final long oldRV = file.getReplicationVector();
		...
		if (totalRepl > oldBR) {
			...
			fsd.updateCount(iip, 0L, dsDelta, oldRV, replicationVector, true);
		}

		file.setFileReplication(replicationVector, iip.getLatestSnapshotId());

		final short newBR = file.getBlockReplication();
		final long newRV = file.getReplicationVector();
        ...
        if (fileReplVectors != null) {
          fileReplVectors[0] = oldRV;
          fileReplVectors[1] = newRV;
        }
