...
class FSDirStatAndListingOp {
      ...
      for (int i=0; i<numOfListing && locationBudget>0; i++) {
        ...
        if (needLocation) {
            ...
            locationBudget -= (blks == null) ? 0 :
               blks.locatedBlockCount() * listing[i].getTotalReplication();
        }
      }
  ...
  static HdfsFileStatus createFileStatus(
     ...
     if (node.isFile()) {
       ...
       replicationVector = fileNode.getReplicationVector(snapshot);
  ...
  private static HdfsLocatedFileStatus createLocatedFileStatus(
    ...
    HdfsLocatedFileStatus status =
        new HdfsLocatedFileStatus(size, node.isDirectory(), replicationVector,
