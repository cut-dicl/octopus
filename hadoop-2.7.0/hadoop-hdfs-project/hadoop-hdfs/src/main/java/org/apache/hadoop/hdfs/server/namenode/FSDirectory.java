...
public class FSDirectory implements Closeable {
  ...
  private static INodeFile newINodeFile(long id, PermissionStatus permissions,
      long mtime, long atime, long replicationVector, long preferredBlockSize) {
    return newINodeFile(id, permissions, mtime, atime, replicationVector,
  ...
  private static INodeFile newINodeFile(long id, PermissionStatus permissions,
    ...
    return new INodeFile(id, null, permissions, mtime, atime,
        BlockInfoContiguous.EMPTY_ARRAY, replicationVector, preferredBlockSize,
        storagePolicyId);
  }
  ...
  INodesInPath addFile(INodesInPath existing, String localName, PermissionStatus
      permissions, long replicationVector, long preferredBlockSize,
    ...
    INodeFile newNode = newINodeFile(allocateNewInodeId(), permissions, modTime,
        modTime, replicationVector, preferredBlockSize);
  ...
  INodeFile addFileForEditLog(long id, INodesInPath existing, byte[] localName,
      ...
      List<XAttr> xAttrs, long replicationVector, long modificationTime, long atime,
    ...
    if (underConstruction) {
      newNode = newINodeFile(id, permissions, modificationTime,
          modificationTime, replicationVector, preferredBlockSize, storagePolicyId);
      newNode.toUnderConstruction(clientName, clientMachine);
    } else {
      newNode = newINodeFile(id, permissions, modificationTime, atime,
          replicationVector, preferredBlockSize, storagePolicyId);
    }
  ...
  BlockInfoContiguous addBlock(String path, INodesInPath inodesInPath,
    ...
    try {
      ...
      updateCount(inodesInPath, 0, fileINode.getPreferredBlockSize(),
          fileINode.getBlockReplicationVector(), true);
  ...
  boolean unprotectedRemoveBlock(String path, INodesInPath iip,
    ...
    updateCount(iip, 0, -fileNode.getPreferredBlockSize(),
        fileNode.getBlockReplicationVector(), true);
    return true;
  }
   ...
   */
  void updateSpaceConsumed(INodesInPath iip, long nsDelta, long ssDelta,
      long replicationVector) throws QuotaExceededException,
    ...
    try {
      ...
      updateCount(iip, nsDelta, ssDelta, replicationVector, true);
    } finally {
      writeUnlock();
    }
  }
   ...
   */
  void updateCount(INodesInPath iip, long nsDelta, long ssDelta, long replicationVector,
   ...
   */
  void updateCount(INodesInPath iip, long nsDelta, long ssDelta, long oldRepVector,
      long newRepVector, boolean checkQuota) throws QuotaExceededException {
    final INodeFile fileINode = iip.getLastINode().asFile();
    EnumCounters<StorageType> typeSpaceDeltas = getStorageTypeDeltas(
        fileINode.getStoragePolicyID(), ssDelta, oldRepVector, newRepVector);
    updateCount(iip, iip.length() - 1,
        new QuotaCounts.Builder().nameSpace(nsDelta).
            storageSpace(ssDelta * 
                (ReplicationVector.GetTotalReplication(newRepVector)
                - ReplicationVector.GetTotalReplication(oldRepVector))).
            typeSpaces(typeSpaceDeltas).build(),
        checkQuota);
  }
  ...
  public EnumCounters<StorageType> getStorageTypeDeltas(byte storagePolicyID,
      long dsDelta, long oldRepVector, long newRepVector) {
    EnumCounters<StorageType> typeSpaceDeltas =
    ...
    if (storagePolicyID != BlockStoragePolicySuite.ID_UNSPECIFIED) {
      ...
      if (oldRepVector != newRepVector) {
        List<StorageType> oldChosenStorageTypes =
            storagePolicy.chooseStorageTypes(oldRepVector);
      ...
      List<StorageType> newChosenStorageTypes =
          storagePolicy.chooseStorageTypes(newRepVector);
  ...
  private void verifyQuotaForTruncate(INodesInPath iip, INodeFile file,
    ...
    final short totalRepl = file.getBlockReplication();
    final long replVector = file.getBlockReplicationVector();
    delta.addStorageSpace(diff * totalRepl);
    final BlockStoragePolicy policy = getBlockStoragePolicySuite()
        .getPolicy(file.getStoragePolicyID());
    List<StorageType> types = policy.chooseStorageTypes(replVector);
