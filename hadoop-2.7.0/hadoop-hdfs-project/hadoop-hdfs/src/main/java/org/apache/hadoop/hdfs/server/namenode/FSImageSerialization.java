...
public class FSImageSerialization {
  ...
  static INodeFile readINodeUnderConstruction(
    ...
    long replicationVector = in.readLong();
    short totalReplication = ReplicationVector.GetTotalReplication(replicationVector);
    ...
    for (; i < numBlocks-1; i++) {
      blk.readFields(in);
      blocks[i] = new BlockInfoContiguous(blk, totalReplication);
    }
    // last block is UNDER_CONSTRUCTION
    if(numBlocks > 0) {
      blk.readFields(in);
      blocks[i] = new BlockInfoContiguousUnderConstruction(
        blk, totalReplication, BlockUCState.UNDER_CONSTRUCTION, null);
    }
    ...
    INodeFile file = new INodeFile(inodeId, name, perm, modificationTime,
        modificationTime, blocks, replicationVector, preferredBlockSize, (byte)0);
  ...
  static void writeINodeUnderConstruction(DataOutputStream out, INodeFile cons,
    ...
    out.writeLong(cons.getReplicationVector());
  ...
  public static void writeINodeFile(INodeFile file, DataOutput out,
    ...
    out.writeLong(file.getReplicationVector());
  ...
  public static void writeCacheDirectiveInfo(DataOutputStream out,
    ...
    int flags =
        ...
        ((directive.getReplVector() != null) ? 0x2 : 0) |
    ...
    if (directive.getReplVector() != null) {
      writeLong(directive.getReplVector(), out);
    }
  ...
  public static CacheDirectiveInfo readCacheDirectiveInfo(DataInput in)
    ...
    if ((flags & 0x2) != 0) {
      builder.setReplVector(readLong(in));
    }
    if ((flags & 0x4) != 0) {
  ...
  public static CacheDirectiveInfo readCacheDirectiveInfo(Stanza st)
    ...
    if (replicationString != null) {
      builder.setReplVector(Long.parseLong(replicationString));
    }
  ...
  public static void writeCacheDirectiveInfo(ContentHandler contentHandler,
    ...
    if (directive.getReplVector() != null) {
      XMLUtils.addSaxString(contentHandler, "REPLICATION",
          Long.toString(directive.getReplVector()));
    }
    if (directive.getPool() != null) {
  ...
  public static void writeCachePoolInfo(DataOutputStream out, CachePoolInfo info)
    ...
    if (hasLimit) {
        int size = limitPerType.size();
        writeInt(size, out);
        for(Entry<StorageType, Long> limit : limitPerType.entrySet()){
            writeLong(limit.getValue(), out);
            writeString(String.valueOf(limit.getKey()), out);
        }
    }
  ...
  public static CachePoolInfo readCachePoolInfo(DataInput in)
    ...
    if ((flags & 0x8) != 0) {
        ...
        for (long i = 0; i < size; i++) {
            info.setLimit(readLong(in), StorageType.valueOf(readString(in)));
        }
    }
  ...
  public static void writeCachePoolInfo(ContentHandler contentHandler,
    ...
    if (limitPerType != null) {
        for(final Entry<StorageType, Long>  entry : limitPerType.entrySet()){ 
            XMLUtils.addSaxString(contentHandler, 
                    "TYPE-"+String.valueOf(entry.getKey()), 
  ...
  public static CachePoolInfo readCachePoolInfo(Stanza st)
    ...
    for (StorageType t : StorageType.values()) {
        String name = "TYPE-"+String.valueOf(t);
        if(st.hasChildren(name))
            info.setLimit(Long.parseLong(st.getValue(name)), t);
    }
