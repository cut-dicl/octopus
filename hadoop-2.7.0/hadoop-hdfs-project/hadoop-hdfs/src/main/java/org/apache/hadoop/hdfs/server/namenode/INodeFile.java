...
public class INodeFile extends INodeWithAdditionalFields
  ...
  private ReplicationVector replVector;
  
  private BlockInfoContiguous[] blocks;

  INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime,
            long atime, BlockInfoContiguous[] blklist, long replicationVector,
            long preferredBlockSize) {
    this(id, name, permissions, mtime, atime, blklist, replicationVector,
  ...
  INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime,
    ...
    this.replVector = new ReplicationVector(replicationVector);
    this.header = HeaderFormat.toLong(preferredBlockSize,
        replVector.getTotalReplication(), storagePolicyID);
    this.blocks = blklist;
  }

  public INodeFile(INodeFile that) {
    super(that);
    this.replVector = new ReplicationVector(that.replVector);
  ...
  /**
   * Get block replication vector for the collection 
   * @return block replication value
   */
  @Override
  public long getBlockReplicationVector() {
    long max = getReplicationVector(CURRENT_STATE_ID);
    FileWithSnapshotFeature sf = this.getFileWithSnapshotFeature();
    if (sf != null) {
      long maxInSnapshot = sf.getMaxBlockRepVectorInDiffs();
      if (sf.isCurrentFileDeleted()) {
        max = maxInSnapshot;
      } else {
        // use the max replication from each type
        ReplicationVector maxRV =  new ReplicationVector(max);
        ReplicationVector currRV = new ReplicationVector(maxInSnapshot);
        for (StorageType type : StorageType.values()) {
          if (currRV.getReplication(type) > maxRV.getReplication(type)) {
            maxRV.setReplication(type, currRV.getReplication(type));
          }
        }

        max = maxRV.getVectorEncoding();
      }
    }
    return max;
  }

  /** Set the replication vector of this file. */
  public final void setFileReplication(long replicationVector) {
    replVector.setVectorEncoding(replicationVector);
    header = HeaderFormat.REPLICATION.BITS.combine(
        replVector.getTotalReplication(), header);
  }

  /** Set the replication factor of this file. */
  public final INodeFile setFileReplication(long replicationVector,
      int latestSnapshotId) throws QuotaExceededException {
    recordModification(latestSnapshotId);
    setFileReplication(replicationVector);
    return this;
  }
  ...
  public short getReplicationFactor(StorageType type) {
     return replVector.getReplication(type);
  }

  public long getReplicationVector(int snapshot) {
    if (snapshot != CURRENT_STATE_ID) {
      return getSnapshotINode(snapshot).getReplicationVector();
    }
    return replVector.getVectorEncoding();
  }
  ...
  public final QuotaCounts computeQuotaUsage(
    ...
    if (sf != null) {
      ...
      if (lastSnapshotId == Snapshot.CURRENT_STATE_ID
        ...
        replicationVector = getBlockReplicationVector();
      } else if (last < lastSnapshotId) {
        ssDeltaNoReplication = computeFileSize(true, false);
        replicationVector = getReplicationVector();
      } else {
        int sid = fileDiffList.getSnapshotById(lastSnapshotId);
        ssDeltaNoReplication = storagespaceConsumedNoReplication(sid);
        replicationVector = getReplicationVector(sid);
      }
    } else {
      ssDeltaNoReplication = storagespaceConsumedNoReplication();
      replicationVector = getBlockReplicationVector();
    ...
    counts.addStorageSpace(ssDeltaNoReplication *
        ReplicationVector.GetTotalReplication(replicationVector));

    if (blockStoragePolicyId != ID_UNSPECIFIED){
      BlockStoragePolicy bsp = bsps.getPolicy(blockStoragePolicyId);
      List<StorageType> storageTypes = bsp.chooseStorageTypes(replicationVector);
  ...
  public final ContentSummaryComputationContext computeContentSummary(
    ...
    if (getStoragePolicyID() != ID_UNSPECIFIED){
      ...
      List<StorageType> storageTypes = bsp.chooseStorageTypes(getReplicationVector());
      for (StorageType t : storageTypes) {
        if (!t.supportTypeQuota()) {
          continue;
        }
