 ...
 * downgrade among the top-k files based on the recency/frequency weights
 * 
 * @author elena.kakoulli
 */
public class ReplicationDowngradePolicyDynamicW extends
      ReplicationDowngradePolicyWeighted {
   ...
   protected INodeFile selectFileToDowngrade(StorageType type) {
      int maxFiles = Math.min((dirWeighted.size() + 1) / 2, topk);
