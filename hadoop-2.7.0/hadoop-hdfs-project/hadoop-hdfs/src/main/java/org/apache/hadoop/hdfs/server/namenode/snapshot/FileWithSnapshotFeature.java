...
public class FileWithSnapshotFeature implements INode.Feature {
  ...
  /** @return the max total replication factor in diffs */
  ...
  /** @return the replication vector with the max replication factor
   * of each storage type in diffs */
  public long getMaxBlockRepVectorInDiffs() {
    ReplicationVector maxRV = new ReplicationVector(0l);
    ReplicationVector currRV = new ReplicationVector(0l);
    for(FileDiff d : getDiffs()) {
      if (d.snapshotINode != null) {
        currRV.setVectorEncoding(d.snapshotINode.getReplicationVector());
    ...
    return maxRV.getVectorEncoding();
  }
  ...
  public QuotaCounts updateQuotaAndCollectBlocks(BlockStoragePolicySuite bsps, INodeFile file,
    ...
    if (removed.snapshotINode != null) {
      long replicationVector = removed.snapshotINode.getReplicationVector();
      short totalReplication = ReplicationVector.GetTotalReplication(replicationVector);
      long currentReplVector = file.getBlockReplicationVector();
      short currentTotalRepl = ReplicationVector.GetTotalReplication(currentReplVector);
      
      if (currentTotalRepl == 0) {
        long oldFileSizeNoRep = file.computeFileSize(true, true);
        oldStoragespace =  oldFileSizeNoRep * totalReplication;

        if (bsp != null) {
          List<StorageType> oldTypeChosen = bsp.chooseStorageTypes(replicationVector);
      ...
      } else if (totalReplication > currentTotalRepl) {
        ...
        if (bsp != null) {
          ...
          List<StorageType> newTypeChosen = bsp.chooseStorageTypes(currentReplVector);
