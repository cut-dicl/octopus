...
public class BlockCommand extends DatanodeCommand {
   ...
   */
  public BlockCommand(int action, String poolId, BlockTypePair blockPairs[]) {
    super(action);
    this.poolId = poolId;
    blocks = new Block[blockPairs.length]; 
    targetStorageTypes = new StorageType[blocks.length][];
    targets = EMPTY_TARGET_DATANODES;
    targetStorageIDs = EMPTY_TARGET_STORAGEIDS;

    for(int i = 0; i < blocks.length; i++) {
      BlockTypePair p = blockPairs[i];
      blocks[i] = p.block;
      targetStorageTypes[i] = new StorageType[] { p.type };
    }
  }
