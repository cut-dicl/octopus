...
public class CacheAdmin extends Configured implements Tool {
  ...
  private static class AddCacheDirectiveInfoCommand
    ...
    public String getShortUsage() {
      return "[" + getName() +
          ...
          "[-force] " +
          "[-replication <replicationVector>] [-ttl <time-to-live>]]\n";
    }
    ...
    public String getLongUsage() {
      ...
      listing.addRow("<replicationVector>", "The replication vector to use. " +
          "Defaults to 1 in memory.");
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      if (replicationString != null) {
        long replication = ReplicationVector.ParseReplicationVector(replicationString);
        builder.setReplVector(replication);
      }
  ...
  private static class ModifyCacheDirectiveInfoCommand
    ...
    public String getShortUsage() {
      return "[" + getName() +
          " -id <id> [-path <path>] [-force] [-replication <replicationVector>] " +
  ...
  private static class ListCacheDirectiveInfoCommand
    ...
    public String getShortUsage() {
      return "[" + getName()
          + " [-stats] [-path <path>] [-pool <pool>] [-id <id>] [-skip_rescan]]\n";
    }
    ...
    public String getLongUsage() {
      ...
      listing.addRow("-skip_rescan",
              "Skips rescanning the cache directives.");
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      //--------------------------------------- 
      boolean skip_rescan = StringUtils.popOption("-skip_rescan", args);
      ...
      if (printStats) {
       tableBuilder.addField("TYPE", Justification.LEFT).
      ...
      try {
        DistributedFileSystem dfs = AdminHelper.getDFS(conf);
        RemoteIterator<CacheDirectiveEntry> iter =
            dfs.listCacheDirectives(builder.build(), flags);
        int numEntries = 0;
        while (iter.hasNext()) {
          CacheDirectiveEntry entry = iter.next();
          ...
          row.add("" + ReplicationVector.StringifyReplVector(directive.getReplVector()));
          String expiry;
          ...
          if (printStats) {
        	    // Add one row per storage type
             EnumMap<StorageType, CacheDirectiveStats> stats = entry.getStats();
        	    for(CacheDirectiveStats s : stats.values()){
        	       LinkedList<String> newRow = new LinkedList<String>(row);
                newRow.add("" + s.getStorageType());
                newRow.add("" + s.getBytesNeeded());
                newRow.add("" + s.getBytesCached());
                newRow.add("" + s.getFilesNeeded());
                newRow.add("" + s.getFilesCached());
                tableListing.addRow(newRow.toArray(new String[newRow.size()]));
        	  }
          } else {
        ...
        System.out.print(String.format("Found %d cache directive%s%n",
  ...
  private static class AddCachePoolCommand implements AdminHelper.Command {
    ...
    public String getShortUsage() {
      return "[" + NAME + " <name> [-owner <owner>] " +
          "[-group <group>] [-mode <mode>] [-type <type> -limit <limitPerType>] " +
          "[-maxTtl <maxTtl>]\n";
    }
    ...
    public String getLongUsage() {
      ...
      listing.addRow("<type>", "The storage type with the maximum number of bytes that can be " +
          "cached by directives in this pool to follow, in aggregate. By default, " +
          "memory is set.");
      listing.addRow("<limitPerType>", "The maximum number of bytes that can be " +
              "cached by storage type that preceded by directives in this pool, in aggregate. By default, " +
              "no limit is set.");
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      do{
          typeString = StringUtils.popOptionWithArgument("-type", args);
          if (typeString != null) {
              StorageType type = StorageType.parseStorageType(typeString);
              String limitPerTypeString = StringUtils.popOptionWithArgument("-limit", args); 
              Long limitPerType = AdminHelper.parseLimitString(limitPerTypeString);
              if (limitPerType != null) {
                  info.setLimit(limitPerType, type);
              }
              else{
                  System.err.println(
                          "Error: Did not set limit value for the given type!");
                  return 1;
              }
          }
  ...
  private static class ModifyCachePoolCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow("<type>", "Storage type with the maximum number of bytes that can be cached " +
          "by this pool to follow.");
      listing.addRow("<limitPerType>", "The maximum number of bytes that can be cached by storage type that preceded " +
              "by this pool.");
      listing.addRow("<maxTtl>", "The maximum allowed time-to-live for " +
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      //Added by Elena
      String typeString = null;
      String limitPerTypeString = null;
      EnumMap<StorageType, Long> typesLimits = new EnumMap<StorageType, Long>(StorageType.class); 
      do{
          typeString = StringUtils.popOptionWithArgument("-type", args);
          if (typeString != null) {
              StorageType type = StorageType.parseStorageType(typeString);
              limitPerTypeString = StringUtils.popOptionWithArgument("-limit", args); 
              Long limitPerType = AdminHelper.parseLimitString(limitPerTypeString);
              if (limitPerType != null) {
                  typesLimits.put(type, limitPerType);
              }
              else{
                  System.err.println(
                          "Error: Did not set limit value for the given type!");
      ...
      if (!typesLimits.isEmpty()) {
         for (Map.Entry<StorageType, Long> entry : typesLimits.entrySet()) {
            info.setLimit(entry.getValue(), entry.getKey());
         }
         changed = true;
      }
      ...
      for (Entry<StorageType, Long> entry : typesLimits.entrySet()) {
          System.out.print(prefix + "type " + entry.getKey());
          prefix = " and ";
          System.out.print(prefix + "limit " + entry.getValue() + " ");
      }
  ...
  private static class ListCachePoolsCommand implements AdminHelper.Command {
    ...
    @Override
    public String getShortUsage() {
      return "[" + getName() + " [-stats] [<name>] [-skip_rescan]]\n";
    }
    ...
    public String getLongUsage() {
      ...
      listing.addRow("-skip_rescan", "Skips rescanning the cache directives.");
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      TableListing.Builder builder = new TableListing.Builder().
          ...
          addField("TYPE", Justification.LEFT).
      ...
      try {
        ...
        while (iter.hasNext()) {
          // Create one or more rows for each cache pool
          CachePoolEntry entry = iter.next();
          CachePoolInfo info = entry.getInfo();
          LinkedList<String> row = new LinkedList<String>();
          EnumMap<StorageType, LinkedList<String>> allRows = 
                new EnumMap<StorageType, LinkedList<String>>(StorageType.class);
          if (name == null || info.getPoolName().equals(name)) {
            ...
            // Create a new row for each storage type
            ...
            for (Entry<StorageType, Long> e : limitPerType.entrySet()) {
               ...
               LinkedList<String> newRow = new LinkedList<String>(row);
               newRow.add(e.getKey().toString());
               newRow.add(limitPerTypeString);
               allRows.put(e.getKey(), newRow);
            }
            
            if (printStats) {
                LinkedList<String> currRow = null;
                EnumMap<StorageType, CachePoolStats> stats = entry.getStats();
                for(Entry<StorageType, CachePoolStats> e : stats.entrySet()){
                   currRow = allRows.get(e.getKey());
                   if (currRow == null) {
                      currRow = new LinkedList<String>(row);
                      currRow.add(e.getKey().toString());
                      currRow.add("unknown");
                      allRows.put(e.getKey(), currRow);
                   }
                   
                   CachePoolStats s = e.getValue();
                   currRow.add(Long.toString(s.getBytesNeeded()));
                   currRow.add(Long.toString(s.getBytesCached()));
                   currRow.add(Long.toString(s.getBytesOverlimit()));
                   currRow.add(Long.toString(s.getFilesNeeded()));
                   currRow.add(Long.toString(s.getFilesCached()));
                }
                
                for (Entry<StorageType, LinkedList<String>> e : allRows.entrySet()) {
            ...
            for (LinkedList<String> r : allRows.values())
               listing.addRow(r.toArray(new String[r.size()]));
            ++numResults;
            if (name != null) {
              break;
            }
      ...
      System.out.print(String.format("Found %d cache pool%s.%n", numResults,
          (numResults == 1 ? "" : "s")));
      if (numResults > 0) { 
