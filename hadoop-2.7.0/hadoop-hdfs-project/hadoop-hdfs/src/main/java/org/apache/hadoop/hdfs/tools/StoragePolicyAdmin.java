...
public class StoragePolicyAdmin extends Configured implements Tool {
  ...
  //Added from Elena
  /** Command to get the default storage policy **/
  private static class GetDefaultStoragePolicyCommand implements AdminHelper.Command {
      @Override
      public String getName() {
      ...
      public int run(Configuration conf, List<String> args) throws IOException {
          final DistributedFileSystem dfs = AdminHelper.getDFS(conf);
          try {
            BlockStoragePolicy policy = dfs.getDefaultStoragePolicy();
            System.out.println("Default Storage Policy: " + policy);
          } catch (IOException e) {
            System.err.println(AdminHelper.prettifyException(e));
            return 2;
          }
...
/** Command to set the default storage policy **/
private static class SetDefaultStoragePolicyCommand implements AdminHelper.Command {
    @Override
    public String getName() {
        return "-setDefaultStoragePolicy";
    }
    ...
    public String getLongUsage() {
        ...
        listing.addRow("<policy>", "The name of the default storage policy");
        return getShortUsage() + "\n" +
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
        ...
        final String policyName = StringUtils.popFirstNonOption(args);
        if (policyName == null) {
          System.err.println("Please specify the policy name.\nUsage: " +
              getLongUsage());
          return 1;
        ...
        try {
          ...
          System.out.println("Set default storage policy " + policyName);
        } catch (Exception e) {
          System.err.println(AdminHelper.prettifyException(e));
          return 2;
        }
  ...
  private static final AdminHelper.Command[] COMMANDS = {
      ...
      new SetDefaultStoragePolicyCommand(),
      new GetDefaultStoragePolicyCommand()
  };
}