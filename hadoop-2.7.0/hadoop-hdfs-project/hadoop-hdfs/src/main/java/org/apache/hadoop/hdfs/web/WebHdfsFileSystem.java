...
public class WebHdfsFileSystem extends FileSystem
  ...
  private FileStatus makeQualified(HdfsFileStatus f, Path parent) {
    return new FileStatus(f.getLen(), f.isDir(), f.getTotalReplication(),
  ...
  @Override
  public boolean setReplication(final Path p, final long replicationVector
     ) throws IOException {
    statistics.incrementWriteOps(1);
    final HttpOpParam.Op op = PutOpParam.Op.SETREPLICATION;
    return new FsPathBooleanRunner(op, p,
        new ReplicationParam(replicationVector)
    ).run();
  }
  ...
  public long getDefaultReplication() {
    return getConf().getLong(DFSConfigKeys.DFS_REPLICATION_KEY,
