...
public class DFSTestUtil {
  ...
  public static void addHAConfiguration(Configuration conf,
    ...
    conf.setLong(DFSConfigKeys.DFS_REPLICATION_KEY, 1l);
  }
  ...
  public static void createFile(FileSystem fs, Path fileName, long fileLen, 
      long replicationVector, long seed) throws IOException {
    ...
    try {
      out = fs.create(fileName, replicationVector);
  ...
  public static void createFile(FileSystem fs, Path fileName, int bufferLen,
                                long fileLen, long blockSize, long replicationVector, long seed)
      throws IOException {
    createFile(fs, fileName, false, bufferLen, fileLen, blockSize, replicationVector,
      seed, false);
  }

  public static void createFile(FileSystem fs, Path fileName,
      boolean isLazyPersist, int bufferLen, long fileLen, long blockSize,
      long replicationVector, long seed, boolean flush) throws IOException {
        createFile(fs, fileName, isLazyPersist, bufferLen, fileLen, blockSize,
            replicationVector, seed, flush, null);
  }

  public static void createFile(FileSystem fs, Path fileName,
      boolean isLazyPersist, int bufferLen, long fileLen, long blockSize,
      long replicationVector, long seed, boolean flush,
  ...
  try {
    if (favoredNodes == null) {
        out = fs.create(
            ...
            replicationVector, blockSize, null);
    } else {
      out = ((DistributedFileSystem) fs).create(fileName,
        FsPermission.getDefault(), true, bufferLen, replicationVector, blockSize,
        null, favoredNodes);
    }
      if (fileLen > 0) {
  ...
  public static DatanodeInfo getLocalDatanodeInfo(String ipAddr,
    ...
    return new DatanodeInfo(ipAddr, hostname, "",
        ...
        DFSConfigKeys.DFS_DATANODE_HTTPS_DEFAULT_PORT,
        DFSConfigKeys.DFS_DATANODE_IPC_DEFAULT_PORT,
        1l, 2l, 3l, 4l, 0l, 0l, 0l, 0l, 0l, 5, 6, "local", adminState);
  }
  ...
  public static void runOperations(MiniDFSCluster cluster,
    ...
    filesystem.modifyCachePool(new CachePoolInfo("pool1").setLimit(99l, StorageType.MEMORY));
  ...
  public static long verifyExpectedCacheUsage(final long expectedCacheUsed,
    ...
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      ...
      @Override
      public Boolean get() {
        long curCacheUsed = 0; //need here initialization and try-catch for exception (getCacheUsed() changed) - below wrong?!
        try {
            curCacheUsed = fsd.getCacheUsed();
        } catch (IOException e) {
