...
public class TestFileCreation {
  ...
  public void testServerDefaults() throws IOException {
    ...
    int numNodes = ReplicationVector
        .GetTotalReplication(DFS_REPLICATION_DEFAULT) + 1;
    ...
    conf.setLong(DFS_REPLICATION_KEY, numNodes);
   ...
   cluster.waitActive();
    ...
    try {
      ...
      assertEquals(numNodes, serverDefaults.getTotalReplication());
  ...
  static FSDataOutputStream createNonRecursive(FileSystem fs, Path name,
      long replVector, EnumSet<CreateFlag> flag) throws IOException {
    System.out.println("createNonRecursive: Created " + name
        + " with replcation: "
        + ReplicationVector.StringifyReplVector(replVector));
    FSDataOutputStream stm = ((DistributedFileSystem) fs).createNonRecursive(
        name, FsPermission.getDefault(), flag, fs.getConf().getInt(
            CommonConfigurationKeys.IO_FILE_BUFFER_SIZE_KEY, 4096), replVector,  blockSize, null);
    return stm;
  }
