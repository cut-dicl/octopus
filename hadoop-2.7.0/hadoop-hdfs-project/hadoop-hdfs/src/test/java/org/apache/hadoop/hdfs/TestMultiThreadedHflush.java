...
public class TestMultiThreadedHflush {
   ...
   */ 
  private FSDataOutputStream createFile(FileSystem fileSys, Path name, long replVector)
    throws IOException {
    FSDataOutputStream stm = fileSys.create(name, true, fileSys.getConf()
        .getInt(CommonConfigurationKeys.IO_FILE_BUFFER_SIZE_KEY, 4096),
        replVector, blockSize);
    return stm;
  }
  ...
  public void doMultithreadedWrites(
      ...
      long replicationVector) throws Exception {
    initBuffer(bufferSize);

    // create a new file.
    FileSystem fs = p.getFileSystem(conf);
    FSDataOutputStream stm = createFile(fs, p, replicationVector);
  ...
  private static class CLIBenchmark extends Configured implements Tool {
    public int run(String args[]) throws Exception {
      ...
      long replicationVector = conf.getLong(DFSConfigKeys.DFS_REPLICATION_KEY,
