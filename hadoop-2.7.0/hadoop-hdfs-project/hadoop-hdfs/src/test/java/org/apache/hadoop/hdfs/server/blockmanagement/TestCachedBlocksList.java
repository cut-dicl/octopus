...
public class TestCachedBlocksList {
  ...
  public void testSingleList() {
    ...
    CachedBlock[] blocks = new CachedBlock[] {
          new CachedBlock(0L, (short)1, (byte)1),
          new CachedBlock(1L, (short)1, (byte)1),
          new CachedBlock(2L, (short)1, (byte)1),
      };
    
    for(DatanodeStorageInfo dn : datanode.getStorageInfos()){
  ...
  public void testMultipleLists() {
      DatanodeStorageInfo[] datanodes = new DatanodeStorageInfo[] {
      new DatanodeStorageInfo(
        new DatanodeDescriptor(new DatanodeID("127.0.0.1", "localhost", "abcd",
          5000, 5001, 5002, 5003)), 
        new DatanodeStorage("1234", State.NORMAL, StorageType.MEMORY)),
      new DatanodeStorageInfo(
        new DatanodeDescriptor(new DatanodeID("127.0.1.1", "localhost", "efgh",
          6000, 6001, 6002, 6003)),
        new DatanodeStorage("5678", State.NORMAL, StorageType.MEMORY)),
    };
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
      blocks[i] = new CachedBlock(i, (short)i, (byte)1);
    }
