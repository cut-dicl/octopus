...
public class TestFsDatasetCache {
  ...
  private static DatanodeCommand getResponse(HdfsBlockLocation[] locs,
    ...
    return new BlockIdCommand(action, bpid, blocks, StorageType.MEMORY); //wrong?!
  }
  ...
  public void testUncacheQuiesces() throws Exception {
    ...
    dfs.addCacheDirective(new CacheDirectiveInfo.Builder()
        .setPool("pool").setPath(fileName).setReplVector((long)3).build());
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      @Override
      public Boolean get() {
  ...
  public void testReCacheAfterUncache() throws Exception {
    ...
    final long bigCacheDirectiveId = 
        dfs.addCacheDirective(new CacheDirectiveInfo.Builder()
        .setPool("pool").setPath(BIG_FILE).setReplVector((long)1).build());
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      @Override
      public Boolean get() {
        MetricsRecordBuilder dnMetrics = getMetrics(dn.getMetrics().name());
    ...
    final long shortCacheDirectiveId =
      dfs.addCacheDirective(new CacheDirectiveInfo.Builder()
        .setPool("pool").setPath(SMALL_FILE).setReplVector((long)1).build());
    Thread.sleep(10000);
    MetricsRecordBuilder dnMetrics = getMetrics(dn.getMetrics().name());
    ...
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      ...
      public Boolean get() {
        ...
        try {
          ...
          if (entry.getStats().get(StorageType.MEMORY).getFilesCached() != 1) { //Wrong!
