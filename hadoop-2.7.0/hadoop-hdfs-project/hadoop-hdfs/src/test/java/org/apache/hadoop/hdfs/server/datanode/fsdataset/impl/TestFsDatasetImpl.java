...
public class TestFsDatasetImpl {
  ...
  public void testDeletingBlocks() throws IOException {
    ...
    try {
      ...
      StorageType[] types = new StorageType[blockList.size()];
      Arrays.fill(types, StorageType.DEFAULT);
      ds.invalidate(BLOCKPOOL, blockList.toArray(new Block[0]), types);
      try {
        Thread.sleep(1000);
      ...
      info.getMetaFile().createNewFile();
      blockList.add(info);
      types = new StorageType[blockList.size()];
      Arrays.fill(types, StorageType.DEFAULT);
      ds.invalidate(BLOCKPOOL, blockList.toArray(new Block[0]), types);
      try {
        Thread.sleep(1000);
