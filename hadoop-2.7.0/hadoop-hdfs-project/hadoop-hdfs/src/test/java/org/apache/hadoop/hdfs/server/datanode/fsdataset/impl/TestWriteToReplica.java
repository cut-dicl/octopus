...
public class TestWriteToReplica {
  ...
  private void testAppend(String bpid, FsDatasetImpl dataSet, ExtendedBlock[] blocks) throws IOException {
    ...
    StorageType type = v.getStorageType();
    try {
      v.decDfsUsed(bpid, -available);
      blocks[FINALIZED].setNumBytes(expectedLen+100);
      dataSet.append(type, blocks[FINALIZED], newGS, expectedLen);
    ...
    dataSet.append(type, blocks[FINALIZED], newGS, 
        blocks[FINALIZED].getNumBytes());  // successful
    blocks[FINALIZED].setGenerationStamp(newGS);
        
    try {
      dataSet.append(type, blocks[TEMPORARY], blocks[TEMPORARY].getGenerationStamp()+1, 
    ...
    try {
      dataSet.append(type, blocks[RBW], blocks[RBW].getGenerationStamp()+1,
    ...
    try {
      dataSet.append(type, blocks[RWR], blocks[RWR].getGenerationStamp()+1,
          blocks[RBW].getNumBytes());
    ...
    try {
      dataSet.append(type, blocks[RUR], blocks[RUR].getGenerationStamp()+1,
    ...
    try {
      dataSet.append(type, blocks[NON_EXISTENT], 
  ...
  private void testWriteToTemporary(FsDatasetImpl dataSet, ExtendedBlock[] blocks) throws IOException {
    try {
      dataSet.createTemporary(null, StorageType.DEFAULT, blocks[FINALIZED]);
    ...
    try {
      dataSet.createTemporary(null, StorageType.DEFAULT, blocks[TEMPORARY]);
      Assert.fail("Should not have created a replica that had created as" +
      		"temporary " + blocks[TEMPORARY]);
    } catch (ReplicaAlreadyExistsException e) {
    }
    ...
    try {
      dataSet.createTemporary(null, StorageType.DEFAULT, blocks[RBW]);
      Assert.fail("Should not have created a replica that had created as RBW " +
          blocks[RBW]);
    } catch (ReplicaAlreadyExistsException e) {
    }
    ...
    try {
      dataSet.createTemporary(null, StorageType.DEFAULT, blocks[RWR]);
      Assert.fail("Should not have created a replica that was waiting to be " +
      		"recovered " + blocks[RWR]);
    } catch (ReplicaAlreadyExistsException e) {
    }
    ...
    try {
      dataSet.createTemporary(null, StorageType.DEFAULT, blocks[RUR]);
      Assert.fail("Should not have created a replica that was under recovery " +
          blocks[RUR]);
    } catch (ReplicaAlreadyExistsException e) {
    }
    
    dataSet.createTemporary(null, StorageType.DEFAULT, blocks[NON_EXISTENT]);
    ...
    try {
      ReplicaInPipelineInterface replicaInfo =
          dataSet.createTemporary(null, StorageType.DEFAULT, blocks[NON_EXISTENT]).getReplica();
