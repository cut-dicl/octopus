...
public class TestCacheDirectives {
  ...
  public void testBasicPoolOperations() throws Exception {
    ...
    CachePoolInfo info = new CachePoolInfo(poolName).
        ...
        setMode(new FsPermission((short)0755)).setLimit(150l, StorageType.MEMORY);
    ...
    info.setOwnerName("jane").setGroupName("janegroup")
        .setMode(new FsPermission((short)0700)).setLimit(314l, StorageType.MEMORY);
  ...
  public void testCreateAndModifyPools() throws Exception {
    ...
    dfs.addCachePool(new CachePoolInfo(poolName).
        ...
        setMode(mode).setLimit(limit, StorageType.MEMORY));
    ...
    assertEquals(limit, info.getLimits());

    dfs.removeCachePool(poolName);
    iter = dfs.listCachePools();
    assertFalse("expected no cache pools after deleting pool", iter.hasNext());

    proto.listCachePools(null, EnumSet.noneOf(CacheFlag.class));
  ...
  public void testAddRemoveDirectives() throws Exception {
    ...
    dfs.modifyCacheDirective(new CacheDirectiveInfo.Builder().
        setId(relativeId).
        setReplVector((long)555).
        build());
    iter = dfs.listCacheDirectives(null);
    assertTrue(iter.hasNext());
    CacheDirectiveInfo modified = iter.next().getInfo();
    assertEquals(relativeId, modified.getId().longValue());
    assertEquals((short)555, ReplicationVector.GetTotalReplication(modified.getReplVector()));
    ...
    dfs.modifyCacheDirective(new CacheDirectiveInfo.Builder(
        directive).setId(id).setReplVector((long)2).build());
  ...
  public void testCacheManagerRestart() throws Exception {
    ...
    try {
      ...
      dfs.addCachePool(new CachePoolInfo(pool)
          ...
          .setMode(mode)
          .setLimit(limit, StorageType.MEMORY));
  ...
  private static void waitForCacheDirectiveStats(final DistributedFileSystem dfs,
    ...
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      ...
      public Boolean get() {
        ...
        if ((targetBytesNeeded == stats.get(StorageType.MEMORY).getBytesNeeded()) &&
            (targetBytesCached == stats.get(StorageType.MEMORY).getBytesCached()) &&
            (targetFilesNeeded == stats.get(StorageType.MEMORY).getFilesNeeded()) &&
            (targetFilesCached == stats.get(StorageType.MEMORY).getFilesCached())) {
          return true;
        } else {
          LOG.info(infoString + ": " +
              "filesNeeded: " +
              stats.get(StorageType.MEMORY).getFilesNeeded() + "/" + targetFilesNeeded +
              ", filesCached: " + 
              stats.get(StorageType.MEMORY).getFilesCached() + "/" + targetFilesCached +
              ", bytesNeeded: " +
              stats.get(StorageType.MEMORY).getBytesNeeded() + "/" + targetBytesNeeded +
              ", bytesCached: " + 
              stats.get(StorageType.MEMORY).getBytesCached() + "/" + targetBytesCached);
          return false;
        }
      }
    }, 500, 60000);
  ...
  public void testWaitForCachedReplicas() throws Exception {
    ...
    //Changed from Elena
    Map<DatanodeStorage, List<Long>> blockIds = dn0.getFSDataset().getCacheReport(bpid); 
    List<CacheBlockReport> reports = new ArrayList<CacheBlockReport>();

    for (Entry<DatanodeStorage, List<Long>> entry : blockIds.entrySet()) {
        reports.add(new CacheBlockReport(entry.getKey(), bogusBlockIds));
        break;
    }
       
    nnRpc.cacheReport(dn0.getDNRegistrationForBP(bpid), bpid, reports);
    ...
    for (DatanodeInfo dn : live) {
        ...
        //you must do this for each datanodestorageinfo?! - YOU MUST CHANGE THE BELOW!!!
    ...
    RemoteIterator<CacheDirectiveEntry> entries =
      new CacheDirectiveIterator(nnRpc, null, Sampler.NEVER, EnumSet.noneOf(CacheFlag.class));
    for (int i=0; i<numFiles; i++) {
  ...
  public void testListCachePoolPermissions() throws Exception {
    ...
    assertNull("Unexpected limit", info.getLimits());
    ...
    assertEquals("Mismatched limit", limit, info.getLimits());
  }
  ...
  public void testLimit() throws Exception {
    try {
      dfs.addCachePool(new CachePoolInfo("poolofnegativity").setLimit(-99l, StorageType.MEMORY));
    ...
    final CachePoolInfo poolInfo = new CachePoolInfo(destiny)
        .setLimit(2*BLOCK_SIZE-1, StorageType.MEMORY);
    dfs.addCachePool(poolInfo);
    ...
    poolInfo.setLimit(2*BLOCK_SIZE, StorageType.MEMORY);
    ...
    poolInfo.setLimit(BLOCK_SIZE, StorageType.MEMORY);
    dfs.modifyCachePool(poolInfo);
    waitForCachePoolStats(dfs,
        2*BLOCK_SIZE, 0,
        1, 0,
    ...
    EnumMap<StorageType,CachePoolStats> stats = it.next().getStats();
    assertEquals("Overlimit bytes should be difference of needed and limit",
        BLOCK_SIZE, stats.get(StorageType.MEMORY).getBytesOverlimit());
    // Moving a directive to a pool without enough limit should fail
    CachePoolInfo inadequate =
        new CachePoolInfo("poolofinadequacy").setLimit(BLOCK_SIZE, StorageType.MEMORY);
  ...
  private void checkPendingCachedEmpty(MiniDFSCluster cluster)
    ...
    try {
      ...
      for (DataNode dn : cluster.getDataNodes()) {
        ...
        for(StorageReport storage : descriptor.getStorageReports()){
            Assert.assertTrue("Pending cached list of " + descriptor +
                    " is not empty, "
                    + Arrays.toString(descriptor.getStorageInfo(storage.getStorage().getStorageID()).getPendingCached().toArray()), 
                    descriptor.getStorageInfo(storage.getStorage().getStorageID()).getPendingCached().isEmpty());
        }
      } 
    } finally {
  ...
  public void testExceedsCapacity() throws Exception {
    ...
    dfs.addCacheDirective(new CacheDirectiveInfo.Builder().setPool("pool")
        .setPath(fileName).setReplVector((long) 1).build());
