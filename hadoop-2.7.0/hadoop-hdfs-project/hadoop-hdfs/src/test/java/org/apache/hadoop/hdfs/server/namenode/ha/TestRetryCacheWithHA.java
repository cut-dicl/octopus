...
public class TestRetryCacheWithHA {
  ...
  class ModifyCacheDirectiveInfoOp extends AtMostOnceOp {
    ...
    void invoke() throws Exception {
      client.modifyCacheDirective(
          new CacheDirectiveInfo.Builder().
              setId(id).
              setReplVector((long)newReplication).
    ...
    boolean checkNamenodeBeforeReturn() throws Exception {
      for (int i = 0; i < CHECKTIMES; i++) {
        ...
        while (iter.hasNext()) {
          ...
          if ((result.getId() == id) &&
              (ReplicationVector.GetTotalReplication(result.getReplVector()) == newReplication)) { //wrong?!
            return true;
          }
        }
        Thread.sleep(1000);
  ...
  class ModifyCachePoolOp extends AtMostOnceOp {
    ...
    @Override
    void prepare() throws Exception {
      client.addCachePool(new CachePoolInfo(pool).setLimit(10l, StorageType.MEMORY));
    }

    @Override
    void invoke() throws Exception {
      client.modifyCachePool(new CachePoolInfo(pool).setLimit(99l, StorageType.MEMORY));
    }
    ...
    boolean checkNamenodeBeforeReturn() throws Exception {
      for (int i = 0; i < CHECKTIMES; i++) {
        RemoteIterator<CachePoolEntry> iter = dfs.listCachePools();
        if (iter.hasNext() && iter.next().getInfo().getLimit(StorageType.MEMORY) == 99) { //Wrong!
          return true;
        }
        Thread.sleep(1000);
      }
