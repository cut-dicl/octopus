...
public class JobImpl implements org.apache.hadoop.mapreduce.v2.app.job.Job, 
  ...
  private Set<Path> jobFilePaths = new HashSet<>();
  ...
  public void handle(JobEvent event) {
    ...
    if (MRPrefetcher.isPrefetchingEnabled(conf)
          && JobStateInternal.getInactiveStates().contains(getInternalState())) {
         MRPrefetcher.clearPrefetchedFiles(conf);
    }
  }
  ...
  public static class InitTransition 
    ...
    public JobStateInternal transition(JobImpl job, JobEvent event) {
      ...
      try {
        ...
        if (MRPrefetcher.isPrefetchingEnabled(job.conf)) {
           MRPrefetcher.waitForPrefetching(job.conf);
        }
    ...
    private void createMapTasks(JobImpl job, long inputLength,
      ...
      for (int i=0; i < job.numMapTasks; ++i) {
         if (splits[i].getFilePath() != null)
            job.jobFilePaths.add(splits[i].getFilePath());
        TaskImpl task =
