...
public abstract class TaskAttemptImpl implements
  ...
  protected List<SplitLocationDetails> splitLocs;
  protected Path filePath;
  protected JobSplit.TaskSplitMetaInfo taskSplitMetaInfo;
  ...
  public TaskAttemptImpl(TaskId taskId, int i,
    ...
    resolveSplitLocations(dataLocalHosts, storageTypes, 
          splitMetaInfo.getIsCached());

    this.dataLocalHosts = new HashSet<String>();
    this.dataLocalRacks = new HashSet<String>(); 
    for (SplitLocationDetails splitLoc : splitLocs) {
       this.dataLocalHosts.add(splitLoc.getHost());
       this.dataLocalRacks.add(splitLoc.getRack());
    }

    this.filePath = filePath;
    this.taskSplitMetaInfo = splitMetaInfo;
    ...
    public void transition(TaskAttemptImpl taskAttempt, 
      ...
      if (rescheduled) {
        ...
        taskAttempt.eventHandler.handle(new ContainerRequestEvent(
            ...
            taskAttempt.splitLocs.toArray(
                  new SplitLocationDetails[taskAttempt.splitLocs.size()]),
            taskAttempt.filePath,
            taskAttempt.taskSplitMetaInfo));
      }
    }
  }

   private void resolveSplitLocations(String[] src, StorageType[] types,
         boolean[] isCached) {

     this.splitLocs = new ArrayList<>();
     ...
     if (src != null) {
       for (int i = 0; i < src.length; i++) {
         ...
         this.splitLocs.add(new SplitLocationDetails(rack, host, type));
         
         // Add one more location for the cached replica
         if (isCached != null && isCached.length == src.length
               && isCached[i] == true && type != StorageType.MEMORY)
            this.splitLocs.add(new SplitLocationDetails(rack, host, StorageType.MEMORY));
         }
     }
  }
