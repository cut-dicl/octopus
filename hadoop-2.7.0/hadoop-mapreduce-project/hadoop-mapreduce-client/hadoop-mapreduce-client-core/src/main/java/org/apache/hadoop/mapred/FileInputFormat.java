...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
   ...
   */
  protected FileSplit makeSplit(Path file, long start, long length, 
                                String[] hosts, StorageType[] types,
                                String[] inMemoryHosts) {
    return new FileSplit(file, start, length, hosts, types, inMemoryHosts);
  }
      ...
      if (length != 0) {
        ...
        if (isSplitable(fs, path)) {
          ...
          while (((double) bytesRemaining)/splitSize > SPLIT_SLOP) {
             SplitHostsInfo splitHosts = getSplitHostsAndCachedHosts(blkLocations,
                length-bytesRemaining, splitSize, clusterMap);
            splits.add(makeSplit(path, length-bytesRemaining, splitSize,
                splitHosts.splitHosts, splitHosts.types, splitHosts.cachedHosts));
            bytesRemaining -= splitSize;
          }

          if (bytesRemaining != 0) {
             SplitHostsInfo splitHosts = getSplitHostsAndCachedHosts(blkLocations, length
                - bytesRemaining, bytesRemaining, clusterMap);
            splits.add(makeSplit(path, length - bytesRemaining, bytesRemaining,
                  splitHosts.splitHosts, splitHosts.types, splitHosts.cachedHosts));
          }
        } else {
           SplitHostsInfo splitHosts = getSplitHostsAndCachedHosts(blkLocations,0,length,clusterMap);
   ...
   * @throws IOException
   */
  private SplitHostsInfo getSplitHostsAndCachedHosts(BlockLocation[] blkLocations, 
  ...
  throws IOException {
    ...
    if (bytesInThisBlock >= splitSize) {
      return new SplitHostsInfo(blkLocations[startIndex].getHosts(),
    ...
    StorageType [] allTypes = new StorageType[0];
    ...
    for (index = startIndex; index <= endIndex; index++) {
      ...
      allTypes = blkLocations[index].getStorageTypes();
      ...
      for (int i = 0; i < allTopos.length; ++i) {
        
        String topo = allTopos[i];
        StorageType type = StorageType.DEFAULT;
        
        if (allTypes != null && i < allTypes.length)
           type = allTypes[i];
        ...
        nodeInfo.addValue(index, bytesInThisBlock, type);
        parentNodeInfo.addValue(index, bytesInThisBlock, type);
  ...
  private SplitHostsInfo identifyHosts(int replicationFactor, 
                                 Map<Node,NodeInfo> racksMap) {
    
    SplitHostsInfo retVal = new SplitHostsInfo(replicationFactor);
    ...
    for (NodeInfo ni: rackList) {
      ...
      for (NodeInfo host: hostList) {
        ...
        retVal.splitHosts[index] = host.node.getName().split(":")[0];
        retVal.types[index] = host.getStorageType();
        ++index;
  ...
  private static class NodeInfo {
    ...
    void addValue(int blockIndex, long value, StorageType type) {
      if (blockIds.add(blockIndex) == true) {
        this.value += value;
      }
      
      if (value > maxValue) {
         // Keep track fo storage type with highest contribution
         maxValue = value;
         storageType = type;
      }
    }
  ...
  private static class SplitHostsInfo {
     String[] splitHosts;
     StorageType[] types;
     String[] cachedHosts;
     
     SplitHostsInfo(String[] splitHosts, StorageType[] types, String[] cachedHosts) {
        this.splitHosts = splitHosts;
        this.types = types;
        this.cachedHosts = cachedHosts;       
     }
     
     SplitHostsInfo(int replication) {
        this.splitHosts = new String[replication];
        this.types = new StorageType[replication];
        this.cachedHosts = new String[0];
     }
  }
}
