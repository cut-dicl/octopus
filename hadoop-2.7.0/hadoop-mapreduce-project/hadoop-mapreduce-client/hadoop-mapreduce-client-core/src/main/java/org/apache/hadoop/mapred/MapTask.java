...
public class MapTask extends Task {
  ...
  class TrackedRecordReader<K, V> 
    ...
    RecordReader<K,V> getRawRecordReader() {
       return rawIn;
    }
  }
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  // added from elena for read (input) block informations
   private <T> void recordInputSplitLocation(FSDataInputStream input,
         JobConf job, T inputSplit, TaskReporter reporter) {

 	  Path path = null;
	  long offset = 0;
	  long length = 0;
	  Object localInputSplit = inputSplit;
	  if (localInputSplit instanceof org.apache.hadoop.mapred.lib.TaggedInputSplit) {
        localInputSplit = ((org.apache.hadoop.mapred.lib.TaggedInputSplit) localInputSplit)
               .getInputSplit();
 	  }
	  if (localInputSplit instanceof org.apache.hadoop.mapreduce.lib.input.FileSplit) {
		  org.apache.hadoop.mapreduce.lib.input.FileSplit fileSplit = 
				  (org.apache.hadoop.mapreduce.lib.input.FileSplit) localInputSplit;
		  path = fileSplit.getPath();
		  offset = fileSplit.getStart();
		  length = fileSplit.getLength();
	  } else if (localInputSplit instanceof org.apache.hadoop.mapred.FileSplit) {
		  org.apache.hadoop.mapred.FileSplit fileSplit = 
				  (org.apache.hadoop.mapred.FileSplit) localInputSplit;
		  path = fileSplit.getPath();
		  offset = fileSplit.getStart();
		  length = fileSplit.getLength();
	  } else {
        LOG.info("Unable to record input split locations. Input split class="
               + localInputSplit.getClass().toString() + " toString="
               + localInputSplit.toString());
 		  return;
	  }

	  try {
         List<LocatedBlock> bl = null;
         if (input != null) {
            InputStream in = input.getWrappedStream();
            if (in != null && in instanceof DFSInputStream) {
               // Get block locations from input stream - no call to HDFS
               DFSInputStream dfsIn = (DFSInputStream) in;
               bl = dfsIn.getBlockRange(offset, length);
            }
         }
            
         if (bl == null && path != null) {
            FileSystem fs = path.getFileSystem(job);
            if (fs instanceof DistributedFileSystem) {
               // Get block locations from HDFS
               LOG.info("Getting block locations from HDFS");
               bl = ((DistributedFileSystem) fs).getClient()
                     .getLocatedBlocks(path.toUri().getPath(), offset, length)
                     .getLocatedBlocks();
            }
         }
         
         if (bl == null) {
            LOG.info("Unable to record input split locations: "
                  + " no block locations found");
            return;
         }
	     
		  String Reader = DNS.getDefaultHost("default");

		  for (LocatedBlock b : bl) {
			  String locationToRead = b.getFirstLocation().getHostName();
			  StorageType type = b.getFirstLocation().getStorageType();
           if (b.getCachedLocations() != null
                  && b.getCachedLocations().length > 0
                  && b.getCachedLocations()[0].getHostName()
                        .equals(locationToRead))
              type = StorageType.MEMORY; // special case for caching
           
           //------------------------------------------------------------------
           //Added for Octopus Prefetching by Elena
           //------------------------------------------------------------------
           // if it is a local access and if there is a pending cached location 
           // and if the location to read matches the pending cache location, 
           // then it is a local partial access
           boolean local_partial = false;
           if(b.getPendingCachedLocations().length > 0 && locationToRead.equals(Reader)) {
        	   DatanodeInfoWithStorage[] pendingCachedLocations = b.getPendingCachedLocations();
        	   for (int i=0; i<pendingCachedLocations.length; i++) {
        		   if(pendingCachedLocations[i].getHostName().equals(Reader)) {
        		      LOG.info("Location of pending cache for blk_" + b.getBlock().getBlockId() 
        		            + " is " + pendingCachedLocations[i].getHostName());
        			   local_partial = true;
        			   break;
        		   }
        	   }
 			  ...
 			  switch (type) {
			  case MEMORY:
				  reporter.incrCounter(JobCounter.TOTAL_MEMORY_ACCESSES, 1);
				  reporter.incrCounter(JobCounter.TOTAL_MEMORY_BYTES, b.getBlockSize());
				  if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_MEMORY_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_MEMORY_ACCESSES, 1);
				  break;
			  case RAM_DISK:
				  reporter.incrCounter(JobCounter.TOTAL_RAM_DISK_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_RAM_DISK_BYTES, b.getBlockSize());
				  if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_RAM_DISK_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_RAM_DISK_ACCESSES, 1);
				  break;
			  case SSD_H:
				  reporter.incrCounter(JobCounter.TOTAL_SSD_H_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_SSD_H_BYTES, b.getBlockSize());
				  if(local_partial)
					  reporter.incrCounter(JobCounter.LOCAL_PARTIAL_SSD_H_ACCESSES, 1);
				  else if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_SSD_H_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_SSD_H_ACCESSES, 1);
				  break;
			  case SSD:
				  reporter.incrCounter(JobCounter.TOTAL_SSD_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_SSD_BYTES, b.getBlockSize());
				  if(local_partial)
					  reporter.incrCounter(JobCounter.LOCAL_PARTIAL_SSD_ACCESSES, 1);
				  else if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_SSD_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_SSD_ACCESSES, 1);
				  break;
			  case DISK_H:
				  reporter.incrCounter(JobCounter.TOTAL_DISK_H_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_DISK_H_BYTES, b.getBlockSize());
				  if(local_partial)
					  reporter.incrCounter(JobCounter.LOCAL_PARTIAL_DISK_H_ACCESSES, 1);
				  else if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_DISK_H_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_DISK_H_ACCESSES, 1);
				  break;
			  case DISK:
				  reporter.incrCounter(JobCounter.TOTAL_DISK_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_DISK_BYTES, b.getBlockSize());
				  if(local_partial) 
					  reporter.incrCounter(JobCounter.LOCAL_PARTIAL_DISK_ACCESSES, 1);
				  else if (locationToRead.equals(Reader)) 
					  reporter.incrCounter(JobCounter.LOCAL_DISK_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_DISK_ACCESSES, 1);
				  break;
			  case ARCHIVE:
				  reporter.incrCounter(JobCounter.TOTAL_ARCHIVE_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_ARCHIVE_BYTES, b.getBlockSize());
				  if(local_partial)
					  reporter.incrCounter(JobCounter.LOCAL_PARTIAL_ARCHIVE_ACCESSES, 1);
				  else if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_ARCHIVE_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_ARCHIVE_ACCESSES, 1);
				  break;
			  case REMOTE:
				  reporter.incrCounter(JobCounter.TOTAL_REMOTE_ACCESSES, 1);
              reporter.incrCounter(JobCounter.TOTAL_REMOTE_BYTES, b.getBlockSize());
				  if(local_partial)
					  reporter.incrCounter(JobCounter.LOCAL_PARTIAL_REMOTE_ACCESSES, 1);
				  else if (locationToRead.equals(Reader))
					  reporter.incrCounter(JobCounter.LOCAL_REMOTE_ACCESSES, 1);
				  else
					  reporter.incrCounter(JobCounter.REMOTE_REMOTE_ACCESSES, 1);
				  break;
			  default:
				  break;
			  }
			
			  String dL = Reader + " BlockLocations:";
			  for(DatanodeInfoWithStorage  d : b.getAllLocations()){
				  dL = dL.concat(" " + d.getHostName());
			  }
			  
			  LOG.info("Read blk_" + b.getBlock().getBlockId() + " from: "
					  + b.getFirstLocation() + "(Reader: " + Reader + " "
					  + " File path: " + path.toString() + ")" + " DNS.getDefaultHost=" + dL );
		  }
	  } catch (UnknownHostException e) {
		  LOG.warn("The hostname cannot be resolved", e);
	  } catch (IOException e) {
        LOG.warn("The path: " + path.toString() + " cannot be resolved: "
               + e.getMessage());
 	  }
  }
  ...
  void runOldMapper(final JobConf job,
    ...
    try { 
      if (inputSplit.getLength() != 0) // print info in log and increment counters
            recordInputSplitLocation(
                  getInputStreamFromOldReader(
                        ((TrackedRecordReader<INKEY, INVALUE>) in).getRawRecordReader()),
                  job, inputSplit, reporter);
  ...
  private FSDataInputStream getInputStreamFromOldReader(RecordReader reader) {
     if (reader instanceof FixedLengthRecordReader)
        return ((FixedLengthRecordReader) reader).getInputStream();
     else if (reader instanceof KeyValueLineRecordReader)
        return ((KeyValueLineRecordReader) reader).getInputStream();
     else if (reader instanceof LineRecordReader)
        return ((LineRecordReader) reader).getInputStream();
     else if (reader instanceof SequenceFileAsTextRecordReader)
        return ((SequenceFileAsTextRecordReader) reader).getInputStream();
     else if (reader instanceof SequenceFileAsBinaryRecordReader)
        return ((SequenceFileAsBinaryRecordReader) reader).getInputStream();
     else if (reader instanceof SequenceFileRecordReader)
        return ((SequenceFileRecordReader) reader).getInputStream();
     else
        return null;
  }
  ...
  void runNewMapper(final JobConf job,
    ...
    try {
      ...
      if (split.getLength() != 0) // print info in log and increment counters
         recordInputSplitLocation(input.getInputStream(), job, split, reporter);
