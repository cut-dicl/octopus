...
class JobSubmitter {
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    try {
      ...
      int maps = writeSplits(job, cluster, submitJobDir);
  ...
  int writeNewSplits(JobContext job, Cluster cluster, Path jobSubmitDir) throws IOException,
      InterruptedException, ClassNotFoundException {
    Configuration conf = job.getConfiguration();
    InputFormat<?, ?> input =
      ReflectionUtils.newInstance(job.getInputFormatClass(), conf);
    
    if (MRPrefetcher.isPrefetchingEnabled(conf) 
          && input instanceof FileInputFormat<?, ?>) {
       // Prefetch files into Octopus cache
       List<FileStatus> files = ((FileInputFormat<?, ?>) input).listStatus(job);
       MRPrefetcher.prefetchFiles(files, cluster, conf);
    }
    ...
    if (MRPrefetcher.isPrefetchingEnabled(conf)
       ...
       // Prefetch files into Octopus cache using the input splits
       HashSet<FileStatus> files = new HashSet<FileStatus>();
  ...
  private int writeSplits(org.apache.hadoop.mapreduce.JobContext job,
        Cluster cluster, Path jobSubmitDir) throws IOException,
      InterruptedException, ClassNotFoundException {
    JobConf jConf = (JobConf)job.getConfiguration();
    int maps;
    if (jConf.getUseNewMapper()) {
      maps = writeNewSplits(job, cluster, jobSubmitDir);
    } else {
      maps = writeOldSplits(jConf, cluster, jobSubmitDir);
    }
    return maps;
  }
  
  //method to write splits for old api mapper.
  private int writeOldSplits(JobConf job, Cluster cluster, Path jobSubmitDir) throws IOException {
     
     org.apache.hadoop.mapred.InputFormat<?, ?> input = job.getInputFormat();

     if (MRPrefetcher.isPrefetchingEnabled(job)
           && input instanceof org.apache.hadoop.mapred.FileInputFormat<?, ?>) {
        // Prefetch files into Octopus cache
        FileStatus[] files = ((org.apache.hadoop.mapred.FileInputFormat<?, ?>) input).listStatus(job);
        MRPrefetcher.prefetchFiles(Arrays.asList(files), cluster, job);
     }
    ...
    if (MRPrefetcher.isPrefetchingEnabled(job)
       ...
       for(int i = 0; i < splits.length; ++i) {
          org.apache.hadoop.mapred.InputSplit split = splits[i];
          if (split instanceof org.apache.hadoop.mapred.FileSplit) {
             try {
                Path path = ((org.apache.hadoop.mapred.FileSplit) split).getPath();
                if (path != null) {
       ...
       if (!files.isEmpty()) {
          MRPrefetcher.prefetchFiles(files, cluster, job);
       }
    }
