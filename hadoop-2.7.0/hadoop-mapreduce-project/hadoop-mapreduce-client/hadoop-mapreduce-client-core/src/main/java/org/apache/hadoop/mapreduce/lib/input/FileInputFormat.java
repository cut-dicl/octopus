...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
  ...
  protected FileSplit makeSplit(Path file, long start, long length,
                                String[] hosts, StorageType[] storageTypes) {
    return new FileSplit(file, start, length, hosts, storageTypes);
  }
   ...
   */
  protected FileSplit makeSplit(Path file, long start, long length, 
                                String[] hosts, StorageType[] storageTypes, String[] inMemoryHosts) {
    return new FileSplit(file, start, length, hosts, storageTypes, inMemoryHosts);
  }
      ...
      if (length != 0) {
        ...
        if (isSplitable(job, path)) {
          ...
          splits.add(makeSplit(path, 0, length, blkLocations[0].getHosts(),
                      blkLocations[0].getStorageTypes(),
                      blkLocations[0].getCachedHosts()));
        }
      } else { 
        //Create empty hosts array for zero length files
        splits.add(makeSplit(path, 0, length, new String[0], new StorageType[0]));
      }
    }
