...
public class JobSplitWriter {
  ...
  throws IOException, InterruptedException {
    ...
    if (array.length != 0) {
      ...
      for(T split: array) {
        ...
        long inputDataStart = -1l;
        if (split instanceof FileSplit) {
           inputDataStart = ((FileSplit) split).getStart();
        } else if (split instanceof org.apache.hadoop.mapred.FileSplit) {
           inputDataStart = ((org.apache.hadoop.mapred.FileSplit) split).getStart();
        }
        
        info[i++] = 
          new JobSplit.SplitMetaInfo( split.getPath(),
              locations, split.getStorageTypes(), split.getLocationInfo(),
              offset, inputDataStart, split.getLength());
  ...
  private static SplitMetaInfo[] writeOldSplits(
    ...
    if (splits.length != 0) {
      ...
      for(org.apache.hadoop.mapred.InputSplit split: splits) {
        ...
        // Get internal file split (if applicable)
        if (split instanceof org.apache.hadoop.mapred.lib.TaggedInputSplit)
           split = ((org.apache.hadoop.mapred.lib.TaggedInputSplit) split).getInputSplit();
        else if (locations.length > 0 && split.getStorageTypes().length == 0)
           split = getInternalSplit(split);
        
        SplitLocationInfo[] splitInfo = null;
        if (split instanceof InputSplitWithLocationInfo)
           splitInfo = ((InputSplitWithLocationInfo) split).getLocationInfo();
        ...
        info[i++] = new JobSplit.SplitMetaInfo( split.getPath(),
            locations, split.getStorageTypes(), splitInfo, offset,
            inputDataStart, split.getLength());
  ...
  private static org.apache.hadoop.mapred.InputSplit getInternalSplit(
        org.apache.hadoop.mapred.InputSplit split) {

     // A hack for getting Hive to work!
     org.apache.hadoop.mapred.InputSplit innerSplit = split;
     if (split.getClass().getName().contains("HiveInputSplit")) {
        Method getInputSplit = null;
        for (Method method : split.getClass().getMethods()) {
           if (method.getName().contains("getInputSplit")) {
              getInputSplit = method;
              break;
           }
        }
        
        if (getInputSplit != null) {
           try {
            Object returnValue = getInputSplit.invoke(split);
            if (returnValue != null && returnValue instanceof org.apache.hadoop.mapred.InputSplit) {
               innerSplit = (org.apache.hadoop.mapred.InputSplit) returnValue;
            }
