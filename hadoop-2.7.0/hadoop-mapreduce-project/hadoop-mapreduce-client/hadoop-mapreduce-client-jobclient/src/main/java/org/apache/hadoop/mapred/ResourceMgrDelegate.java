    ...
    try {
      ...
      ClusterMetrics oldMetrics =
          new ClusterMetrics(1, 1, 1, 1, 1, 1,
              ...
              metrics.getNumNodeManagers(), 0, 0, 0,
              metrics.getNumAllocatedMemory(),
              metrics.getNumAvailableMemory(), 
              metrics.getNumReservedMemory(), 
              metrics.getNumAllocatedCores(), 
              metrics.getNumAvailableCores(), 
              metrics.getNumReservedCores(), 
              metrics.getNumAppsRunning(), metrics.getNumAppsPending());
      return oldMetrics;
    } catch (YarnException e) {
      throw new IOException(e);
    }
