...
public abstract class AMRMClient<T extends AMRMClient.ContainerRequest> extends
  ...
  public static class ContainerRequest {
    ...
    final HashMap<String, Integer> preferences; // // map host/rack to preference
    ...
    public ContainerRequest(Resource capability, String[] nodes,
      ...
      this(capability, nodes, racks, priority, true, null, null);
    }
    ...
    public ContainerRequest(Resource capability, String[] nodes,
      ...
      this(capability, nodes, racks, priority, relaxLocality, null, null);
    }
    ...
    public ContainerRequest(Resource capability, String[] nodes,
       ...
       this(capability, nodes, racks, priority, relaxLocality, nodeLabelsExpression, null);
    }
     ...
     *          asking for only a single node label
     * @param preferences
     *          Set node/rack preferences. It is expected to have the same
     *          size as nodes. The lower the better.
     */
    public ContainerRequest(Resource capability, String[] nodes,
        String[] racks, Priority priority, boolean relaxLocality,
        String nodeLabelsExpression, int[] preferences) {
      // Validate request
      ...
      if (preferences != null && nodes != null && nodes.length > 0
            && preferences.length == nodes.length) {
         for (int i = 0; i < nodes.length; ++i)
            this.preferences.put(nodes[i], preferences[i]);
         
         int min = preferences[0];
         for (int i = 1; i < preferences.length; ++i)
            if (preferences[i] < min)
               min = preferences[i];
         
         if (racks != null)
            for (int i = 0; i < racks.length; ++i)
               this.preferences.put(racks[i], min);
      }
    }
    ...
    public Integer getPreference(String location) {
