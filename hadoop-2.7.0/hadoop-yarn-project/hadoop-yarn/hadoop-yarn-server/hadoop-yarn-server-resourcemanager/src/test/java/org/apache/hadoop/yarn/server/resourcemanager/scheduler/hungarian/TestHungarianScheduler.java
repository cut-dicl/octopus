...
public class TestHungarianScheduler {
   private static final Log LOG = LogFactory.getLog(TestHungarianScheduler.class);
   ...
   private org.apache.hadoop.yarn.server.resourcemanager.NodeManager registerNode(String hostName,
         int containerManagerPort, int nmHttpPort, String rackName, Resource capability)
         throws IOException, YarnException {
      return new org.apache.hadoop.yarn.server.resourcemanager.NodeManager(hostName,
            containerManagerPort, nmHttpPort, rackName, capability, resourceManager);
   }
   ...
   @Test(timeout = 5000)
   public void testHungarianSchedulerCapacityWhenNoNMs() {
      HungarianScheduler scheduler = new HungarianScheduler();
   ...
   public void testAppAttemptMetrics() throws Exception {
      ...
      RMContext rmContext = new RMContextImpl(dispatcher, null, null, null, null, null, null, null,
            null, writer, scheduler);
   ...
   public void testNodeLocalAssignment() throws Exception {
      ...
      RMContext rmContext = new RMContextImpl(dispatcher, null, null, null, null, null,
            containerTokenSecretManager, nmTokenSecretManager, null, writer, scheduler);
      ((RMContextImpl) rmContext).setSystemMetricsPublisher(mock(SystemMetricsPublisher.class));
   ...
   public void testUpdateResourceOnNode() throws Exception {
      ...
      Map<NodeId, FiCaSchedulerNode> schedulerNodes = (Map<NodeId, FiCaSchedulerNode>) method
            .invoke(scheduler);
      ...
      NodeResourceUpdateSchedulerEvent node0ResourceUpdate = new NodeResourceUpdateSchedulerEvent(
            node0,
            ResourceOption.newInstance(newResource, RMNode.OVER_COMMIT_TIMEOUT_MILLIS_DEFAULT));
   ...
   public void testAssignments() throws Exception {
      ...
      HungarianScheduler scheduler = new HungarianScheduler();
      RMContext rmContext = new RMContextImpl(dispatcher, expirer, null, null, null, null,
            containerTokenSecretManager, nmTokenSecretManager, null, writer, scheduler);
      ((RMContextImpl) rmContext).setSystemMetricsPublisher(mock(SystemMetricsPublisher.class));
      ...
      for (int i = 0; i < nConts * 3; ++i) {
         ...
         nodeLocal.incrCountPreference(1 + rand.nextInt(3));
      }

      ResourceRequest rackLocal = createResourceRequest(memory, nodes.get(0).getRackName(),
            priority, nConts);
      ResourceRequest any = createResourceRequest(memory, ResourceRequest.ANY, priority, nConts + 2);
      ...
      Allocation allocation = scheduler.allocate(appAttemptId, new ArrayList<ResourceRequest>(),
            new ArrayList<ContainerId>(), null, null);
      ArrayList<ContainerId> release = new ArrayList<ContainerId>();
      for (Container container : allocation.getContainers()) {
         if (container.getNodeId().getHost().equals("host3")) {
            release.add(container.getId());
         }
      }

      scheduler.allocate(appAttemptId, new ArrayList<ResourceRequest>(), release, null, null);
      ...
      LOG.info("Assignment done in (ms):\t" + (endTime - startTime));
   }

//  @Test
   public void testHungarianScheduler() throws Exception {

      LOG.info("--- START: testHungarianScheduler ---");

      final int GB = 1024;

      // Register node1
      String host_0 = "host_0";
      org.apache.hadoop.yarn.server.resourcemanager.NodeManager nm_0 = registerNode(host_0, 1234,
            2345, NetworkTopology.DEFAULT_RACK, Resources.createResource(4 * GB, 1));
      nm_0.heartbeat();

      // Register node2
      String host_1 = "host_1";
      org.apache.hadoop.yarn.server.resourcemanager.NodeManager nm_1 = registerNode(host_1, 1234,
            2345, NetworkTopology.DEFAULT_RACK, Resources.createResource(2 * GB, 1));
      nm_1.heartbeat();

      // ResourceRequest priorities
      Priority priority_0 = org.apache.hadoop.yarn.server.resourcemanager.resource.Priority
            .create(0);
      Priority priority_1 = org.apache.hadoop.yarn.server.resourcemanager.resource.Priority
            .create(1);
      ...
      LOG.info("Send a heartbeat to kick the tires on the Scheduler... "
            + "nm0 -> task_0_0 and task_1_0 allocated, used=4G " + "nm1 -> nothing allocated");
      ...
      // checkNodeResourceUsage(1*GB, nm_0); // final over-commit -> rm.node->1G,
      // test.node=2G
      checkNodeResourceUsage(2 * GB, nm_1);
      ...
      checkApplicationResourceUsage(0 * GB, application_1);
      // checkNodeResourceUsage(2*GB, nm_0); // final over-commit, rm.node->1G,
      // test.node->2G
      ...
      LOG.info("--- END: testHungarianScheduler ---");
   }
   ...
   public void testBlackListNodes() throws Exception {
      ...
      rm.start();
      HungarianScheduler fs = (HungarianScheduler) rm.getResourceScheduler();
      ...
      fs.allocate(appAttemptId, Collections.<ResourceRequest>emptyList(),
            Collections.<ContainerId>emptyList(), Collections.singletonList(host), null);
      Assert.assertTrue(fs.getApplicationAttempt(appAttemptId).isBlacklisted(host));
      fs.allocate(appAttemptId, Collections.<ResourceRequest>emptyList(),
            Collections.<ContainerId>emptyList(), null, Collections.singletonList(host));
   ...
   @Test
   public void testAddAndRemoveAppFromScheduler() throws Exception {
      Configuration conf = new Configuration();
      conf.setClass(YarnConfiguration.RM_SCHEDULER, HungarianScheduler.class,
            ResourceScheduler.class);
      MockRM rm = new MockRM(conf);
      @SuppressWarnings("unchecked")
      AbstractYarnScheduler<SchedulerApplicationAttempt, SchedulerNode> fs = (AbstractYarnScheduler<SchedulerApplicationAttempt, SchedulerNode>) rm
            .getResourceScheduler();
      TestSchedulerUtils.verifyAppAddedAndRemovedFromScheduler(fs.getSchedulerApplications(), fs,
            "queue");
      rm.close();
   }
   ...
   public static void main(String[] arg) throws Exception {
      TestHungarianScheduler t = new TestHungarianScheduler();
      t.setUp();
      t.testHungarianScheduler();
      t.tearDown();
   }
