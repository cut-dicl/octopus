...
private[spark] class ExecutorAllocationManager(
  ...
  // Map host -> preference -> count  (preference = -1 for total count)
  private var hostToLocalTaskCount: Map[String, Map[Int, Int]] = Map.empty
  ...
  private[spark] class ExecutorAllocationListener extends SparkListener {
    ...
    // place the executors.
    private val stageIdToExecutorPlacementHints = new mutable.HashMap[Int, (Int, Map[String, Map[Int, Int]])]
    ...
    override def onStageSubmitted(stageSubmitted: SparkListenerStageSubmitted): Unit = {
      ...
      allocationManager.synchronized {
        ...
        val hostToLocalTaskCountPerStage = new mutable.HashMap[String, mutable.HashMap[Int, Int]]()
        stageSubmitted.stageInfo.taskLocalityPreferences.foreach { locality =>
          if (!locality.isEmpty) {
            numTasksPending += 1
            locality.foreach { location =>
              if (!hostToLocalTaskCountPerStage.contains(location.host)) {
                hostToLocalTaskCountPerStage.put(location.host, new mutable.HashMap[Int, Int]())
              }
              
              val prefereceToCount = hostToLocalTaskCountPerStage.get(location.host).get
              prefereceToCount.put(-1, 1 + prefereceToCount.getOrElse(-1, 0))
              
              location match {
                case p: PreferenceHostTaskLocation =>
                  if (p.preference >= 0)
                    prefereceToCount.put(p.preference, 
                        1 + prefereceToCount.getOrElse(p.preference, 0))
                case _ =>
              }
            }
          }
        ...
        stageIdToExecutorPlacementHints.put(stageId,
          (numTasksPending, 
              hostToLocalTaskCountPerStage.map(t => (t._1, t._2.toMap)).toMap))
    ...
    def updateExecutorPlacementHints(): Unit = {
      ...
      val localityToCounts = new mutable.HashMap[String, mutable.HashMap[Int, Int]]()
      stageIdToExecutorPlacementHints.values.foreach { case (numTasksPending, localities) =>
        localityAwareTasks += numTasksPending
        localities.foreach { case (hostname, prefCounts) =>
          prefCounts.foreach { case (preference, count) =>
            if (!localityToCounts.contains(hostname)) {
              localityToCounts.put(hostname, new mutable.HashMap[Int, Int])
            }
            val innerCounts = localityToCounts.get(hostname).get
            innerCounts.put(preference, innerCounts.getOrElse(preference, 0) + count)
          }
        }
      }

      allocationManager.localityAwareTasks = localityAwareTasks
      allocationManager.hostToLocalTaskCount = localityToCounts.map(t => (t._1, t._2.toMap)).toMap
    }
  }
