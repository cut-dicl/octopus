    ...
    val locs = hsplit match {
      case lsplit: InputSplitWithLocationInfo =>
        HadoopRDD.convertSplitLocationInfo(lsplit.getLocationInfo, 
            lsplit.getLocations, lsplit.getStorageTypes)
      case lsplit => 
        HadoopRDD.convertSplitLocationInfo(null, 
            lsplit.getLocations, lsplit.getStorageTypes)
    }
...
private[spark] object HadoopRDD extends Logging {
  ...
  private[spark] def convertSplitLocationInfo(
       infos: Array[SplitLocationInfo],
       hosts: Array[String],
       storageTypes: Array[StorageType]): Option[Seq[String]] = {
    
    if (storageTypes != null && hosts != null 
        && hosts.length > 0 && storageTypes.length == hosts.length) {
      // Exploit storage information
      Option(hosts.zip(storageTypes).filter(_._1 != "localhost")
          .map{ case (host, storType) => 
            PreferenceHostTaskLocation(host, storType.getOrderValue).toString
            }.toSeq)
    } else {
