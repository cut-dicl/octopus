...
/**
 * A location that includes both a host and a numeric preference for that host.
 */
case class PreferenceHostTaskLocation(override val host: String, preference: Int)
  extends TaskLocation {
  override def toString: String = s"${TaskLocation.preferenceLocationTag}${host}_${preference}"
}
...
private[spark] object TaskLocation {
  ...
  // Identify locations of nodes with preferences
  val preferenceLocationTag = "preference_"

  def apply(host: String, executorId: String): TaskLocation = {
    new ExecutorCacheTaskLocation(host, executorId)
  }

  def apply(host: String, preference: Int): TaskLocation = {
    new PreferenceHostTaskLocation(host, preference)
  }

  /**
   * Create a TaskLocation from a string returned by getPreferredLocations.
   * These strings have the form executor_[hostname]_[executorid], [hostname], or
   * hdfs_cache_[hostname], depending on whether the location is cached, or
   * preference_[hostname]_[preference] if there is a preference associated 
   * with the host
   */
  def apply(str: String): TaskLocation = {
    ...
    if (hstr.equals(str)) {
      if (str.startsWith(executorLocationTag)) {
        // case executor_[hostname]_[executorid]
        ...
        new ExecutorCacheTaskLocation(host, executorId)
      } else if (str.startsWith(preferenceLocationTag)) {
        // case preference_[hostname]_[preference]
        val hostAndPreference = str.stripPrefix(preferenceLocationTag)
        val splits = hostAndPreference.split("_", 2)
        require(splits.length == 2, "Illegal preference location format: " + str)
        new PreferenceHostTaskLocation(splits(0), splits(1).toInt)
      } else {
        // case [hostname]
        new HostTaskLocation(str)
      }
    } else {
      // case hdfs_cache_[hostname]
