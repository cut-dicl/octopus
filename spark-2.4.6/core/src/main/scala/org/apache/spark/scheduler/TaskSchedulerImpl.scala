...
private[spark] class TaskSchedulerImpl(
  ...
  // Options for task assignment policy
  val enableTiers = conf.getBoolean("spark.scheduler.enable.tiers", false)
  val useHungarian = conf.getBoolean("spark.scheduler.use.hungarian", false)
  val pruneExcessResources = conf.getBoolean("spark.scheduler.hungarian.prune.resources", false)
  val pruneExcessTasks = conf.getBoolean("spark.scheduler.hungarian.prune.tasks", false)
  val hungarianDelay = conf.getLong("spark.hungarian.delay", -1l); // Delay in ms
  ...
  private def resourceOffersHungarian(
      taskSet: TaskSetManager,
      shuffledOffers: Seq[WorkerOffer],
      availableCpus: Array[Int],
      tasks: IndexedSeq[ArrayBuffer[TaskDescription]],
    ...
    // Build the available offers
    val offers = new ArrayBuffer[(String, String)]()
    val availCpusCopy = availableCpus.clone()
    var moreSlots = true

    if (pruneExcessResources && availCpusCopy.sum >= 3 * taskSet.numTasks) {
      // Optimization: Create offers only on hosts that will lead to local tasks
      val hostToCountMap = taskSet.getPendingTasksForHostCounts()
      
      while (moreSlots) {
        moreSlots = false;
        // Arrange offers in round robin to ensure optimal balancing across executors
        for (i <- 0 until shuffledOffers.size) {
          val workerOffer = shuffledOffers(i)
          if (availCpusCopy(i) >= CPUS_PER_TASK 
              && hostToCountMap.getOrElse(workerOffer.host, 0) > 0) {
            offers.append((workerOffer.executorId, workerOffer.host))
            availCpusCopy(i) = availCpusCopy(i) - CPUS_PER_TASK
            hostToCountMap.update(workerOffer.host, hostToCountMap.get(workerOffer.host).get - 1)
            moreSlots = true
          }
        }
      }
      
      if (offers.length < taskSet.numTasks) {
    ...
    val execIdToOfferIndex = new HashMap[String, Int]()
    for (i <- 0 until shuffledOffers.size) {
      execIdToOfferIndex.put(shuffledOffers(i).executorId, i)
    }

    var taskDescrs: Seq[TaskDescription] = null
    try {
      // Make the offers
      taskDescrs = taskSet.resourceOffersHungarian(offers)
    } catch {
      case e: TaskNotSerializableException =>
        logError(s"Resource offer failed, task set ${taskSet.name} was not serializable")
        // Do not offer resources for this task, but don't throw an error to allow other
    ...
    for (task <- taskDescrs) {
      val tid = task.taskId
      val execId = task.executorId
      val i = execIdToOfferIndex.get(execId).get
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    for (taskSet <- sortedTaskSets) {
      ...
      if (taskSet.isBarrier && availableSlots < taskSet.numTasks) {
        ...
        if (useHungarian) {
          launchedAnyTask = resourceOffersHungarian(taskSet, shuffledOffers, availableCpus, 
              tasks, addressesWithDescs)
        } else {
