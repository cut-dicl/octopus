...
private[spark] class TaskSetManager(
  ...
  // Set of pending tasks per media per host -- similar to the above
  private val pendingTasksForMemHost = new HashMap[String, ArrayBuffer[Int]]
  private val pendingTasksForSsdHost = new HashMap[String, ArrayBuffer[Int]]
  private val pendingTasksForHddHost = new HashMap[String, ArrayBuffer[Int]]
  ...
  private[spark] def addPendingTask(index: Int) {
    for (loc <- tasks(index).preferredLocations) {
      loc match {
        ...
        case e: PreferenceHostTaskLocation =>
          e.preference match {
            case 0 => pendingTasksForMemHost.getOrElseUpdate(loc.host, new ArrayBuffer) += index
            case 3 => pendingTasksForSsdHost.getOrElseUpdate(loc.host, new ArrayBuffer) += index
            case 5 => pendingTasksForHddHost.getOrElseUpdate(loc.host, new ArrayBuffer) += index
          }
        case _ =>
      }
  ...
  private val EMPTY_BUFFER = ArrayBuffer[Int]()
  ...
  private def getPendingTasksForExecutor(executorId: String): ArrayBuffer[Int] = {
    pendingTasksForExecutor.getOrElse(executorId, EMPTY_BUFFER)
  }
  ...
  private def getPendingTasksForHost(host: String): ArrayBuffer[Int] = {
    pendingTasksForHost.getOrElse(host, EMPTY_BUFFER)
  }

  private def getPendingTasksForMemHost(host: String): ArrayBuffer[Int] = {
    pendingTasksForMemHost.getOrElse(host, EMPTY_BUFFER)
  }
  
  private def getPendingTasksForSsdHost(host: String): ArrayBuffer[Int] = {
    pendingTasksForSsdHost.getOrElse(host, EMPTY_BUFFER)
  }

  private def getPendingTasksForHddHost(host: String): ArrayBuffer[Int] = {
    pendingTasksForHddHost.getOrElse(host, EMPTY_BUFFER)
  }
  ...
  private def getPendingTasksForRack(rack: String): ArrayBuffer[Int] = {
    pendingTasksForRack.getOrElse(rack, EMPTY_BUFFER)
  }

  def getPendingTasksForHostCounts(): HashMap[String, Int] = {
    pendingTasksForHost.map{ case (host, arr) => host -> arr.length }
  }
  ...
  private def isExecOrNodeBlacklisted(execId: String, host: String): Boolean = {
    taskSetBlacklistHelperOpt.exists { blacklist =>
    ...
    : Option[(Int, TaskLocality.Value, Boolean)] =
  {
    ...
    if (sched.enableTiers) {
      if (TaskLocality.isAllowed(maxLocality, TaskLocality.MEM_LOCAL)) {
        for (index <- dequeueTaskFromList(execId, host, getPendingTasksForMemHost(host))) {
          return Some((index, TaskLocality.MEM_LOCAL, false))
        }
      }
  
      if (TaskLocality.isAllowed(maxLocality, TaskLocality.SSD_LOCAL)) {
        for (index <- dequeueTaskFromList(execId, host, getPendingTasksForSsdHost(host))) {
          return Some((index, TaskLocality.SSD_LOCAL, false))
        }
      }
  
      if (TaskLocality.isAllowed(maxLocality, TaskLocality.HDD_LOCAL)) {
        for (index <- dequeueTaskFromList(execId, host, getPendingTasksForHddHost(host))) {
          return Some((index, TaskLocality.HDD_LOCAL, false))
        }
      }
    }
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.NODE_LOCAL)) {
      for (index <- dequeueTaskFromList(execId, host, getPendingTasksForHost(host))) {
        if (sched.enableTiers)
          ...
          return Some((index, getNodeLocality(host, index), false))
      }
    }
  ...
  /**
   * Return the specific node locality
   */
  private def getNodeLocality(host: String, index: Int) : TaskLocality.Value = {
    if (pendingTasksForMemHost.contains(host))
      if (pendingTasksForMemHost.get(host).get.contains(index))
        return TaskLocality.MEM_LOCAL
    
    if (pendingTasksForSsdHost.contains(host))
      if (pendingTasksForSsdHost.get(host).get.contains(index))
        return TaskLocality.SSD_LOCAL

    if (pendingTasksForHddHost.contains(host))
      if (pendingTasksForHddHost.get(host).get.contains(index))
        return TaskLocality.HDD_LOCAL

    return TaskLocality.NODE_LOCAL
  }
  
  /**
   * Given multiple offers (executor, host), assign pending tasks on them
   * using the Hungarian algorithm.
   */
  @throws[TaskNotSerializableException]
  def resourceOffersHungarian(offers: Seq[(String, String)])
    : Seq[TaskDescription] =
  {
    val taskDescrs = new ArrayBuffer[TaskDescription]()

    // Filter valid offers
    val validOffers = offers.filterNot{ 
      case (execId, host) => isExecOrNodeBlacklisted(execId, host)
      }
    
    if (isZombie || validOffers.isEmpty)
      return taskDescrs
    
    // Filter valid pending tasks
    var validPendingTasks = allPendingTasks.filter(
        index => copiesRunning(index) == 0 && !successful(index)).distinct
    
    if (validPendingTasks.isEmpty)
      return taskDescrs
    
    if (sched.pruneExcessTasks 
        && pendingTasksForExecutor.isEmpty
        && validPendingTasks.length >= 3 * validOffers.length) {
      // Optimization: Select a subset of the tasks to consider for Hungarian
      val hostToOfferCount = scala.collection.mutable.Map[String, Int]()
      validOffers.foreach{ case (exec, host) => 
        hostToOfferCount.update(host, hostToOfferCount.getOrElse(host, 0) + 1)
        }
      
      val potentialTasks = scala.collection.mutable.Set[Int]()
      if (sched.enableTiers && !pendingTasksForMemHost.isEmpty) {
        hostToOfferCount.foreach{ case (host, count) =>
          potentialTasks ++= this.getPendingTasksForMemHost(host).take(2 * count) 
          }
      } else {
        hostToOfferCount.foreach{ case (host, count) =>
          potentialTasks ++= this.getPendingTasksForHost(host).take(5 * count) 
          }
      }
      
      if (potentialTasks.size >= validOffers.length) {
        val filteredTasks = validPendingTasks.filter(t => potentialTasks.contains(t))
        if (filteredTasks.size >= validOffers.length) {
          validPendingTasks = filteredTasks
        }
      }
    }
      
    // Initialize the cost matrix
    val numTasks = validPendingTasks.length
    val numOffers = validOffers.length
    var costMatrix = Array.ofDim[Int](numTasks, numOffers)
    
    for (t <- 0 until numTasks) {
      for (o <- 0 until numOffers) {
        costMatrix(t)(o) = computeCost(validPendingTasks(t), validOffers(o));
      }
    }
    ...
    var numAssignments: Int = 0
    val delayCost = if (sched.enableTiers) 4 else 9
    
    for (t <- 0 until results.length) {
      if (results(t) >= 0) {
        val offer = validOffers(results(t))
        val index = validPendingTasks(t)
        val cost = costMatrix(t)(results(t))
        
        if (cost < delayCost || cost == 10 || 
            (cost < 30 && curTime - lastLaunchTime > sched.hungarianDelay)) {
          // Found a task; do some bookkeeping and return a task description
          taskDescrs += assignTaskToExec(index, offer._1, offer._2, 
              getLocalityFromCost(cost, index, offer._2), curTime, false)
          numAssignments += 1
    ...
    logInfo("Hungarian in TaskSet: pendingTasks = %d, resourceOffers = %d, assignments = %d"
      .format(numTasks, numOffers, numAssignments))
      
    return taskDescrs
  }
  
  /**
   * Compute the cost of assigning the task (represented by the index)
   * to the offer (execID, host)
   */
  private def computeCost(index: Int, offer: (String, String)): Int = {
    val execId = offer._1
    val host = offer._2
    ...
    if (getPendingTasksForExecutor(execId).contains(index))
      return 0
    
    if (sched.enableTiers) {
      if (getPendingTasksForMemHost(host).contains(index))
        return 1
      if (getPendingTasksForSsdHost(host).contains(index))
        return 3
      if (getPendingTasksForHddHost(host).contains(index))
        return 5
    }

    if (getPendingTasksForHost(host).contains(index))
      return 8
    if (pendingTasksWithNoPrefs.contains(index))
      return 10

    val rack = sched.getRackForHost(host)
    if (!rack.isEmpty && getPendingTasksForRack(rack.get).contains(index))
      return 15
  ...
  /**
   * Determine locality based on the cost computed by computeCost()
   */
  private def getLocalityFromCost(cost: Int, index: Int, host: String) : TaskLocality.TaskLocality = {
    cost match {
      case 0 => TaskLocality.PROCESS_LOCAL
      case 1 => TaskLocality.MEM_LOCAL
      case 3 => TaskLocality.SSD_LOCAL
      case 5 => TaskLocality.HDD_LOCAL
      case 8 => if (sched.enableTiers) TaskLocality.NODE_LOCAL else getNodeLocality(host, index)
      case 10 => TaskLocality.NO_PREF
      case 15 => TaskLocality.RACK_LOCAL
      case 20 => TaskLocality.ANY
      case _ => null
    }
  }
    ...
    : Option[TaskDescription] =
  {
    ...
    if (!isZombie && !offerBlacklisted) {
      ...
      dequeueTask(execId, host, allowedLocality).map { case ((index, taskLocality, speculative)) =>
        ...
        // Found a task; do some bookkeeping and return a task description
        assignTaskToExec(index, execId, host, taskLocality, curTime, speculative)
      }
    } else {
      None
    }
  ...
  /**
   * Assign a task to a particular executor and host. Return the TaskDescription
   */
  @throws[TaskNotSerializableException]
  private def assignTaskToExec(
      index: Int,
      execId: String,
      host: String,
  ...
  private def getAllowedLocalityLevel(curTime: Long): TaskLocality.TaskLocality = {
    ...
    while (currentLocalityIndex < myLocalityLevels.length - 1) {
      val moreTasks = myLocalityLevels(currentLocalityIndex) match {
        ...
        case TaskLocality.MEM_LOCAL => moreTasksToRunIn(pendingTasksForMemHost)
        case TaskLocality.SSD_LOCAL => moreTasksToRunIn(pendingTasksForSsdHost)
        case TaskLocality.HDD_LOCAL => moreTasksToRunIn(pendingTasksForHddHost)
  ...
  private def getLocalityWait(level: TaskLocality.TaskLocality): Long = {
    ...
    val localityWaitKey = level match {
      ...
      case TaskLocality.MEM_LOCAL => "spark.locality.wait.memnode"
      case TaskLocality.SSD_LOCAL => "spark.locality.wait.ssdnode"
      case TaskLocality.HDD_LOCAL => "spark.locality.wait.hddnode"
  ...
  private def computeValidLocalityLevels(): Array[TaskLocality.TaskLocality] = {
    import TaskLocality.{PROCESS_LOCAL, MEM_LOCAL, SSD_LOCAL, HDD_LOCAL, NODE_LOCAL, NO_PREF, RACK_LOCAL, ANY}
    ...
    //    if (sched.enableTiers) {
    //      if (!pendingTasksForMemHost.isEmpty &&
    //          pendingTasksForMemHost.keySet.exists(sched.hasExecutorsAliveOnHost(_))) {
    //        levels += MEM_LOCAL
    //      }
    //      if (!pendingTasksForSsdHost.isEmpty &&
    //          pendingTasksForSsdHost.keySet.exists(sched.hasExecutorsAliveOnHost(_))) {
    //        levels += SSD_LOCAL
    //      }
    //      if (!pendingTasksForHddHost.isEmpty &&
    //          pendingTasksForHddHost.keySet.exists(sched.hasExecutorsAliveOnHost(_))) {
    //        levels += HDD_LOCAL
    //      }
    //    }
