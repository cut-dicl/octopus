...
private[ui] class StagePage(parent: StagesTab, store: AppStatusStore) extends WebUIPage("stage") {
  ...
  private def getLocalitySummaryString(localitySummary: Map[String, Long]): String = {
    val names = Map(
      ...
      TaskLocality.MEM_LOCAL.toString() -> "Memory local",
      TaskLocality.SSD_LOCAL.toString() -> "SSD local",
      TaskLocality.HDD_LOCAL.toString() -> "HDD local",
      TaskLocality.NO_PREF.toString() -> "No preference",
