...
class TaskSchedulerImplSuite extends SparkFunSuite with LocalSparkContext with BeforeAndAfterEach
  ...
  test("Schedule tasks with tier awareness") {
    
    for (scenario <- Seq((false, false), (false, true), (true, false), (true, true))) {
    
      val taskScheduler = setupSchedulerNoShuffle(
      ...
      // Create offers 
      val offers = IndexedSeq(
        new WorkerOffer("executor1", "host1", 3),
        new WorkerOffer("executor2", "host2", 2),
        new WorkerOffer("executor3", "host2", 2),
        new WorkerOffer("executor4", "host3", 3)
      )
  
      // Create tasks with prefered locations
      val locations = ArrayBuffer[Seq[TaskLocation]]()
      locations += Seq(new ExecutorCacheTaskLocation("host2", "executor2"), 
          new PreferenceHostTaskLocation("host3", 5))
      locations += Seq(new PreferenceHostTaskLocation("host1", 5), 
          new PreferenceHostTaskLocation("host2", 0),
          new PreferenceHostTaskLocation("host3", 3))
      locations += Seq(new PreferenceHostTaskLocation("host1", 3),
          new PreferenceHostTaskLocation("host3", 5))
      ...
      assert(locations.length === taskDescriptions.length)
      assert(!failedTaskSet)
