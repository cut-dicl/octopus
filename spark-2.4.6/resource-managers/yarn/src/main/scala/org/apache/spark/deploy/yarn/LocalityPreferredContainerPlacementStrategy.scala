...
private[yarn] case class ContainerLocalityPreferences(
    nodes: Array[String], racks: Array[String], prefereces: Array[Int])
...
private[yarn] class LocalityPreferredContainerPlacementStrategy(
  ...
  def localityOfRequestedContainers(
    ...
    val hostToLocalTaskGlobalCount = hostToLocalTaskCount.map {
      case (host, preferences) => (host, preferences.get(-1).get)
    }
    
    val updatedHostToContainerCount = expectedHostToContainerCount(
      numLocalityAwareTasks, hostToLocalTaskGlobalCount, allocatedHostToContainersMap,
    ...
    if (requiredLocalityAwareContainerNum > 0) {
      ...
      // Copy the host to preference counts without the "-1" entry
      val hostToPreferencesCount = hostToLocalTaskCount.map {
        case (host, preferences) => 
          val prefs = collection.mutable.Map(preferences.toSeq: _*)
          prefs.remove(-1)
          (host, prefs)
      }
      ...
      for (i <- 0 until requiredLocalityAwareContainerNum) {
        ...
        val countPrefs = hostToPreferencesCount.values.map(_.size).sum
        if (countPrefs > 0) {
          // Collect the host preferences
          val prefs = hosts.map {h =>
            val hp = hostToPreferencesCount.get(h).get
            if (!hp.isEmpty) {
              val pref = hp.keys.min
              hp.put(pref, hp.get(pref).get - 1)
              if (hp.get(pref).get <= 0)
                hp.remove(pref)
              pref
            } else {
              9 // use number higher than num storage types
            }
          }
          
          containerLocalityPreferences += ContainerLocalityPreferences(
              hosts, racks.toArray, prefs)
          
        } else {
          // No host preferences
          containerLocalityPreferences += ContainerLocalityPreferences(
              hosts, racks.toArray, null.asInstanceOf[Array[Int]])
        }
