...
private[yarn] class YarnAllocator(
  ...
  private var hostToLocalTaskCounts: Map[String, Map[Int, Int]] = Map.empty
  ...
  // Option for allocation policy
  private val useHungarian: Boolean = sparkConf.get(ALLOCATOR_USE_HUNGARIAN);
  private val enableTiers: Boolean = sparkConf.get(ALLOCATOR_ENABLE_TIERS);
  ...
  def requestTotalExecutorsWithPreferredLocalities(
      ...
      hostToLocalTaskCount: Map[String, Map[Int, Int]],
  ...
  def updateResourceRequests(): Unit = {
    ...
    if (missing > 0) {
      ...
      containerLocalityPreferences.foreach {
        case ContainerLocalityPreferences(nodes, racks, prefs) 
          if nodes != null =>
            newLocalityRequests += createContainerRequest(resource, nodes, racks, prefs)
        case _ =>
      }
      ...
      if (availableContainers >= newLocalityRequests.size) {
        ...
        for (i <- 0 until (availableContainers - newLocalityRequests.size)) {
          newLocalityRequests += createContainerRequest(resource, null, null, null)
        }
      } else {
  ...
  private def hostStr(request: ContainerRequest): String = {
    val prefs = request.getPreferences.asScala
  ...
  private def createContainerRequest(
      ...
      nodes: Array[String],
      racks: Array[String],
      preferences: Array[Int]): ContainerRequest = {
    new ContainerRequest(resource, nodes, racks, RM_REQUEST_PRIORITY, true, labelExpression.orNull, preferences)
  }
  ...
  def handleAllocatedContainers(allocatedContainers: Seq[Container]): Unit = {
    ...
    val remainingContainers = new ArrayBuffer[Container]

    if (useHungarian)
      handleAllocatedContainersHungarian(allocatedContainers, containersToUse, remainingContainers)
    else
      handleAllocatedContainersDefault(allocatedContainers, containersToUse, remainingContainers)
    
    if (!remainingContainers.isEmpty) {
      logDebug(s"Releasing ${remainingContainers.size} unneeded containers that were " +
        s"allocated to us")
      for (container <- remainingContainers) {
        internalReleaseContainer(container)
      }
    }
  ...
  /**
   * Default approach for assigning requests to containers
   * 
   * Places the containers into containersToUse or remainingContainers.
   */
  private def handleAllocatedContainersDefault(
      allocatedContainers: Seq[Container],
      containersToUse: ArrayBuffer[Container],
      remainingContainers: ArrayBuffer[Container]): Unit = {
  ...
  private def matchContainerToRequest(
    ...
    if (!matchingRequests.isEmpty) {
      val containerRequest = if (enableTiers) {
        // Find request with lowest preference for this location
        var minPref: Int = Int.MaxValue
        var minRequest: ContainerRequest = null;
        
        matchingRequests.asScala.flatMap(m => m.asScala).map{request =>
          val pref = request.getPreference(location)
          if (pref != null && pref < minPref) {
            minPref = pref
            minRequest = request
          }
        }
        ...
        // Default approach: match with the first container request
        matchingRequests.get(0).iterator.next
      }
  ...
  /**
   * Assigning requests to containers using the Hungarian algorithm
   * 
   * Places the containers into containersToUse or remainingContainers.
   */
  private def handleAllocatedContainersHungarian(
      allocatedContainers: Seq[Container],
      containersToUse: ArrayBuffer[Container],
      remainingContainers: ArrayBuffer[Container]): Unit = {
    
    val contsPerPriority = allocatedContainers.groupBy(c => c.getPriority)
    val contsPerPrioPerRes = contsPerPriority.map{ case (p, lc) => 
      (p, lc.groupBy(c => c.getResource.getMemory))}
    
    contsPerPrioPerRes.map{ case (prio, conts1) => 
      conts1.map{ case (mem, conts2) => 
        handleAllocatedContainersHungarianInternal(prio, mem, conts2, 
            containersToUse, remainingContainers)
      }
    }
  }

  private def handleAllocatedContainersHungarianInternal(
      priority: Priority,
      memory: Int,
      allocatedContainers: Seq[Container],
      containersToUse: ArrayBuffer[Container],
    ...
    // SPARK-6050: see matchContainerToRequest above
    val matchingResource = Resource.newInstance(memory, resource.getVirtualCores)
    val matchingRequests = amClient.getMatchingRequests(
        priority, ANY_HOST, matchingResource).asScala.flatten(
            inner => inner.asScala).toSeq

    if (matchingRequests.isEmpty) {
      // Unable to find matching requests
      remainingContainers ++= allocatedContainers
      return
    }
    
    // Initialize the cost matrix
    val numAlloc = allocatedContainers.length
    val numReq = matchingRequests.length
    var costMatrix = Array.ofDim[Int](numAlloc, numReq)
    
    for (a <- 0 until numAlloc) {
      for (r <- 0 until numReq) {
        costMatrix(a)(r) = computeCost(allocatedContainers(a), matchingRequests(r));
      }
    }
    ...
    for (a <- 0 until results.length) {
      if (results(a) >= 0) {
        amClient.removeContainerRequest(matchingRequests(results(a)))
        containersToUse += allocatedContainers(a)
      } else {
        remainingContainers += allocatedContainers(a)
      }
    }
    
    logInfo("Hungarian for priority %d: allocations = %d, requests = %d, assignments = %d"
      .format(priority.getPriority, numAlloc, numReq, results.length))
  }

  /**
   * Compute the cost of assigning the allocated container to the request
   */
  private def computeCost(container: Container, request: ContainerRequest): Int = {
    
    val host = container.getNodeId.getHost
    val prefs = request.getPreferences
    ...
    val rack = resolver.resolve(conf, host)
    if (prefs.containsKey(rack)) {
        // rack-local cost
        if (enableTiers)
            return 10 + prefs.get(rack)
        else
            return 10
    }
