...
package object config {
  ...
  private[spark] val ALLOCATOR_ENABLE_TIERS = 
    ConfigBuilder("spark.yarn.allocator.enable.tiers")
      .doc("Enable tier awareness in YarnAllocator.")
      .booleanConf
      .createWithDefault(false)

  private[spark] val ALLOCATOR_USE_HUNGARIAN = 
    ConfigBuilder("spark.yarn.allocator.use.hungarian")
      .doc("Enable the Hungarian allocation algorithm in YarnAllocator.")
      .booleanConf
      .createWithDefault(false)
