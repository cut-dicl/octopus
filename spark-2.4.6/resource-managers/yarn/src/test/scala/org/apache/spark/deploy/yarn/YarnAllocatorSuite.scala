...
class YarnAllocatorSuite extends SparkFunSuite with Matchers with BeforeAndAfterEach {
  ...
  test("make host local requests") {
    sparkConf.set("spark.yarn.allocator.enable.tiers", "true")
    val handler = createAllocator(4)
    val hostToLocalCounts = Map("host1" -> Map(-1 -> 2, 0 -> 1, 3 -> 1), 
                                "host2" -> Map(-1 -> 4, 0 -> 1, 3 -> 2, 5 -> 1), 
                                "host3" -> Map(-1 -> 2, 3 -> 1, 5 -> 1))
    handler.requestTotalExecutorsWithPreferredLocalities(4, 4, hostToLocalCounts, Set.empty)
    handler.updateResourceRequests()
    handler.getNumExecutorsRunning should be (0)
    handler.getPendingAllocate.size should be (4)
    ...
    handler.getNumExecutorsRunning should be (2)
    handler.allocatedContainerToHostMap.get(containers(0).getId).get should be ("host1")
    handler.allocatedHostToContainersMap.get("host1").get should contain (containers(0).getId)
    handler.allocatedContainerToHostMap.get(containers(1).getId).get should be ("host3")
    handler.allocatedHostToContainersMap.get("host3").get should contain (containers(1).getId)

    val size = rmClient.getMatchingRequests(containers(0).getPriority, "host1", containerResource).size
    size should be (1)
  }

  test("make host local requests using Hungarian algorithm") {
    sparkConf.set("spark.yarn.allocator.use.hungarian", "true")
    sparkConf.set("spark.yarn.allocator.enable.tiers", "true")
    val handler = createAllocator(4)
    val hostToLocalCounts = Map("host1" -> Map(-1 -> 2, 0 -> 1, 3 -> 1), 
                                "host2" -> Map(-1 -> 4, 0 -> 1, 3 -> 2, 5 -> 1), 
