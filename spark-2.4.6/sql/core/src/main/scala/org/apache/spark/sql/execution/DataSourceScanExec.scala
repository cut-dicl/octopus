...
case class FileSourceScanExec(
  ...
  private def createBucketedReadRDD(
    ...
    val filesGroupedToBuckets =
      selectedPartitions.flatMap { p =>
        p.files.map { f =>
          val (hosts, types) = getBlockHosts(getBlockLocations(f), 0, f.getLen)
          PartitionedFile(p.values, f.getPath.toUri.toString, 0, f.getLen, hosts, types)
        }
      }.groupBy { f =>
        BucketingUtils
      ...
      partition.files.flatMap { file =>
        ...
        if (fsRelation.fileFormat.isSplitable(
          ...
          (0L until file.getLen by maxSplitBytes).map { offset =>
            ...
            val (hosts, types) = getBlockHosts(blockLocations, offset, size)
            PartitionedFile(
              partition.values, file.getPath.toUri.toString, offset, size, hosts, types)
          }
        } else {
          val (hosts, types) = getBlockHosts(blockLocations, 0, file.getLen)
          Seq(PartitionedFile(
            partition.values, file.getPath.toUri.toString, 0, file.getLen, hosts, types))
        }
      }
  ...
  // fraction the segment, and returns location hosts and storage types of that block. 
  // If no such block can be found, returns two empty arrays.
  private def getBlockHosts(
    ...
    val candidates = blockLocations.map {
      ...
      case b if b.getOffset <= offset && offset < b.getOffset + b.getLength =>
        (b.getHosts, b.getStorageTypes, (b.getOffset + b.getLength - offset).min(length))

      // The fragment ends at a position within this block
      case b if offset <= b.getOffset && offset + length < b.getLength =>
        (b.getHosts, b.getStorageTypes, (offset + length - b.getOffset).min(length))

      // The fragment fully contains this block
      case b if offset <= b.getOffset && b.getOffset + b.getLength <= offset + length =>
        (b.getHosts, b.getStorageTypes, b.getLength)

      // The fragment doesn't intersect with this block
      case b =>
        (b.getHosts, b.getStorageTypes, 0L)
    }.filter { case (hosts, types, size) =>
      size > 0L
    }
    ...
    if (candidates.isEmpty) {
      ...
      val (hosts, types, _) = candidates.maxBy { case (_, _, size) => size }
      if (types == null)
        return (hosts, Array.empty[Int])
      else
        (hosts, types.map(_.getOrderValue))
    }
  }
